...
Run the door script from the plugin's own directory. Claude Code exports the
plugin's absolute path as the environment variable CLAUDE_PLUGIN_ROOT;
substitute it for `<plugin-root>` below.

    node <plugin-root>/lib/door.mjs init <dir>
