...
Claude Code exports the plugin's absolute path as the environment variable
CLAUDE_PLUGIN_ROOT; substitute it for `<plugin-root>` below.

    node <plugin-root>/lib/door.mjs badge <receipt.json>
...
4. If retained transcripts sit beside the receipt under
   ...
   the retained generation text and refuse — naming the case and the digest —
   on any mismatch.
...
lie. When the transcripts are there, step 4 checks `generation_hash` for real
and the statement says so; `judge_sample_hashes` stays unchecked even then,
because the judge sample texts are not retained.
