...
2. **Promote: render published, retire the draft, move the receipts. One step.**
   ...
   page stops matching the approved one. The gate asserts that this command
   reproduces the approved page byte-for-byte.
   ...
   `build-public.sh` excludes every `*-draft/` path from the published tree.
   ...
   that the frozen run-time projection ($20.89) and its label survive, that the
   run record and the queue entry state the same figure, and that the rendered
   receipts path moved. Per **DECISIONS #13** the guard runs the shipped
   invocation, not an analog of it.
...
2. **The gate is green on that tree.** `bash ~/build-public.sh` rebuilds it and
