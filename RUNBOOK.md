...
CONSTITUTION § Proportional oversight sets what a merge needs, by tier. No merge runs a sweep. The
spec's own gate and the core set run on the merge result, and the nightly sweeps `dev`
(§ The nightly sweep).

**T1 and T2: run the precondition check first. Every time.**
...
**T3 has no approval, so `merge-check.js` refuses it. Run its hygiene scan over the branch tip:**

```sh
node -e 'const r = require("./scripts/merge-check.js").hygieneScan("spec/NNN-slug"); console.log(r.error || `${r.hits.length} hit(s)`); process.exit(r.error || r.hits.length ? 1 : 0)'
git checkout dev && git merge --no-ff spec/NNN-slug -m "merge(spec NNN): ... - T3, no approval"
```

**Then, on the merge result, whatever the tier:**

```sh
bash specs/NNN-slug/gate.sh --final
node specs/029-scanner-loop/probes/sweep.mjs --core NNN-slug    # the core set, against the last nightly
node tests/gate.js
```

`--core` compares each core gate with the last nightly's emission, or with the emission committed
in the tree until a nightly has run. It exits 1 on a new red. When other sessions are running gates,
re-run a red gate alone before it counts: on 24 Sep 2026 the first two `--core` reds were a real defect and
an edit made mid-run, and only the alone re-runs told them apart. The same session writes the
DECISIONS entry, updates `BACKLOG.md` § Status (the merge row, what is in flight, and the findings
the merge carries, each naming its DECISIONS entry), and pushes `dev` to `private`.
...
One full sweep of `dev` runs each night at 18:00 UTC (02:00 SGT), alone on the box
(CONSTITUTION § Proportional oversight, clause 3). `scripts/nightly.mjs` clones the `private`
remote fresh, checks out `dev` with `main` pinned, and copies in `node_modules` and the ignored run
bundles from `--inputs-from`. It copies the reference out, the last green nightly or, before the
first, the emission committed in the tree, and then runs spec 029's `sweep.mjs --emit --against` it.
Everything it writes goes under `~/.driftproof-nightly/`, outside every tree:

- `runs/<stamp>/`: `sweep-run.json`, `sweep-raw/`, `report.md`, `result.json`, `emit.log`;
- `morning-report.md`: the last full run's one-screen report (GREEN or RED, every new red by spec
  and assertion id, the merges since the last green, whether it ran alone);
- `last.json`, and `last-green.json` when green: the reference `--core` and the next nightly read;
- `last-partial.json`: an `--only` run, which is never a reference.

A RED names the merges since the last green. The next change is the fix or a revert. A release is
cut only from a `dev` commit whose nightly is green.
...
it and `spectrace` and `claude` live there: without it spec 013's AC-7 and spec 028's plugin rows read
red on the environment, not the tree (spec 058).

```sh
mkdir -p ~/.config/systemd/user ~/.driftproof-nightly
cat > ~/.config/systemd/user/driftproof-nightly.service <<'UNIT'
[Unit]
Description=Driftproof nightly full sweep of dev (spec 051)
...
TimeoutStartSec=10h
ExecStart=/bin/bash -c 'U=$(git -C %h/driftproof remote get-url private); R=%h/.driftproof-nightly/runner; [ -d $R/.git ] || git clone -q "$U" $R; git -C $R fetch -q origin && git -C $R checkout -q --detach origin/dev && exec /usr/bin/node $R/scripts/nightly.mjs --repo "$U" --ref dev --inputs-from %h/driftproof'
UNIT
cat > ~/.config/systemd/user/driftproof-nightly.timer <<'UNIT'
[Unit]
Description=Driftproof nightly full sweep of dev at 18:00 UTC
...
systemctl --user enable --now driftproof-nightly.timer
systemctl --user list-timers driftproof-nightly.timer          # the next run
journalctl --user -u driftproof-nightly.service -n 50         # the last run's log
```

By hand, the same run: `node scripts/nightly.mjs --ref dev`. A proof or a re-run of a few gates
adds `--only <spec,spec>`, and the report says it is partial. `scripts/nightly.cron` holds the same
schedule as a cron line, for a box that has cron.
...
**A token for a driven publish (spec 048, the operator's ruling of 2026-09-23).** The rule
above stands: the box holds no npm credential. `scripts/drive.mjs` has a token path for the npm
step, `NPM_TOKEN` read from its environment and handed to npm only as the `${NPM_TOKEN}` reference
in a mode-0600 userconfig in scratch, removed afterwards. A token may reach that path only from a
secrets store at publish time, for that run. It is never stored on disk: not in `~/.npmrc`, not in a
shell profile and not in a file the queue names. On this box nothing supplies one, so a driven
release stops for a human at the npm step, and the publish runs from the operator's machine as
below.
