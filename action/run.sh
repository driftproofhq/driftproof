...
# badge.json lands in the same directory after the first run; a second run in
# the same job must not hand the badge writer its own earlier output (spec 026
# AC-20: the writer now verifies the file's receipt_hash, and badge.json has
# none, so the pick has to be a receipt).
