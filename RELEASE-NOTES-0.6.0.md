...
**Released 2026-08-29.** Tagging and `npm publish` are done by hand, at the
maintainer's terminal, and both have now happened: the public tree is `a5474f8`,
tagged **`v0.6.0`**, and npm `driftproof@0.6.0` carries `gitHead a5474f8` — the
same commit, verified against the registry rather than assumed.

*(This paragraph read "neither has happened for this version" until the release
completed. Amended rather than deleted: a release note that quietly rewrites its
own pre-release state leaves no trace that the sequence had a before.)*
...
- Generation sampling in the receipt spec landed in **v0.5 of the receipt
  schema**, which ships in a later package version — not in 0.6.0.
