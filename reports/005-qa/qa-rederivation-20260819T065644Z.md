...
This record says *"20 separated / 10 within noise"* and discusses cells rendered *n/a (within noise)* and the lift column's *(within noise)* annotation. A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. The ten cells the tally counts as within noise were not separated under the rule at the sample size used, since no case in them separated: no separation detected, which is not evidence of equivalence and not evidence that nothing changed. The renderer now writes `n/a (no separation detected)` for such a cell's cost-per-benefit figure. The six cells this record's finding V-4 names were also rendered *n/a (within noise)* in the draft it reviewed, but each had a case separated under the rule with an aggregate lift below the floor; the published page renders those six *n/a (driver-only)*.

Wherever the record reads a separation as settled, as cells that *earned* a regression, read it as a separation detected under the rule in one or more cases, which is not proof that a skill lowered quality. Wherever it reads the four saving cells as settled, as *the same real phenomenon*, *something good happened*, *improved measured quality and reduced cost*, or *the benefit is free*, read it as a floor-clearing aggregate lift that is not separated under the rule, in cells with a separation detected in one or more cases, which is not proof that a skill raised quality or cut cost: no cell's aggregate lift in the reviewed report is separated, since in every cell the with_skill and baseline aggregate bands overlap. *n/a (skill regressed)* names a rendering for a floor-clearing negative aggregate lift, which no cell in the reviewed report has. Labels and suggested wording are read the same way and are left as written.

Each band in this record is a mean plus or minus one sample standard deviation: a cell's band over its seven per-case means, which is suite dispersion, or a case's band over its five judge samples. Each band is a descriptive spread, not a confidence interval, with no coverage probability. Filed under the wording rules of the repository's spec 031, amendment A-031-20.
