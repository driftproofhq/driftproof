...
**v1.1** · 2026-09-14. This entry corrects how the record's band wording is read; no earlier text is changed, and no figure, classification, table value or receipt reference changes.

This record says *within noise* in its verdict column, its band verdicts and its §3 prose, and classifies two within-noise cases *REAL*: *REAL (no change)* and *REAL (judge noise)*. A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. Wherever the record reads a case that was not separated as settled, as *within noise*, *REAL (no change)*, *REAL (judge noise)*, *on par with old*, a regression that *did not reproduce* or was *a one-off*, or a regression that was *entirely the parse-fallback artifact* with the *Artifact confirmed* because a clean re-run was not separated, read it as no separation detected at the sample size used, which is not evidence of equivalence, not evidence that nothing changed, and not proof of the cause named. Its *genuine judge disagreement* is an observation about one sample, not proof of that case's cause. Wherever it reads a separated case as settled, as *REAL* model-behaviour change, *6 are REAL*, *real-but-contaminated*, *A genuine new-model failure*, *REAL regression reconfirmed* or *real regressions surfaced*, read it as a separation detected under the rule, which is not proof that the model's behaviour moved; *6 are REAL* also counts the two within-noise cases, and on `match-existing-adr-convention` a `0.0` sample, which the record attributes to the parse fallback, is present in both runs, so the record's *(and now artifact-free)* driving cases are not all free of it. Classifications are read the same way and are left as published.

Each band in this record is a case's mean plus or minus the sample standard deviation of its five judge samples. Where §3 calls a list of five samples a *band*, the band is that list's mean and standard deviation. Each band is a descriptive spread, not a confidence interval, with no coverage probability. Filed under the wording rules of the repository's spec 031, amendment A-031-20.
