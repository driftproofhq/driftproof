...
**Record version:** v1.1 (2026-09-14), band wording amended; see [Amendments](#amendments).
...
**v1.1** · 2026-09-14. This entry corrects how the record's band wording is read; no earlier text is changed, and no figure, capability label, table value or receipt reference changes.

This record says that two aggregate lifts are *within noise*, that one pair of baselines is *statistically identical*, and that two lifts are *within noise of zero*. Those aggregates are not separated: their bands overlap. Inside them single cases did separate: writing-clearly-and-concisely's emphatic-word-at-end on fable-5, and skill-creator's domain-variant-organization on opus-5. A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. The record compares with_skill against baseline within a receipt, one tier against the other, and this run against Report 003's. Of the aggregate lifts it quotes, one is separated at the aggregate, Report 003's requesting-code-review on opus-5 at +0.200; no other aggregate lift in either report is.

Wherever the record reads a result that was not separated as settled, as *within noise*, *statistically identical*, *with held*, *NO EFFECT*, *ceremonial* or *near-ceremonial*, *ABSORBED*, *lift ≈ 0*, *continues the state*, *roughly equal*, *rejected in its strong form*, *"model caught up"*, *collapsed*, *reproduced exactly*, *reproduced*, *reproduced ~0*, *direction stable*, *sign-or-noise character*, *verdicts repeat*, *does not eat the skills*, *robust outright*, *never measurably hurts*, or *encoded expertise survives the frontier tier*, read it as no separation detected at the sample size used, which is not evidence of equivalence and not evidence that nothing changed. Inside some of those, cases did separate: documentation-and-adrs' with_skill match-existing-adr-convention between the tiers, behind *with held*, skill-creator's baseline domain-variant-organization between the tiers, behind *roughly equal*, and commit-work's conventional-commit-single-change with_skill against baseline on opus-5, behind *lift ≈ 0*, *ceremonial* and *ABSORBED*; where a sentence rests on an aggregate that was not separated while a case inside it was, both readings apply. Between Report 003's opus-5 run and this one, twelve cases on seven of the skills the record calls reproduced separated under the rule: code-review-and-quality's baseline oversized-change-split; commit-work's baseline conventional-commit-single-change; crafting-effective-readmes' baseline lead-with-one-sentence-problem; documentation-and-adrs' with_skill match-existing-adr-convention; naming-analyzer's with_skill abbreviations-wellknown-js and baseline boolean-prefixes-js; requesting-code-review's with_skill triage-review-findings and baseline request-package-basic, crafted-context-not-session-history, identify-base-head-shas and mandatory-vs-optional-triggers; and skill-creator's baseline domain-variant-organization. Each is a separation detected, which is not proof. Wherever it reads a separation, or a shift that was not tested, as settled, as *DURABLE*, *TIER-DEPENDENT*, *AMPLIFIED*, *earns more*, *hurt*, *keep or grow real lift*, *genuinely earn their keep*, *shrinks*, *kept sign but shrank*, *The #003 finding was real*, *opus-5-specific*, *case-level reproduction*, *stands on a real band*, *holds*, or *survives on the one clean driver*, read it as a separation detected under the rule in one or more cases, or as a shift no rule was applied to, which is not proof. The baseline moves it reads as *dropped*, *nudged up*, *came in lower*, *meaningfully higher* or *moved run-to-run* are not separated: their aggregate bands overlap, and they are read as no separation detected at the sample size used. Inside them baseline cases did separate: between the tiers, code-review-and-quality's severity-labeled-findings, documentation-and-adrs' comment-intent-not-implementation, naming-analyzer's boolean-prefixes-js, requesting-code-review's crafted-context-not-session-history and mandatory-vs-optional-triggers, and skill-creator's domain-variant-organization; between the runs, crafting-effective-readmes' lead-with-one-sentence-problem; each is a separation detected, which is not proof. Its *not drift*, said of those baselines, names a cause no band here tests. Capability labels are read the same way and are left as published.

Each band in this record is a mean plus or minus one sample standard deviation, of two kinds: a skill's aggregate band, over its seven per-case means, which is suite dispersion, and a case's band, over its five judge samples, the zero-width point bands and the *real band* among them; the lifts it quotes carry no band. Each band is a descriptive spread, not a confidence interval, with no coverage probability. Filed under the wording rules of the repository's spec 031, amendment A-031-20.
