...
**v1.1** · 2026-09-14. This entry corrects how the record's band wording is read; no earlier text is changed, and no figure, verdict token, table value or receipt reference changes.

This record says *"2 regressed · 4 improved · 0 mixed · 4 within noise"*, labels four skills *WITHIN NOISE*, and calls aggregates *within noise*. The four skills so labelled had no with_skill case separated under the rule between the two models. The aggregates it calls within noise are aggregate with_skill bands that overlap; inside both, naming-analyzer's and writing-clearly-and-concisely's, one case did separate. A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated.

Wherever the record reads a result that was not separated as settled, as *within noise*, *did not move beyond noise*, *barely did anything on either model*, *the skill is fine on the new model*, *remains net-beneficial in aggregate*, *otherwise fine*, *stays flat and high*, *isn't regressing*, *nearly the same quality*, or *"Model caught up"*, read it as no separation detected at the sample size used, which is not evidence of equivalence and not evidence that nothing changed. Wherever it reads a separation, or a result resting on separated cases, as settled, as *The regression is real*, *degraded*, *net-negative*, *net-positive*, *flipped cleanly*, *the strongest release-drift story*, *still HELPS on opus-5*, *dead weight on the older model earns its keep on the newer one*, *hurt on the old model and helped on the new one*, baselines *JUMPING*, or *The verdict stands* with its *confidence*, read it as a separation detected under the rule in one or more cases, which is not proof. Where a sentence rests on an aggregate that was not separated while a case inside it was, both readings apply: none of the aggregate lifts, aggregate with_skill moves or aggregate baseline moves behind these readings is separated, and naming-analyzer on opus-5, crafting-effective-readmes and documentation-and-adrs on both models, and commit-work, git-workflow-and-versioning and skill-creator on opus-4-8, whose lifts the caught-up note says collapsed, have separated lift cases. On the three skills of the caught-up note, each also has one baseline case that separated between the models (commit-work's two-sentence-describability-test, git-workflow-and-versioning's commit-message-conventional-type, skill-creator's domain-variant-organization). The proposed essay tallies are read the same way. The words are left as written.

Each band in this record is a mean plus or minus one sample standard deviation, of two kinds: a skill's with_skill band, over its seven per-case means, which is suite dispersion, and a driver case's band, over its five judge samples. The record's *mean ± stddev (over 5 judge samples, averaged across the 7 cases)*, said of the skill bands, reads as suite dispersion, and *very noisy* and *erratic*, said of a skill band, mean a wide suite dispersion, not an unstable measurement; *stabilised*, said of the narrower opus-5 skill bands, means a narrower suite dispersion, and *a release effect* names a cause no band here tests. *got noisier* and *destabilised*, said of naming-analyzer's abbreviations-wellknown-js, describe its case band widening from ± 0.013 to ± 0.096, to which no rule was applied. Each band is a descriptive spread, not a confidence interval, with no coverage probability. Filed under the wording rules of the repository's spec 031, amendment A-031-20.
