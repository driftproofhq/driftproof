...
| Revision drift | `Revision drift report` | Does a skill's measured lift hold when the *skill's own text* is revised upstream? | the SKILL.md revision (pinned vs current upstream HEAD) — model, surface, suite, judge and sampling all held fixed |
| Instrument re-measurement | `Instrument re-measurement report` | Does an earlier report's result survive a *corrected or strengthened instrument*, measuring the same cells again? | nothing moves under the skill and nothing moves in the substrate — the **instrument** moves: sampling design, a defect fix, or a rule the runner now applies. The cells, suites, substrates and judge are the ones the earlier report used, so the comparison is against the archive rather than against a fresh control |
| Instrument comparison | `Instrument comparison report` | What can each of two instruments establish about the *same skills* on pinned inputs? | nothing moves under the skill and nothing in its inputs: the same SKILL.md bytes, prompts and rubrics are measured by two **instruments**, and the report states what each can and cannot establish rather than ranking them |
...
An **instrument comparison report** is the one type that says so. Driftproof's
own arms are read under the rule below; the other instrument's results are read
by that instrument's own scoring and are never put through Driftproof's rule or
compared with its scores on one scale. What the report adds is what each
instrument can and cannot establish on the same pinned inputs.

A **revision drift report** is the one type that moves the skill itself. Every
other type holds the text fixed and moves the substrate; this one inverts that,
which is why `driftproof diff --mode revision` refuses a pair whose model,
provider, surface or suite differs: in this type those are not caveats, they are
preconditions. Its per-cell classification (revision improved, revision
regressed, mixed, or no separation detected) is the same four-way summary of the
same per-case band verdicts every other type produces.
...
3. **Ratio framings are floor-gated, and priced per unit of benefit.** The
   ...
   **`n/a (no separation detected)`**, never a number: dividing a noise-level lift by a real
   ...
   **Four states, one price.** A floor-clearing positive lift is priced. A cell
   ...
   `n/a (no separation detected)`. A skill that improved quality *and* reduced cost states
...
Every number is a **band**: a descriptive spread, the mean plus or minus one sample
standard deviation (over `n = 5` judge samples on a legacy receipt, or across
generation draws), with no coverage probability. A directional claim (improved /
regressed / drift) is made **only** when two bands do **not** overlap **and** the
mean moves at least the **0.05 effect floor** (`config.EFFECT_FLOOR`), and it is a
separation detected under this rule, not proof that the skill moved. Overlapping
bands, or a sub-floor move, are "no separation detected at this sample size": never
a verdict, and never evidence that nothing changed. This restraint is the point; it is identical
