...
or symmetric), never on overlapping bands. The band is a descriptive spread of one
sample standard deviation. The honest answer is allowed to be "no separation
...
quantization step). Separated but trivial moves are labelled *below effect floor* and
are not called a verdict. Band separation **plus** a floor-sized delta, never either
alone, is what triggers a verdict.
...
- **`comparison.delta`** — the skill's measured lift (`with_skill − baseline`). Read it
  through the effect floor: `≥ 0.05` clears the floor, and `|delta| < 0.05` does not (the badge's word for it is no effect).
...
## Input bounds (what the loader refuses)

The skill loader and the post-checks are bounded (spec 026 AC-13), and every
bound is a named constant in `config.js` that the refusal names:

| bound | value | what it binds |
|---|---|---|
| `SKILL_MAX_FILES` | 2000 | bundled files under the skill directory (`evals/` excluded) |
| `SKILL_MAX_BYTES` | 32 MiB | the bundled files' bytes together |
| `SKILL_MAX_DEPTH` | 16 | directory depth below the skill directory |
| `SUITE_MAX_CASES` | 500 | cases in one `evals.json` |
| `CASE_MAX_CHARS` | 65536 | characters in one prompt or one rubric |
| `CHECK_MAX_PATTERN` | 256 | characters in one `checks[]` regex |

A skill or suite past a bound is refused before any model call: `loadSkill`
throws naming the bound and the value, and `driftproof run` exits 2 with no
receipt. A `checks[]` regex longer than `CHECK_MAX_PATTERN`, or one carrying a
nested quantifier of the `(x+)+` family, is recorded `pass: false` with a
`refused` reason rather than evaluated: a suite's regex runs against model
output, and JavaScript cannot time a regex out. The nested-quantifier test is
syntactic and narrow (a quantified group whose body ends in a quantifier); it
is not a general detector. Every suite under `suites/` and the bundled example
load well inside all six.
