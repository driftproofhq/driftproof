...
- **Undocumented expectations** — the rubric demands behaviour the skill never claims.
  If the rubric asks for it, the `SKILL.md` must say it.
...
Put the suite itself at `suites/<slug>/evals.json`. The gate enforces the manifest
shape, permissive licensing, and that every tested skill has a committed suite — run
`npm run gate` before opening the PR. See [CONTRIBUTING.md](CONTRIBUTING.md) for ground
