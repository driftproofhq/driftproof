...
const BAND_SITE_KIND = {
  ...
  // Spec 049: the two Anthropic formats, the same kind of site as lib/importers.js.
  'lib/importers-anthropic.js': 'emission: builds a receipt from an imported document; compares nothing',
  'lib/receipt.js': 'emission — assembles a receipt aggregate',
  'lib/run.js': 'emission — writes the draw set a receipt records',
  // Spec 044: the regrade carries a receipt's draw set and writes new judge fields into it.
  'lib/regrade.js': 'emission — writes the regraded draw set a receipt records',
...
const PROBE_COPY_EXCEPTIONS = [
  ...
  // Found at the 0.11.2 release sweep (c50ad3aa): spec 042's merge exported `verdicts` from
  // scripts/prepare-report-011.js after spec 046's emission had read its probes, and neither
  // emission read the other's tree, so the repository gate on dev read this rule red from 32554fff.
  { probe: 'specs/046-ajv-floor/probes/ajv.mjs', name: 'verdicts', independent_of: 'scripts/prepare-report-011.js verdicts', reason: 'a name collision: runs lib/receipt.js validateReceipt over the committed receipts under a chosen ajv in a child process and returns valid or invalid per file; Report 011\'s drift verdicts are not involved', seen_failing_at: { gate: 'specs/046-ajv-floor/gate.sh', assertion: 'every committed receipt validates under the new floor and under the locked version, as it does under 8.17.1' } },
];
...
function publishExclusionHits(files) {
  return {
    ...
    governance: files.filter((f) => /^specs\//.test(f) || f === 'BACKLOG.md' || f === 'CLAUDE.md' || f === 'CONSTITUTION.md' || f === 'DECISIONS.md'),
