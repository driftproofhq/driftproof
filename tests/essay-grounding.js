...
const VARIANTS = {
  ...
  mixed: ['mixed'],
  'within noise': ['within noise', 'within-noise', 'with no separation detected', 'no separation detected'],
  durable: ['durable'],
  ...
  // The instrument-comparison shape's labels (spec 039): Driftproof's own arms
  // read under the rule, one row per skill.
  separated: ['separated'],
  'no separation detected': ['with no separation detected', 'no separation detected'],
  // The release-drift-table shape's labels (spec 047): the runner's display labels, read off
  // an "under the rule" column; "not enough draws" is the underpowered one.
...
// ── the fifth shape: an instrument comparison (spec 039, Report 009) ─────────
//
// Report 009 reads two instruments side by side and gives no verdict across them,
// so none of the shapes above yields a tally and derive() threw. What it does
// publish under the rule is Driftproof's own arms, one row per skill, in a column
// the table names "Driftproof arms under the rule". Read off that column, as the
// refusal shape reads its table, never out of the prose. null for every page
// without such a column, so every earlier report is unaffected.
function instrumentComparisonPairs(html) {
  for (const t of html.matchAll(/<table[^>]*>([\s\S]*?)<\/table>/g)) {
    const table = t[1];
    const head = (table.match(/<thead>[\s\S]*?<\/thead>/) || [''])[0];
    const heads = [...head.matchAll(/<th[^>]*>([\s\S]*?)<\/th>/g)].map((m) => stripTags(m[1]).trim().toLowerCase());
    const col = heads.findIndex((h) => /arms under the rule/.test(h));
    if (col < 0) continue;
    const body = (table.match(/<tbody>([\s\S]*?)<\/tbody>/) || [null, ''])[1];
    const rows = body.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || [];
    const counts = { separated: 0, 'no separation detected': 0 };
    for (const row of rows) {
      const cells = [...row.matchAll(/<td[^>]*>([\s\S]*?)<\/td>/g)].map((m) => stripTags(m[1]).trim().toLowerCase());
      if (cells.length <= col) continue;
      if (cells[col] === 'separated') counts.separated += 1;
      else if (cells[col] === 'no separation detected') counts['no separation detected'] += 1;
      else return null;
    }
    if (!rows.length) continue;
    return Object.entries(counts).map(([label, n]) => pair(n, label));
  }
  return null;
}

// ── the sixth shape: repeated runs of another harness (spec 041, Report 010) ──
//
// Report 010 takes no Driftproof measurement: it publishes one upstream eval run
// several times under each of two configurations, one row per run (or one row for
// a run of identical runs, "1 to 5"), with the harness's pass_rate and exit code.
// Its tally is the runs that exited 0 and the runs that did not, read off those
// tables, never out of the prose, as the two shapes above read theirs. A table
// counts only when its header carries both a "pass_rate" and an "Exit" column, so
// every earlier report is unaffected.
function runTablePairs(html) {
  let passed = 0;
  let failed = 0;
  let seen = false;
  for (const t of html.matchAll(/<table[^>]*>([\s\S]*?)<\/table>/g)) {
    ...
    const head = (table.match(/<thead>[\s\S]*?<\/thead>/) || [''])[0];
    const heads = [...head.matchAll(/<th[^>]*>([\s\S]*?)<\/th>/g)].map((m) => stripTags(m[1]).trim().toLowerCase());
    const runCol = heads.indexOf('run');
    const exitCol = heads.indexOf('exit');
    if (runCol < 0 || exitCol < 0 || !heads.includes('pass_rate')) continue;
    const body = (table.match(/<tbody>([\s\S]*?)<\/tbody>/) || [null, ''])[1];
    for (const row of body.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || []) {
      const cells = [...row.matchAll(/<td[^>]*>([\s\S]*?)<\/td>/g)].map((m) => stripTags(m[1]).trim());
      if (cells.length <= Math.max(runCol, exitCol)) return null;
      const span = cells[runCol].match(/^(\d+)(?: to (\d+))?$/);
      if (!span || !/^\d+$/.test(cells[exitCol])) return null;
      const n = span[2] ? Number(span[2]) - Number(span[1]) + 1 : 1;
      if (cells[exitCol] === '0') passed += n; else failed += n;
      seen = true;
    }
  }
  return seen ? [pair(passed, 'passed'), pair(failed, 'failed')] : null;
}

// ── the seventh shape: a release drift table read under the rule (spec 047, Report 011) ──
//
// Report 011 publishes its verdicts in two tables, one row per skill, each with a column
// the table names "under the rule", holding the runner's display label for that skill's
// verdict. None of the shapes above reads it, so derive() threw. The report's tally is the
// release drift comparison, the type's own question (the model moves, the harness stays
// fixed), which the page sets under <h2 id="model">; the second table holds the model and
// moves the harness, which the page says the type does not describe. So the shape reads the
// table in the "model" section and no other: with that table's column gone it yields
// nothing rather than falling back to the harness table. Read off the column, never out of
// the prose, as the shapes above read theirs. A page with no "model" section is unaffected,
// which is every earlier report; and it is tried last.
function underTheRulePairs(html) {
  const sec = html.match(/<h2 id="model">[\s\S]*?(?=<h2 |<\/main>)/);
  if (!sec) return null;
  const t = sec[0].match(/<table[^>]*>([\s\S]*?)<\/table>/);
  if (!t) return null;
  const table = t[1];
  const head = (table.match(/<thead>[\s\S]*?<\/thead>/) || [''])[0];
  const heads = [...head.matchAll(/<th[^>]*>([\s\S]*?)<\/th>/g)].map((m) => stripTags(m[1]).trim().toLowerCase());
  const col = heads.indexOf('under the rule');
  ...
  for (const row of rows) {
    ...
    const c = cells[col];
    const label = /^no separation detected; not enough draws/.test(c) ? 'not enough draws'
      : c === 'no separation detected' ? 'no separation detected'
        : /^separation detected, upward/.test(c) ? 'separated upward'
          : /^separation detected, downward/.test(c) ? 'separated downward' : null;
...
function derive(root) {
  ...
  for (const id of publishedReportIds(root)) {
    ...
    const pairs = valuePairs(text) || tallyLinePairs(text) || verdictCellPairs(html) || refusalTablePairs(html) || instrumentComparisonPairs(html) || runTablePairs(html) || underTheRulePairs(html);
...
module.exports = { CHECKS, DESCRIPTIONS, run, derive, refusalTablePairs, runTablePairs, stripTags, numbersIn, tallyViolations, rotateCounts, citingParagraphs, publishedReportIds, publishedTypeLabels, declaredTypeLabels, paragraphs, WORDS, ESSAY_REL };
