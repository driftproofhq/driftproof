...
//   - drift: no separation on overlapping bands, "regression" ONLY on non-overlap
...
// A Node built-in, so the classifier below adds no dependency: `zTXt` and
// compressed `iTXt` payloads are deflate, and a woff2's table stream is brotli.
...
// SPEC 030 AC-5. Whether THIS tree is a published one, decided from its own
// contents rather than from an argv flag.
//
// The publish-state-aware checks below keyed off `SCAN_ROOT_ARG`, which only
// `scripts/build-public.sh` passes when it verifies a tree it just built. That
// is true of the build's own run and false of every other reader - including a
// CONTRIBUTOR who clones the published repository and runs `npm run gate` as
// CONTRIBUTING.md documents. The cost-log check then demanded a file the
// published tree correctly does not carry, and no clone could be green.
//
// A published tree identifies itself by exactly what build-public.sh's
// EXCLUDE_RE removes, plus the markers that say it is still this project. The
// test is positive and content-based, so it cannot be satisfied by a dev tree
// with a file deleted: `specs/` is what a dev tree always has.
const ROOT_IS_PUBLISHED_TREE =
  ['specs', 'DECISIONS.md', 'CONSTITUTION.md'].every((p) => !fs.existsSync(path.join(ROOT, p)))
  && ['README.md', 'docs', 'package.json', 'lib'].every((p) => fs.existsSync(path.join(ROOT, p)));
...
// v0.6 (spec 026): a fixture is not exempt from the shape it tests. RUN_META
// describes a hypothetical model-answered run on the claude-cli surface, and
// says so: what answered, that the surface attested the model, which spawn path,
// which judge and which grading template. The template hash is a fixture value,
// as the rubric hash above is.
const RUN_META = {
  model_id: 'claude-haiku-4-5-20251001', model_release_date: '2025-10-01', provider: 'anthropic', surface: 'claude-cli', runner_version: '0.3.0', date_utc: '2026-07-27T00:00:00.000Z', registry: 'registered', transcripts: 'hashes-only',
  judge: { samples: 5, temperature: null, sampling: 'surface-controlled', surface: 'claude-cli', model_id: 'claude-haiku-4-5-20251001', prompt_template_hash: 'e'.repeat(64) },
  answered_by: { kind: 'model', attested: true, reported_model: 'claude-haiku-4-5-20251001', reported_models: ['claude-haiku-4-5'], isolation: 'same-user' },
};
...
gate.section('schema v0.4 + round-trip');
{
  ...
  // Named for the thing, not a number (spec 026: a name that fixed "0.4" was
  // name-vs-thing; the comparison was always to RECEIPT_SCHEMA_VERSION).
  gate.check('receipt stamps the current schema_version + run.provider + run.registry + run.transcripts',
...
// ── 5. Drift band logic: no separation vs regression ─────────────────────────
gate.section('drift bands');
{
  ...
  gate.check('overlapping bands → the not-separated verdict value (no false regression)', byId.noise === require('../lib/stats').WITHIN_NOISE, byId);
...
// ── CLASSIFY-BEGIN: the tree is classified before it is scanned (spec 027) ───
//
// THE DEFECT. Section 7 read every file with `readFileSync(..., 'utf8')` and
// matched the deny list against the decoded string. One committed screenshot
// carries three literal ASCII bytes, at offset 23576 inside an IDAT chunk,
// that spell a deny-listed acronym with non-word bytes on both sides, so the
// word-boundary test matched them. They are in the file (`grep -a` finds them)
// and absent from the inflated pixel data: a coincidence of the compressed
// stream's bit-packing, not a name anybody wrote. This comment first said the
// lossy decode invented a string that was nowhere in the file. It did not, and
// for a pure-ASCII deny list it cannot: an ASCII byte decodes to itself and
// every other byte becomes a non-word character either way. The old reader
// was a byte-exact reader in effect, and the classification below is a
// NARROWING of it, argued from the gate's threat model (CONSTITUTION) and
// measured per file by the IDAT residue check. DECISIONS 2026-09-05.
//
// WHY NOT SKIP BINARIES. It would remove the noise and open a hole: a file
// carrying a real term plus one invalid byte would stop being scanned, silently,
// by a control whose whole point is that it never stops. Three variants of that
// fix were proposed and all three were ruled out.
//
// SO EVERY FILE IS CLASSIFIED, AND NO CLASS IS A SKIP.
//
//   text          bytes survive a strict UTF-8 round-trip -> the existing scan,
//                 over the decoded string, unchanged
//   a recognised  bytes match a signature in RECOGNISED_BINARIES -> that
//   binary        format's own check, over the parts of the format that can
//                 carry text, and nothing else
//   unrecognised  neither -> THE GATE FAILS, naming the file and the reason
//
// Unrecognised is red. A new binary format arriving here is a decision somebody
// makes in a spec, not a file that quietly stops being checked. A recognised
// binary whose check throws is red for the same reason: a check that returns no
// text because it could not read the file is indistinguishable, at the call
// site, from one that read the file and found nothing. That is
// `absence-vs-unreadable`, the register's own class.
//
// CLASSIFICATION KEYS ON THE SIGNATURE, NEVER ON THE EXTENSION. An
// extension-keyed classifier is the ruled-out fix wearing a different hat:
// rename a file and the control changes its mind.

// The recognised-type list, declared ONCE, as data. `specs/027-.../spec.md`
// AC-7 carries the same table and the gate holds the two equal in both
// directions, so the prose beside this cannot go stale the way a list beside an
// authoritative file always does.
const RECOGNISED_BINARIES = [
  {
    id: 'png',
    signature: '89504e470d0a1a0a',
    check: 'pngScannableText',
    scans: 'the contents of every tEXt, iTXt and zTXt chunk, zTXt and compressed iTXt inflated',
    extract: pngScannableText,
    residue: pngIdatResidue,
  },
  {
    id: 'woff2',
    signature: '774f4632',
    check: 'woff2ScannableText',
    scans: 'every name table record string decoded per platform id, the extended metadata block decompressed, and the private data block',
    extract: woff2ScannableText,
  },
];

// The PNG signature, named once and derived from the list above. Section 40's
// two Open Graph card assertions used to carry the same hex literal of their
// own: a second definition of what a PNG is, in the same file, free to disagree
// with this one. They read this constant now. The check is unchanged.
const PNG_SIGNATURE_HEX = RECOGNISED_BINARIES.find((b) => b.id === 'png').signature;

// Strict, and written as the round trip it is named for: decode, re-encode,
// compare bytes. Equivalent to a fatal UTF-8 decode on every input, and it says
// what it does.
function utf8RoundTrips(buf) {
  return Buffer.compare(Buffer.from(buf.toString('utf8'), 'utf8'), buf) === 0;
}

// ── PNG: the three chunk types that carry text, and nothing else ────────────
//
// IDAT is not term-scanned. A confidential name rendered INTO a screenshot is
// pixels, which no byte scan can see either way. IDAT is inflated instead, by
// pngIdatResidue below, so that a byte-exact run inside it is adjudicated
// against the pixel data rather than assumed to be noise.
function pngScannableText(buf) {
  const out = [];
  let off = 8;
  let sawIEND = false;
  while (off + 8 <= buf.length) {
    ...
    if (len > 0x7fffffff || dataEnd + 4 > buf.length) throw new Error(`chunk ${type} at offset ${off} runs past the end of the file`);
    const data = buf.subarray(dataStart, dataEnd);
    if (type === 'tEXt') {
      const z = data.indexOf(0);
      if (z < 0) throw new Error('a tEXt chunk has no keyword terminator');
      out.push({ region: 'tEXt', text: `${data.subarray(0, z).toString('latin1')} ${data.subarray(z + 1).toString('latin1')}` });
    } else if (type === 'zTXt') {
      const z = data.indexOf(0);
      if (z < 0) throw new Error('a zTXt chunk has no keyword terminator');
      if (data[z + 1] !== 0) throw new Error(`a zTXt chunk declares compression method ${data[z + 1]}, which is not deflate`);
      out.push({ region: 'zTXt', text: `${data.subarray(0, z).toString('latin1')} ${zlib.inflateSync(data.subarray(z + 2)).toString('latin1')}` });
    } else if (type === 'iTXt') {
      const z = data.indexOf(0);
      if (z < 0) throw new Error('an iTXt chunk has no keyword terminator');
      const flag = data[z + 1];
      const method = data[z + 2];
      let p = z + 3;
      const l = data.indexOf(0, p);
      if (l < 0) throw new Error('an iTXt chunk has no language-tag terminator');
      const lang = data.subarray(p, l).toString('latin1');
      p = l + 1;
      const t = data.indexOf(0, p);
      if (t < 0) throw new Error('an iTXt chunk has no translated-keyword terminator');
      const translated = data.subarray(p, t).toString('utf8');
      p = t + 1;
      const rest = data.subarray(p);
      let text;
      if (flag === 0) text = rest.toString('utf8');
      else if (flag === 1) {
        if (method !== 0) throw new Error(`an iTXt chunk declares compression method ${method}, which is not deflate`);
        text = zlib.inflateSync(rest).toString('utf8');
      } else throw new Error(`an iTXt chunk declares compression flag ${flag}, which is neither 0 nor 1`);
      out.push({ region: 'iTXt', text: [data.subarray(0, z).toString('latin1'), lang, translated, text].join(' ') });
    }
    off = dataEnd + 4;
    if (type === 'IEND') { sawIEND = true; break; }
  }
  if (!sawIEND) throw new Error('no IEND chunk: this is not a complete PNG');
  return out;
}

// ── PNG IDAT residue: the codec-coincidence adjudication (spec 027 AC-3) ─────
//
// IDAT is not term-scanned, and this is why that is safe under the gate's
// threat model (CONSTITUTION, the confidentiality gate's threat model):
// accidental plaintext disclosure by a person or an agent writing the tree.
// IDAT bytes are a function of the inflated scanlines and the encoder. Nobody
// writes them. A deny-list term reaches them by exactly two routes - a stored
// deflate block, in which case it is verbatim in the inflated output as well,
// or the bit-packing of a compressed block landing on those byte values by
// chance, at a rate the stream's entropy fixes. So the inflated output is the
// whole of what anyone put into IDAT, and it is pixel samples: a name rendered
// into a screenshot is pixels either way.
//
// The residue check turns that argument into a measurement per file: inflate
// the concatenated IDAT; the caller counts deny-list hits in the output; the
// inflate must consume every IDAT byte. A byte-exact hit in the compressed bytes
// that is absent from the output, in a stream that was fully consumed, is a
// codec coincidence. A stream that is not fully consumed - bytes after the
// final deflate block, or bytes that are not a deflate stream at all - is an
// unauthored region nothing read, and the file goes RED on the same footing as
// an unrecognised binary (rule 3). The chance rate is emitted beside the
// measurement so that a hit in compressed bytes is a number, not a surprise.
function pngIdatResidue(buf) {
  const ranges = [];
  let off = 8;
  while (off + 8 <= buf.length) {
    const len = buf.readUInt32BE(off);
    ...
    const dataEnd = dataStart + len;
    if (len > 0x7fffffff || dataEnd + 4 > buf.length) throw new Error(`chunk ${type} at offset ${off} runs past the end of the file`);
    if (type === 'IDAT') ranges.push([dataStart, dataEnd]);
    off = dataEnd + 4;
    if (type === 'IEND') break;
  }
  const all = Buffer.concat(ranges.map(([a, b]) => buf.subarray(a, b)));
  const counts = new Array(256).fill(0);
  let word = 0;
  for (const x of all) {
    counts[x] += 1;
    if ((x >= 0x30 && x <= 0x39) || (x >= 0x41 && x <= 0x5a) || (x >= 0x61 && x <= 0x7a) || x === 0x5f) word += 1;
  }
  let entropy = 0;
  for (const n of counts) if (n) { const q = n / all.length; entropy -= q * Math.log2(q); }
  const out = {
    ranges,
    idat_chunks: ranges.length,
    idat_bytes: all.length,
    idat_consumed: 0,
    idat_fully_consumed: all.length === 0,
    inflated_bytes: 0,
    inflated: null,
    idat_entropy_bits: all.length ? +entropy.toFixed(2) : 0,
    idat_word_fraction: all.length ? +(word / all.length).toFixed(3) : 0,
    error: null,
  };
  if (!all.length) return out;
  try {
    const r = zlib.inflateSync(all, { info: true });
    out.idat_consumed = r.engine.bytesWritten;
    out.inflated_bytes = r.buffer.length;
    out.inflated = r.buffer;
    out.idat_fully_consumed = out.idat_consumed === all.length;
  } catch (e) {
    out.error = e.message;
  }
  return out;
...
// ── woff2: every region the format defines as carrying text ────────────────
//
// spec 025 adds self-hosted fonts under docs/fonts/, which publish. WOFF2
// defines THREE regions that carry text, and this reader covers all three:
//
//   name   the name table's record strings - copyright, family, designer,
//          foundry - decoded per platform id
//   meta   the extended metadata block at metaOffset/metaLength: a BROTLI-
//          compressed XML document whose defined elements include `credit name`,
//          `vendor name` and `description`, which are names of people and
//          organisations by specification
//   priv   the private data block at privOffset/privLength: arbitrary bytes
//
// WHAT IS NOT READ, AND ON WHAT GROUND. The glyph outlines and the other sfnt
// tables are coordinates, metrics, class definitions and lookups: numeric
// structures with no free-text field, and the woff2 equivalent of pixel data. A
// name rendered as an outline is a shape, which no byte scan reads either way.
// That is a statement about what those regions hold. This reader first covered
// the name table alone, on the ground that it was "the only part of a font that
// could carry a name" - a claim the format does not support, which is how a
// blocking control acquires a hole nobody decided to open (approval finding
// F-2, 2026-09-05). No region is excluded here on a claim about the format.
...
function readUIntBase128(buf, p) {
  let v = 0;
  for (let i = 0; i < 5; i++) {
    if (p >= buf.length) throw new Error('a UIntBase128 runs past the end of the file');
    const b = buf[p++];
    if (i === 0 && b === 0x80) throw new Error('a UIntBase128 has a leading zero');
    if (v & 0xfe000000) throw new Error('a UIntBase128 overflows 32 bits');
    v = ((v << 7) | (b & 0x7f)) >>> 0;
    if ((b & 0x80) === 0) return [v, p];
  }
  throw new Error('a UIntBase128 is longer than five bytes');
}

function decodeUtf16BE(raw) {
  const n = raw.length - (raw.length % 2);
  const swapped = Buffer.alloc(n);
  for (let i = 0; i + 1 < n; i += 2) { swapped[i] = raw[i + 1]; swapped[i + 1] = raw[i]; }
  return swapped.toString('utf16le');
}

function woff2ScannableText(buf) {
  if (buf.length < 48) throw new Error('shorter than a woff2 header');
  const numTables = buf.readUInt16BE(12);
  const totalCompressed = buf.readUInt32BE(20);
  let p = 48;
  const dir = [];
  for (let i = 0; i < numTables; i++) {
    if (p >= buf.length) throw new Error('the table directory runs past the end of the file');
    const flags = buf[p++];
    const idx = flags & 0x3f;
    const transform = (flags >> 6) & 0x03;
    let tag;
    if (idx === 0x3f) { tag = buf.toString('latin1', p, p + 4); p += 4; } else {
      tag = WOFF2_KNOWN_TAGS[idx];
      if (!tag) throw new Error(`the table directory names unknown tag index ${idx}`);
    }
    let origLength;
    [origLength, p] = readUIntBase128(buf, p);
    let transformLength = origLength;
    // glyf and loca are transformed unless the transform version is 3; every
    // other table is transformed only when the version is not 0.
    const transformed = (tag === 'glyf' || tag === 'loca') ? transform !== 3 : transform !== 0;
    if (transformed) [transformLength, p] = readUIntBase128(buf, p);
    dir.push({ tag, transformLength });
  }
  const compressed = buf.subarray(p, p + totalCompressed);
  if (compressed.length !== totalCompressed) throw new Error('the compressed stream is shorter than the header declares');
  const font = zlib.brotliDecompressSync(compressed);
  let off = 0;
  let nameBuf = null;
  for (const t of dir) {
    if (t.tag === 'name') { nameBuf = font.subarray(off, off + t.transformLength); break; }
    off += t.transformLength;
  }
  const out = [];
  // Kept outside the block: the census check at the end needs to know whether
  // the font DECLARES name records, so that a table read and found empty is
  // distinguishable from a table never reached.
  let nameRecords = 0;
  if (nameBuf) {
    if (nameBuf.length < 6) throw new Error('the name table is shorter than its own header');
    const count = nameBuf.readUInt16BE(2);
    nameRecords = count;
    const stringOffset = nameBuf.readUInt16BE(4);
    for (let i = 0; i < count; i++) {
      const r = 6 + i * 12;
      if (r + 12 > nameBuf.length) throw new Error('a name record runs past the name table');
      const platformID = nameBuf.readUInt16BE(r);
      const length = nameBuf.readUInt16BE(r + 8);
      const offset = nameBuf.readUInt16BE(r + 10);
      const s = stringOffset + offset;
      if (s + length > nameBuf.length) throw new Error('a name string runs past the name table');
      const raw = nameBuf.subarray(s, s + length);
      // Platform 1 is Macintosh, single byte. Platforms 0 and 3 are UTF-16BE.
      out.push({ region: 'name', text: platformID === 1 ? raw.toString('latin1') : decodeUtf16BE(raw) });
    }
  }
  // ── the two blocks, at the offsets the WOFF2 HEADER puts them ─────────────
  //
  // The WOFF2 header is 48 bytes: signature 0, flavor 4, length 8, numTables 12,
  // reserved 14, totalSfntSize 16, totalCompressedSize 20, majorVersion 24,
  // minorVersion 26, metaOffset 28, metaLength 32, metaOrigLength 36,
  // privOffset 40, privLength 44. `specs/027-.../spec.md` AC-4 carries the same
  // table and `probes/classify.mjs woff2-header-fields` holds the three in
  // agreement.
  //
  // WOFF 1.0 HAS NO totalCompressedSize, so every field after byte 20 sits four
  // bytes earlier there: metaOffset at 24, metaLength at 28, privOffset at 36,
  // privLength at 40. Those are the four this function read until approval
  // finding F-3, 2026-09-05, while reading totalCompressedSize at 20 and the
  // table directory at 48 as WOFF2 — one header read as two formats. On a
  // conformant font it threw on the metadata block and returned the header and
  // the whole compressed table stream labelled `priv`.
  //
  // A zero length means the block is absent; the offset is then meaningless and
  // is not read, which is the shape every font in this repository had before the
  // four fontTools wrote for AC-19 arrived.
  const metaOffset = buf.readUInt32BE(28);
  const metaLength = buf.readUInt32BE(32);
  const metaOrigLength = buf.readUInt32BE(36);
  if (metaLength > 0) {
    if (metaOffset + metaLength > buf.length) throw new Error('the extended metadata block runs past the end of the file');
    // BROTLI, NOT DEFLATE. WOFF 1.0 compresses the metadata block with zlib and
    // WOFF2 compresses it with Brotli, so an inflate here throws `incorrect
    // header check` on every conformant font and the block is never scanned.
    // The second half of F-3, found while fixing the first.
    const text = zlib.brotliDecompressSync(buf.subarray(metaOffset, metaOffset + metaLength)).toString('utf8');
    // The header declares what the block decompresses to. A mismatch means the
    // bytes read are not the block, which is a parse failure rather than a
    // region quietly recorded as covered.
    if (Buffer.byteLength(text, 'utf8') !== metaOrigLength) {
      throw new Error(`the extended metadata block decompresses to ${Buffer.byteLength(text, 'utf8')} bytes against a declared metaOrigLength of ${metaOrigLength}`);
    }
    out.push({ region: 'meta', text });
  }
  // The private data block: arbitrary bytes, so it is decoded as UTF-8 only when
  // it is UTF-8 and as latin1 otherwise, which keeps an ASCII run readable
  // either way rather than turning it into replacement characters.
  const privOffset = buf.readUInt32BE(40);
  const privLength = buf.readUInt32BE(44);
  if (privLength > 0) {
    if (privOffset + privLength > buf.length) throw new Error('the private data block runs past the end of the file');
    const raw = buf.subarray(privOffset, privOffset + privLength);
    out.push({ region: 'priv', text: utf8RoundTrips(raw) ? raw.toString('utf8') : raw.toString('latin1') });
  }
  // ── the census may not record a region this function did not read ──────────
  //
  // `declared` is what the font's own header says it carries; the regions above
  // are what came back. They must be the same set. A reader that located a block
  // at the wrong offset returned real bytes under a region's name and the run
  // recorded that region as covered — `priv` on a font whose private data had
  // never been touched — which is `absence-vs-unreadable` fail-open in a
  // blocking scan. A mismatch is a parse error, which AC-6 makes red and names.
  const declared = new Set();
  if (nameRecords > 0) declared.add('name');
  if (metaLength > 0) declared.add('meta');
  if (privLength > 0) declared.add('priv');
  const covered = new Set(out.map((x) => x.region));
  const missing = [...declared].filter((r) => !covered.has(r));
  const extra = [...covered].filter((r) => !declared.has(r));
  if (missing.length || extra.length) {
    throw new Error(`the region census would be wrong: the header declares [${[...declared].join(', ')}] and the reader returned [${[...covered].join(', ')}]`);
  }
  return out;
}
...
function classifyBytes(buf) {
  if (utf8RoundTrips(buf)) return { cls: 'text' };
  for (const b of RECOGNISED_BINARIES) {
    const sig = Buffer.from(b.signature, 'hex');
    if (buf.length >= sig.length && buf.subarray(0, sig.length).equals(sig)) return { cls: b.id, binary: b };
  }
  return { cls: 'unrecognised' };
}

// What the deny list is tested against, and why it is that and not the bytes.
// `items` is how many pieces of text the format's own check returned, which is
// what makes an empty result distinguishable from an unread one.
function scannableProjection(buf) {
  const c = classifyBytes(buf);
  if (c.cls === 'text') return { cls: 'text', text: buf.toString('utf8'), items: 1, error: null };
  if (c.cls === 'unrecognised') {
    return { cls: 'unrecognised', text: null, items: 0, error: 'the bytes are not valid UTF-8 and the file matches no recognised binary signature' };
  }
  try {
    // Each part carries the region of the format it came out of, so a run can
    // record WHICH regions a reader covered rather than only how much it
    // returned. A check that quietly stopped reading one region would otherwise
    // look exactly like a file that does not use it.
    const parts = c.binary.extract(buf);
    const regions = {};
    for (const x of parts) regions[x.region] = (regions[x.region] || 0) + 1;
    // The residue, for a format that declares one. It runs after the extractor
    // succeeded, on the same bytes, and a failure inside it is the residue's
    // own (recorded, and red for consumption) rather than a parse failure.
    let residue = null;
    if (c.binary.residue) {
      try { residue = c.binary.residue(buf); } catch (e) { residue = { ranges: [], idat_chunks: 0, idat_bytes: 0, idat_consumed: 0, idat_fully_consumed: false, inflated_bytes: 0, inflated: null, idat_entropy_bits: 0, idat_word_fraction: 0, error: e.message }; }
    }
    return { cls: c.cls, text: parts.map((x) => x.text).join('\n'), items: parts.length, regions, error: null, residue };
  } catch (e) {
    return { cls: c.cls, text: null, items: 0, regions: null, error: `the ${c.cls} check could not read the file: ${e.message}`, residue: null };
  }
}

// The three matchers, unchanged in what they test, taking their term lists as an
// argument so that a mutation can register a term of its own without the real
// lists being involved at all.
//
// A HIT CARRIES ITS LIST AND ITS INDEX as well as its term, because a record of
// what matched has to be emitted without emitting the thing that matched. The
// term stays for the failure detail the operator reads; everything written to
// `tests/gate-results.json` is the coded form.
//
// EACH BRANCH IS ONE LINE, and each is load-bearing: `probes/classify.mjs
// mutation-matcher-branch` removes them one at a time in a scratch worktree and
// requires the repo gate red every time. Before AC-18 two of the three could be
// deleted with the gate at 603/603, which is a false negative in a blocking
// confidentiality control and was found at approval on 2026-09-05, not
// introduced by the change that surfaced it.
function denyHits(text, terms) {
  const hits = [];
  (terms.SUBSTR || []).forEach((t, i) => { if (text.toLowerCase().includes(t.toLowerCase())) hits.push({ term: t, list: 'SUBSTR', index: i }); });
  (terms.WORD || []).forEach((t, i) => { if (new RegExp(`\\b${t}\\b`).test(text)) hits.push({ term: t, list: 'WORD', index: i }); });
  (terms.WORD_CI || []).forEach((t, i) => { if (new RegExp(`\\b${t}\\b`, 'i').test(text)) hits.push({ term: t, list: 'WORD_CI', index: i }); });
  return hits;
}

// The scan. `names` is what to look at and `readBytes` is how to get the bytes,
// so a mutation is a map from name to Buffer and nothing is ever written into
// the tree the control is protecting.
function classifiedScan(names, readBytes, terms) {
  const hits = [];
  const unreadable = [];
  const counts = {};
  const binaries = {};
  for (const name of names) {
    let buf;
    try { buf = readBytes(name); } catch (e) {
      unreadable.push({ file: name, reason: `the file could not be read: ${e.message}` });
      counts.unreadable = (counts.unreadable || 0) + 1;
      continue;
    }
    const p = scannableProjection(buf);
    counts[p.cls] = (counts[p.cls] || 0) + 1;
    let residueRed = null;
    if (p.cls !== 'text') {
      // How many terms a byte-exact read of the whole file matches: one byte,
      // one character, no decode. The pre-change reader's lossy decode agreed
      // with this count on every ASCII pattern (a lossy decode cannot invent an
      // ASCII run, and cannot destroy one) and this is the count it was, by
      // accident, all along. A COUNT, never a term: printing the string would put
      // it in the tree the scan protects.
      const latin = buf.toString('latin1');
      const rec = { class: p.cls, items: p.items, chars: p.text ? p.text.length : 0, regions: p.regions, error: p.error, raw_hits: denyHits(latin, terms).length };
      const rz = p.residue;
      if (rz) {
        // The residue record. `raw_hits_in_idat` is the byte-exact count over the
        // concatenated IDAT bytes - the deflate stream as the decoder sees it -
        // and `raw_hits_outside_idat` over the file with those ranges blanked, so
        // a run cannot be counted on both sides. `inflated_hits` is the count
        // over the inflated output: a hit there is in the image, not in the
        // codec. `idat_chance_expected` is how many byte-exact runs the stream's
        // length and its word-character density predict by chance, summed over
        // every list entry at its own length with a boundary on both sides.
        const idatLatin = Buffer.concat(rz.ranges.map(([a, b]) => buf.subarray(a, b))).toString('latin1');
        const blanked = Buffer.from(buf);
        for (const [a, b] of rz.ranges) blanked.fill(0, a, b);
        const allTerms = [...(terms.SUBSTR || []), ...(terms.WORD || []), ...(terms.WORD_CI || [])];
        const boundary = (1 - rz.idat_word_fraction) ** 2;
        const chance = allTerms.reduce((acc, t) => acc + rz.idat_bytes * (256 ** -String(t).length) * boundary, 0);
        Object.assign(rec, {
          raw_hits_in_idat: denyHits(idatLatin, terms).length,
          raw_hits_outside_idat: denyHits(blanked.toString('latin1'), terms).length,
          inflated_hits: rz.inflated ? denyHits(rz.inflated.toString('latin1'), terms).length : 0,
          idat_chunks: rz.idat_chunks,
          idat_bytes: rz.idat_bytes,
          idat_consumed: rz.idat_consumed,
          idat_fully_consumed: rz.idat_fully_consumed,
          inflated_bytes: rz.inflated_bytes,
          idat_entropy_bits: rz.idat_entropy_bits,
          idat_word_fraction: rz.idat_word_fraction,
          idat_chance_expected: +chance.toFixed(4),
          residue_error: rz.error,
        });
        if (!rz.idat_fully_consumed) {
          residueRed = `the png IDAT stream was not fully consumed: ${rz.error ? rz.error : `${rz.idat_consumed} of ${rz.idat_bytes} bytes`}, so ${rz.idat_bytes - rz.idat_consumed} byte(s) are a region nothing read`;
        }
      }
      binaries[name] = rec;
    }
    if (p.error) { unreadable.push({ file: name, reason: p.error }); continue; }
    // AC-3 rule 2. Red on rule 3's footing: the file is not skipped, it is not
    // reported as a hit for bytes nothing decoded, and the gate says why.
    if (residueRed) {
      unreadable.push({ file: name, reason: residueRed });
    }
    for (const h of denyHits(p.text, terms)) hits.push({ file: name, term: h.term, list: h.list, index: h.index, cls: p.cls });
  }
  return { hits, unreadable, counts, binaries, walked: names.length };
}
// ── CLASSIFY-END ────────────────────────────────────────────────────────────

// ── 7. Confidentiality scan (BLOCKING) ───────────────────────────────────────
gate.section('confidentiality');
const SCAN_CLASSIFICATION = { assertions: [] };
{
  ...
  const files = listFiles(SCAN_ROOT);
  const TERMS = { SUBSTR, WORD, WORD_CI };
  const readBytes = (rel) => fs.readFileSync(path.join(SCAN_ROOT, rel));
  const scan = classifiedScan(files, readBytes, TERMS);
  ...
  // ── the classification, and the assertions that hold it (spec 027) ────────
  //
  // An id is recorded WHEN ITS CHECK IS CALLED, never where its name is
  // written, and the ledger is emitted beside the results. A comment naming an
  // id is powerless to put it back — the shape spec 019a's round 2 found in the
  // check against `name-vs-thing`.
  const cidcheck = (id, name, pass, detail = null) => {
    const r = gate.check(name, pass, detail);
    SCAN_CLASSIFICATION.assertions.push({ id, name, pass: r });
    return r;
  };

  // The mutation term. Registered for a mutation run and for nothing else: no
  // mutation in this section uses, quotes or writes a real deny-list entry, so
  // neither this file nor any artifact of a run carries one. AC-15 asserts that
  // this string matches no real entry under any of the three matchers.
  const MUT = 'ZZQXVQMUTATIONTERM';
  const MUT_TERMS = { SUBSTR: [], WORD: [MUT], WORD_CI: [] };
  const one = (name, buf, terms = MUT_TERMS) => classifiedScan([name], () => buf, terms);

  // Synthetic artifacts, built rather than committed. A fixture on disk drifts
  // from the format it is supposed to depict; a builder cannot.
  const chunk = (type, data) => Buffer.concat([
    (() => { const b = Buffer.alloc(4); b.writeUInt32BE(data.length); return b; })(),
    Buffer.from(type, 'latin1'), data,
    Buffer.alloc(4), // CRC. This reader does not verify it; see the spec's carry list.
  ]);
  const IHDR = chunk('IHDR', Buffer.from([0, 0, 0, 1, 0, 0, 0, 1, 8, 6, 0, 0, 0]));
  const png = (...mid) => Buffer.concat([Buffer.from(PNG_SIGNATURE_HEX, 'hex'), IHDR, ...mid, chunk('IEND', Buffer.alloc(0))]);
  const kw = (k, rest) => Buffer.concat([Buffer.from(k, 'latin1'), Buffer.from([0]), rest]);

  const base128 = (v) => {
    const out = [];
    let n = v;
    do { out.unshift(n & 0x7f); n >>>= 7; } while (n);
    for (let i = 0; i < out.length - 1; i++) out[i] |= 0x80;
    return Buffer.from(out);
  };
  // `meta` is the extended metadata block, Brotli-compressed as WOFF2 requires;
  // `priv` is the private data block, raw. Both are appended after the table
  // stream and pointed at from the header, which is where a real font carries
  // them.
  //
  // THIS BUILDER TAKES NOTHING FROM THE READER, AND THAT IS THE POINT (AC-19,
  // approval finding F-4, 2026-09-05). It used to write both blocks at bytes
  // 24/28/32 and 36/40 — the WOFF 1.0 offsets — because that is where
  // `woff2ScannableText` read them, and it used zlib because that is what the
  // reader inflated. Fixture and implementation agreed with each other and with
  // no version of the format, so M5 and M6 were green while neither block had
  // ever been read from a spec-conformant font. The two are independent
  // transcriptions of the WOFF2 header field table now: no name is shared, the
  // known-tag index below is a literal rather than a lookup in the reader's
  // table, and `probes/classify.mjs woff2-builder-independent` keeps them apart.
  // The adjudication that would have caught it either way is
  // `probes/classify.mjs woff2-external`, over fonts fontTools wrote.
  const woff2 = (nameString, { meta = null, priv = null } = {}) => {
    const s = Buffer.from(nameString, 'utf16le');
    const be = Buffer.alloc(s.length);
    for (let i = 0; i + 1 < s.length; i += 2) { be[i] = s[i + 1]; be[i + 1] = s[i]; }
    const head = Buffer.alloc(6);
    head.writeUInt16BE(0, 0); head.writeUInt16BE(1, 2); head.writeUInt16BE(18, 4);
    const rec = Buffer.alloc(12);
    rec.writeUInt16BE(3, 0); rec.writeUInt16BE(1, 2); rec.writeUInt16BE(0x0409, 4);
    rec.writeUInt16BE(0, 6); rec.writeUInt16BE(be.length, 8); rec.writeUInt16BE(0, 10);
    const nameTable = Buffer.concat([head, rec, be]);
    const compressed = zlib.brotliCompressSync(nameTable);
    // The WOFF2 header, 48 bytes, written from the specification's field table:
    // signature 0, flavor 4, length 8, numTables 12, reserved 14, totalSfntSize
    // 16, totalCompressedSize 20, majorVersion 24, minorVersion 26, metaOffset
    // 28, metaLength 32, metaOrigLength 36, privOffset 40, privLength 44.
    const header = Buffer.alloc(48);
    header.write('wOF2', 0, 'latin1');
    header.writeUInt32BE(0x00010000, 4);
    header.writeUInt16BE(1, 12);
    header.writeUInt32BE(12 + nameTable.length, 16);
    header.writeUInt32BE(compressed.length, 20);
    header.writeUInt16BE(1, 24); // majorVersion — a 16-bit field, which is why a
    header.writeUInt16BE(0, 26); // 32-bit write at byte 24 is a WOFF 1.0 header
    // One directory entry: the `name` table. Known-tag index 5 is the WOFF2
    // specification's own numbering (cmap 0, head 1, hhea 2, hmtx 3, maxp 4,
    // name 5), written as a literal rather than looked up in the reader's table.
    const dir = Buffer.concat([Buffer.from([5]), base128(nameTable.length)]);
    let out = Buffer.concat([header, dir, compressed]);
    // Each block starts on a four-byte boundary, which is what the format asks
    // of privOffset and what a real writer does to both.
    const align = () => { const pad = (4 - (out.length % 4)) % 4; if (pad) out = Buffer.concat([out, Buffer.alloc(pad)]); };
    if (meta !== null) {
      align();
      const raw = Buffer.from(meta, 'utf8');
      // BROTLI. WOFF 1.0 compresses this block with zlib; WOFF2 does not.
      const block = zlib.brotliCompressSync(raw);
      const at = out.length;
      out = Buffer.concat([out, block]);
      out.writeUInt32BE(at, 28);
      out.writeUInt32BE(block.length, 32);
      out.writeUInt32BE(raw.length, 36);
    }
    if (priv !== null) {
      align();
      const block = Buffer.from(priv, 'utf8');
      const at = out.length;
      out = Buffer.concat([out, block]);
      out.writeUInt32BE(at, 40);
      out.writeUInt32BE(block.length, 44);
    }
    out.writeUInt32BE(out.length, 8);
    return out;
  };

  // AC-1 / AC-14 — the classification covers the walk, and every class it used
  // is one the list declares.
  const declaredClasses = ['text', 'unrecognised', 'unreadable', ...RECOGNISED_BINARIES.map((b) => b.id)];
  const summed = Object.values(scan.counts).reduce((a, b) => a + b, 0);
  const undeclared = Object.keys(scan.counts).filter((k) => !declaredClasses.includes(k));
  cidcheck('classify-record',
    'every file walked is classified into exactly one declared class, and the counts sum to the walk',
    summed === files.length && undeclared.length === 0 && files.length >= 10,
    { walked: files.length, summed, counts: scan.counts, undeclared });

  // AC-2, the half that can run in the published tree — every text file reaches
  // the matcher over its UTF-8 decode, and the scan's text-half hits ARE the
  // matcher's own output over those files, so nothing is dropped between the
  // walk and the match.
  //
  // THIS IS NOT THE IDENTITY PROPERTY AND DOES NOT CLAIM TO BE. Both sides call
  // `denyHits`, so it is blind to a change in `denyHits` itself. It used to be
  // called `classify-text-identical` and to say it compared this scan against
  // the pre-change reader, which it never did: it built both sides of its
  // comparison by calling the same function (approval finding F-1, 2026-09-05).
  // The reader as it stood BEFORE this change lives in the base commit's blob,
  // which the source repository has and a published tree does not, so the
  // identity property is `probes/classify.mjs prechange-text-identical` and the
  // branch mutations of AC-18 are what make it bite.
  const routed = [];
  const textFiles = [];
  for (const rel of files) {
    let buf; try { buf = fs.readFileSync(path.join(SCAN_ROOT, rel)); } catch (_e) { continue; }
    if (!utf8RoundTrips(buf)) continue;
    textFiles.push(rel);
    for (const h of denyHits(buf.toString('utf8'), TERMS)) routed.push(`${rel}|${h.list}|${h.index}`);
  }
  // Coded, never a term: this is the list the emitted record carries and the
  // pre-change comparison reads.
  const textHits = scan.hits.filter((h) => h.cls === 'text').map((h) => `${h.file}|${h.list}|${h.index}`);
  cidcheck('classify-text-routing',
    "every text file reaches the matcher over its UTF-8 decode, and the scan's text-half hits are the matcher's own output",
    JSON.stringify(routed.slice().sort()) === JSON.stringify(textHits.slice().sort()) && textFiles.length >= 10,
    { textFiles: textFiles.length, routed: routed.length, textHits: textHits.length });

  // AC-3 — the png check reads the three text chunk types and nothing else, and
  // every png in the scanned tree parses. The synthetic half is what makes this
  // non-vacuous: no png committed here carries a text chunk of any kind.
  const marker = 'CHUNKTEXT';
  const pngText = png(chunk('tEXt', kw('Comment', Buffer.from(marker, 'latin1'))));
  const pngZ = png(chunk('zTXt', kw('Comment', Buffer.concat([Buffer.from([0]), zlib.deflateSync(Buffer.from(marker))]))));
  const pngI = png(chunk('iTXt', kw('Comment', Buffer.concat([Buffer.from([0, 0]), Buffer.from('en\0\0'), Buffer.from(marker)]))));
  const pngExtract = (b) => { try { return pngScannableText(b).map((x) => x.text).join('\n'); } catch (e) { return `ERROR ${e.message}`; } };
  const treePngErrors = Object.entries(scan.binaries).filter(([, v]) => v.class === 'png' && v.error).map(([k, v]) => `${k}: ${v.error}`);
  cidcheck('classify-png-chunks',
    'the png check returns tEXt, zTXt and iTXt text and no pixel data, and every png in the tree parses',
    pngExtract(pngText).includes(marker) && pngExtract(pngZ).includes(marker) && pngExtract(pngI).includes(marker)
      && !pngExtract(png(chunk('IDAT', Buffer.from(marker)))).includes(marker)
      && treePngErrors.length === 0,
    { tEXt: pngExtract(pngText), zTXt: pngExtract(pngZ), iTXt: pngExtract(pngI), idat: pngExtract(png(chunk('IDAT', Buffer.from(marker)))), treePngErrors });

  // AC-4 — the woff2 check returns the name table's strings. The synthetic font
  // carries a known string, so this is non-vacuous in a tree with no font in it;
  // every real font in the tree is additionally required to parse and to yield a
  // non-empty table.
  const fontStrings = (() => { try { return woff2ScannableText(woff2('NAMEMARKER Regular')).map((x) => x.text); } catch (e) { return [`ERROR ${e.message}`]; } })();
  const treeFonts = Object.entries(scan.binaries).filter(([, v]) => v.class === 'woff2');
  const badFonts = treeFonts.filter(([, v]) => v.error || v.items < 1).map(([k, v]) => `${k}: ${v.error || 'no name strings'}`);
  cidcheck('classify-woff2-names',
    'the woff2 check returns the name table strings, and every font in the tree yields a non-empty table',
    fontStrings.length === 1 && fontStrings[0] === 'NAMEMARKER Regular' && badFonts.length === 0,
    { fontStrings, fonts: treeFonts.length, badFonts });

  // AC-5 — nothing in the scanned tree is unreadable, for any reason. This is
  // the assertion an unrecognised binary fails, and it names the file.
  cidcheck('classify-unrecognised-red',
    'no file in the scanned tree is an unrecognised binary or otherwise unreadable',
    scan.unreadable.length === 0, { unreadable: scan.unreadable.slice(0, 20) });

  // AC-6 — a recognised binary whose check throws is red, not clean.
  const truncatedSubject = Object.entries(scan.binaries).find(([, v]) => v.class === 'png');
  const truncated = truncatedSubject
    ? fs.readFileSync(path.join(SCAN_ROOT, truncatedSubject[0])).subarray(0, 40)
    : pngText.subarray(0, 40);
  const corruptFont = (() => { const b = Buffer.from(woff2('NAMEMARKER Regular')); b[b.length - 1] ^= 0xff; b[b.length - 2] ^= 0xff; return b; })();
  const truncScan = one('truncated.png', truncated);
  const fontScan = one('corrupt.woff2', corruptFont);
  cidcheck('classify-parse-failure-red',
    'a truncated png and a corrupted font are reported unreadable with their parse error, not read as clean',
    truncScan.unreadable.length === 1 && /png check could not read/.test(truncScan.unreadable[0].reason)
      && fontScan.unreadable.length === 1 && /woff2 check could not read/.test(fontScan.unreadable[0].reason),
    { truncated: truncScan.unreadable, font: fontScan.unreadable, subject: truncatedSubject ? truncatedSubject[0] : 'synthetic' });

  // AC-7 — the list is well formed: unique ids, unique signatures, an extractor
  // on every entry. The spec's table is held equal to it by the spec gate.
  const ids = RECOGNISED_BINARIES.map((b) => b.id);
  const sigs = RECOGNISED_BINARIES.map((b) => b.signature);
  cidcheck('classify-list-declared',
    'the recognised-type list is declared once, with a unique id, a unique signature and a check on every entry',
    RECOGNISED_BINARIES.length >= 1
      && new Set(ids).size === ids.length && new Set(sigs).size === sigs.length
      && RECOGNISED_BINARIES.every((b) => typeof b.extract === 'function' && /^[0-9a-f]+$/.test(b.signature) && b.signature.length % 2 === 0),
    { ids, sigs });

  // AC-8 — M1. A registered term in a text file is red.
  const m1 = one('probe.md', Buffer.from(`prose mentioning ${MUT} inline`, 'utf8'));
  cidcheck('classify-m1-text',
    'M1: a registered mutation term planted in a text file is a hit',
    m1.hits.length === 1 && m1.hits[0].file === 'probe.md' && m1.unreadable.length === 0, m1);

  // AC-18 — each branch of the matcher, proved by a subject only that branch can
  // match. Found at approval on 2026-09-05: before these three, deleting the
  // substring branch or the case-insensitive word branch left the repo gate at
  // 603/603, because M1, M2 and M3 all register their term in the word list.
  // Three mutations in `probes/classify.mjs` remove each branch alone and require
  // this gate red; these are the assertions that go red when they do.
  const branch = (id, list, subject, terms, name) => {
    const r = one(`branch-${list.toLowerCase()}.md`, Buffer.from(subject, 'utf8'), terms);
    cidcheck(id, name,
      r.hits.length === 1 && r.hits[0].list === list && r.unreadable.length === 0, r);
  };
  // Registered in the substring list ALONE and planted with word characters on
  // both sides, so no word-boundary matcher can reach it.
  branch('classify-m1-substr', 'SUBSTR', `embedded xx${MUT}xx carrying no word boundary`,
    { SUBSTR: [MUT], WORD: [], WORD_CI: [] },
    'M1a: a term registered in the substring list alone, planted with no word boundary, is a hit from the substring branch');
  // Registered in the case-sensitive word list alone.
  branch('classify-m1-word', 'WORD', `prose mentioning ${MUT} inline`,
    { SUBSTR: [], WORD: [MUT], WORD_CI: [] },
    'M1b: a term registered in the case-sensitive word list alone is a hit from that branch');
  // Registered in BOTH word lists and planted in the other case, so the single
  // hit proves the case-insensitive branch ran and the case-sensitive one did
  // not match — the branch and its case-sensitivity in one subject.
  branch('classify-m1-word-ci', 'WORD_CI', `prose mentioning ${MUT.toLowerCase()} inline`,
    { SUBSTR: [], WORD: [MUT], WORD_CI: [MUT] },
    'M1c: a term registered in the case-insensitive word list and planted in the other case is a hit from that branch alone');

  // AC-9 — M2. The same term in each of the three text chunk types, and in the
  // compressed form of the one that has two.
  const m2 = {
    tEXt: one('t.png', png(chunk('tEXt', kw('Comment', Buffer.from(MUT, 'latin1'))))),
    zTXt: one('z.png', png(chunk('zTXt', kw('Comment', Buffer.concat([Buffer.from([0]), zlib.deflateSync(Buffer.from(MUT))]))))),
    iTXt: one('i.png', png(chunk('iTXt', kw('Comment', Buffer.concat([Buffer.from([0, 0]), Buffer.from('en\0\0'), Buffer.from(MUT)]))))),
    iTXtz: one('iz.png', png(chunk('iTXt', kw('Comment', Buffer.concat([Buffer.from([1, 0]), Buffer.from('en\0\0'), zlib.deflateSync(Buffer.from(MUT))]))))),
  };
  const m2missed = Object.entries(m2).filter(([, v]) => v.hits.length !== 1 || v.unreadable.length !== 0).map(([k]) => k);
  cidcheck('classify-m2-png-text',
    'M2: the same term planted in a tEXt, a zTXt and an iTXt chunk, compressed and not, is a hit in every one',
    m2missed.length === 0, { missed: m2missed, m2 });

  // AC-10 — M3. One invalid byte and no recognised signature. The term is NOT
  // detected — nothing was read from the file — and the gate is red anyway,
  // which is the whole of what this fix buys over skipping the file.
  const m3buf = Buffer.concat([Buffer.from(`prose mentioning ${MUT} inline`, 'utf8'), Buffer.from([0xff])]);
  const m3 = one('probe.bin', m3buf);
  cidcheck('classify-m3-invalid-byte',
    'M3: a file with one invalid byte and no recognised signature is reported unreadable and named, rather than skipped',
    m3.unreadable.length === 1 && m3.unreadable[0].file === 'probe.bin'
      && /not valid UTF-8/.test(m3.unreadable[0].reason) && m3.hits.length === 0
      && m3.counts.unrecognised === 1, m3);

  // AC-11 — M4, the residue shape. Approval finding F-1 of 2026-09-05 (second
  // round) measured that the real false positive is three ASCII bytes INSIDE an
  // IDAT chunk and ABSENT from the inflated pixel data, so "a term present only
  // in pixel data" was never what this assertion's subjects had. What it holds
  // now: the synthetic subject is raw ASCII in an IDAT chunk that is not a
  // deflate stream, and it is RED for consumption (AC-3 rule 2) with no hit and
  // the raw run recorded; and every png in the scanned tree inflated to its
  // last byte, so any byte-exact run inside IDAT there stands adjudicated by the
  // residue the record carries. The real screenshot's residue is read out of
  // the record by the spec gate, held against an independent inflate.
  const pad = (t) => Buffer.from(` ${t} `, 'latin1');
  const m4buf = png(chunk('IDAT', pad(MUT)));
  const m4 = one('raw-ascii-in-idat.png', m4buf);
  const m4rec = m4.binaries['raw-ascii-in-idat.png'] || {};
  const treePngs = Object.entries(scan.binaries).filter(([, v]) => v.class === 'png');
  const treeUnconsumed = treePngs.filter(([, v]) => v.idat_fully_consumed !== true).map(([k]) => k);
  const treeIdatRuns = treePngs.filter(([, v]) => v.raw_hits_in_idat > 0).map(([k, v]) => ({ file: k, inflated_hits: v.inflated_hits, idat_chance_expected: v.idat_chance_expected }));
  const classifiedHitBinaries = scan.hits.filter((h) => scan.binaries[h.file]).map((h) => h.file);
  cidcheck('classify-m4-idat-residue',
    'M4: raw ASCII in an IDAT chunk that is not a deflate stream is red for consumption and not a hit, and every png in the tree inflates to its last byte',
    m4.hits.length === 0 && m4.unreadable.length === 1 && m4.unreadable[0].file === 'raw-ascii-in-idat.png'
      && /not fully consumed/.test(m4.unreadable[0].reason) && m4.counts.png === 1
      && m4rec.raw_hits === 1 && m4rec.raw_hits_in_idat === 1 && m4rec.idat_fully_consumed === false
      && treeUnconsumed.length === 0 && classifiedHitBinaries.length === 0,
    { synthetic: m4, tree_pngs: treePngs.length, tree_unconsumed: treeUnconsumed, tree_idat_runs: treeIdatRuns, classifiedHitBinaries });

  // A term in the INFLATED pixel data is recorded and not red. Two streams: a
  // stored one (level 0), where the term is verbatim in the compressed bytes as
  // well, and a compressed one, where it is not. Neither is a hit and neither is
  // red, and the record says `inflated_hits: 1` for both.
  //
  // WHY NOT RED, stated beside the assertion. The gate's threat model is
  // accidental plaintext disclosure by a person or an agent writing the tree
  // (CONSTITUTION). Pixel samples are not a text channel: a term that reaches
  // them by chance in a photograph is noise, and one that reaches them by
  // construction in a crafted image is an evasion, and the gate is for
  // neither. THIS IS THE FIRST ASSERTION IN THE GATE THAT WOULD FLIP UNDER AN
  // ADVERSARIAL THREAT MODEL. The count is in the record so that a change of
  // model is a change to this one assertion and not a search.
  const m4stored = one('stored.png', png(chunk('IDAT', zlib.deflateSync(pad(MUT), { level: 0 }))));
  const m4packed = one('packed.png', png(chunk('IDAT', zlib.deflateSync(pad(MUT)))));
  const sRec = m4stored.binaries['stored.png'] || {};
  const pRec = m4packed.binaries['packed.png'] || {};
  cidcheck('classify-m4-inflated-recorded',
    'M4: a term in the inflated pixel data is recorded and not red, for a stored stream where it is also in the compressed bytes and for a compressed stream where it is not',
    m4stored.hits.length === 0 && m4stored.unreadable.length === 0 && sRec.inflated_hits === 1 && sRec.raw_hits_in_idat === 1 && sRec.idat_fully_consumed === true
      && m4packed.hits.length === 0 && m4packed.unreadable.length === 0 && pRec.inflated_hits === 1 && pRec.raw_hits_in_idat === 0 && pRec.idat_fully_consumed === true,
    { stored: sRec, packed: pRec, stored_scan: m4stored, packed_scan: m4packed });

  // AC-19 — M5 and M6. The two regions AC-4 first excluded on a claim the format
  // does not support. `credit name` is the element the WOFF2 metadata schema
  // defines for exactly the kind of string the deny list holds, so it is the
  // shape the mutation plants rather than a bare token in an XML file.
  const metaXml = `<?xml version="1.0" encoding="UTF-8"?><metadata version="1.0">`
    + `<credits><credit name="${MUT}" role="Designer"/></credits>`
    + `<description><text lang="en">a subsetted face</text></description></metadata>`;
  const m5 = one('meta.woff2', woff2('Clean Regular', { meta: metaXml }));
  cidcheck('classify-m5-woff2-metadata',
    'M5: the term planted in a credit name inside the extended metadata block is a hit',
    m5.hits.length === 1 && m5.hits[0].file === 'meta.woff2' && m5.unreadable.length === 0
      && !!(m5.binaries['meta.woff2'] || {}).regions
      && (m5.binaries['meta.woff2'].regions.meta || 0) === 1,
    m5);

  const m6 = one('priv.woff2', woff2('Clean Regular', { priv: `licence note ${MUT} end` }));
  cidcheck('classify-m6-woff2-privdata',
    'M6: the term planted as bytes in the private data block is a hit',
    m6.hits.length === 1 && m6.hits[0].file === 'priv.woff2' && m6.unreadable.length === 0
      && !!(m6.binaries['priv.woff2'] || {}).regions
      && (m6.binaries['priv.woff2'].regions.priv || 0) === 1,
    m6);

  // The control for both: the same builder with neither block present returns
  // the name region alone and no hit, so a red above is the planted block and
  // not the builder.
  const m56control = one('clean.woff2', woff2(`Clean Regular`));
  cidcheck('classify-m56-woff2-control',
    'CONTROL: the same font with neither block carries only the name region and produces no hit',
    m56control.hits.length === 0 && m56control.unreadable.length === 0
      && JSON.stringify(Object.keys((m56control.binaries['clean.woff2'] || {}).regions || {})) === '["name"]',
    m56control);

  // AC-12 — the classification is deterministic within a run. The two-run form
  // is the spec gate's; this is the half that catches a classifier depending on
  // anything but its input.
  const d1 = one('pixels.png', m4buf);
  const d2 = one('pixels.png', m4buf);
  cidcheck('classify-deterministic',
    'classifying the same bytes twice returns an identical result',
    JSON.stringify(d1) === JSON.stringify(d2), { d1, d2 });

  // NFR-5 — and the answer depends on the bytes and on nothing else: not the
  // path, not the extension, not the size.
  const pureBuf = png(chunk('IDAT', Buffer.from('pixels')));
  const pureNames = ['a.png', 'b.txt', 'c', 'deep/dir/d.bin'];
  const pureClasses = pureNames.map((n) => Object.keys(one(n, pureBuf).counts)[0]);
  const textUnderPng = Object.keys(one('looks-like.png', Buffer.from('ordinary prose', 'utf8')).counts)[0];
  cidcheck('classify-pure',
    'the same bytes classify identically under four names and a changed extension, and text named .png is still text',
    new Set(pureClasses).size === 1 && pureClasses[0] === 'png' && textUnderPng === 'text',
    { pureClasses, textUnderPng });

  SCAN_CLASSIFICATION.walked = scan.walked;
  SCAN_CLASSIFICATION.counts = scan.counts;
  SCAN_CLASSIFICATION.unrecognised = scan.unreadable.map((u) => u.file);
  SCAN_CLASSIFICATION.unreadable = scan.unreadable;
  SCAN_CLASSIFICATION.hit_files = [...new Set(hits.map((h) => h.file))];
  // The text half's hits, as `<file>|<list>|<index>`. A COUNT AND A POSITION,
  // never a term: `probes/classify.mjs prechange-text-identical` compares this
  // against the reader built from the base commit's blob, and neither side may
  // put a deny-list entry into a file (AC-15).
  SCAN_CLASSIFICATION.text_hits = textHits.slice().sort();
  SCAN_CLASSIFICATION.binaries = scan.binaries;
  SCAN_CLASSIFICATION.mutation_term = MUT;
  SCAN_CLASSIFICATION.recognised = RECOGNISED_BINARIES.map((b) => ({ id: b.id, signature: b.signature, check: b.check, scans: b.scans }));
  ...
  for (const term of [...SUBSTR.slice(-1), ...WORD_CI]) {
    for (const body of [`prose mentioning ${term} inline`, `PROSE MENTIONING ${term.toUpperCase()} INLINE`]) {
      // THROUGH THE SHIPPED MATCHER, not a copy of it. This test used to
      // re-implement the substring and case-insensitive tests over the body,
      // which is why it stayed green when a branch of the real matcher was
      // deleted (approval finding F-1, 2026-09-05). Routed here it sees a
      // WORD_CI deletion; it still cannot see a SUBSTR one, because these
      // bodies are reachable by both branches, which is what AC-18's three
      // single-branch subjects are for.
      const caught = denyHits(body, TERMS).length > 0;
      planted.push({ caught });
    }
  }
...
gate.section('hygiene');
{
  ...
  // A PNG is handed over as BYTES, so lib/hygiene.js can read its text-bearing chunks and leave the
  // compressed image data alone (spec 038 A-038-5). Everything else is read as text, as before.
  const hits = hygiene.scanFiles(files, (rel) => (rel.endsWith('.png')
    ? fs.readFileSync(path.join(SCAN_ROOT, rel))
    : fs.readFileSync(path.join(SCAN_ROOT, rel), 'utf8')), readLink);
...
gate.section('publish-tree draft guard');
{
  ...
  // A-028-35: whether these four apply is read from the tree, never from argv
  // alone, and a tree they do not apply to is NOT APPLICABLE with its cause,
  // never a pass (tests/assertion-scope.js exclusionCheckMode).
  //
  // A-028-36 (N-1 on 90a720e): THE FOUR ROWS ARE ONE FUNCTION, called once over
  // the tree being gated and by the arms below over trees planted on disk. The
  // A-028-35 arms read exclusionCheckMode and publishExclusionHits and never these
  // rows: with the rows reverted to a skip counted as a pass, both stayed green
  // (evidence/red/37-exclusion-arms-and-source-inference.txt).
  const exScope = require('./assertion-scope');
  const EX_ROWS = {
    pending: 'published tree excludes reports/pending-publish.md',
    governance: 'published tree excludes BACKLOG.md, CLAUDE.md, CONSTITUTION.md, DECISIONS.md and specs/',
    state: 'published tree excludes state/',
    internal: 'published tree excludes reports/week-*, reports/phase-* and reports/interop-outreach.md',
  };
  function exclusionRows(gate, root, scanRootArg, modeOpts = {}) {
    const exMode = exScope.exclusionCheckMode(root, { scanRootArg, ...modeOpts });
    const exHits = exScope.publishExclusionHits(listFiles(root));
    const rows = [];
    const exclusion = (name, hits, key) => (exMode.mode === 'assert'
      ? gate.check(name, hits.length === 0, { [key]: hits.slice(0, 20), held_because: exMode.why })
      : gate.notApplicable(name, exMode.why));
    const row = (name, hits, key) => { exclusion(name, hits, key); rows.push(gate.results[gate.results.length - 1]); };
    ...
    // tree (where build-public.sh has excluded it).
    row(EX_ROWS.pending, exHits.pending, 'pendingHits');

    // Spec-anchored governance files are working documents, not products: the
    // decision log carries strategy, rejected options and risk posture, the
    // backlog carries unclosed defects, carried reds and an unexecuted release
    // plan, and the confidentiality scan can only catch deny-listed TERMS — it can
    // never know a document is internal BY NATURE. They are tracked on dev (that
    // is the point: they are version-controlled governance), so like
    // pending-publish.md this is asserted only against a published tree, where
    // build-public.sh has excluded them.
    //
    // BACKLOG.md ADDED 2026-09-12, red first. The rule is this log's first entry
    // (2026-08-18) and it names a property — governance files stay out — but both
    // halves of the control name INSTANCES, and BACKLOG.md was written after them.
    // Measured before it was closed: a scratch build of the tree as it stood
    // carried BACKLOG.md, 43,164 bytes, tracked in the publish commit, and this
    // assertion read PASS over it because none of the three names it then carried
    // was present. A fifth governance artifact is the point at which this
    // enumeration should stop being an enumeration.
    //
    // CLAUDE.md ADDED 2026-09-23, the fifth, on the operator's ruling (spec 048 A-048-2, P-3): it
    // carries environment facts that are not for the public tree. It is added by name, as the four
    // were; replacing the enumeration with a class is owed (spec 048 PACKET.md, Owed). Publishing a curated public constitution stays open as a
    // deliberate choice; this guard only prevents it happening by accident.
    row(EX_ROWS.governance, exHits.governance, 'govHits');
    ...
    row(EX_ROWS.state, exHits.state, 'stateHits');
    ...
    // carrying dollar figures. A regex-only guard fails SILENTLY. This row and the
    // pattern check below are the two assertions that make #15's sentence true
    // rather than aspirational (#22).
    row(EX_ROWS.internal, exHits.internal, 'internalHits');
    return rows;
  }
  const exRows = exclusionRows(gate, SCAN_ROOT, SCAN_ROOT_ARG);

  const isInternalNarrative = exScope.isInternalNarrative;
  {
  ...
  // THE ARMS (A-028-36) read the rows through exclusionRows, over published trees
  // planted ON DISK: a fresh `git init` each, plants force-added and committed (the
  // shape a leak through a failed EXCLUDE_RE takes), read through the same
  // listFiles, fs and git the real run reads, with no --scan-root. Their rows go to
  // a recorder, not to this gate, so a planted red never prints as this gate's own.
  // Whether a row caught its plant is read from the row, never from
  // publishExclusionHits, which is the rows' own function.
  const exRecorder = () => ({
    results: [],
    check(name, pass, detail = null) { this.results.push({ name, pass: !!pass, notApplicable: false, detail }); return !!pass; },
    notApplicable(name, cause) { this.results.push({ name, pass: true, notApplicable: true, cause }); return true; },
  });
  const exBase = ['README.md', 'package.json', 'lib/runner.js', 'docs/index.html', 'tests/gate.js'];
  const exPlanted = [];
  const plantTree = (files) => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'driftproof-exclusion-'));
    exPlanted.push(d);
    for (const f of files) { fs.mkdirSync(path.dirname(path.join(d, f)), { recursive: true }); fs.writeFileSync(path.join(d, f), `planted: ${f}\n`); }
    const git = (...a) => execFileSync('git', ['-c', 'user.name=gate', '-c', 'user.email=gate@local', '-c', 'commit.gpgsign=false', ...a], { cwd: d, stdio: 'pipe' });
    git('init', '-q', '-b', 'main');
    git('add', '-f', '--', ...files);
    git('commit', '-q', '-m', 'planted');
    return d;
  };
  const rowSummary = (r) => ({ name: r.name, pass: r.pass, notApplicable: r.notApplicable });
  try {
    // MUTATION (A-028-36, N-1), THE ROWS THEMSELVES. For each set, a published tree
    // carrying one file from it: that set's row FAILS and names the file, and the
    // other three pass as assertions. A clean tree asserts and passes all four.
    // Reverted to a skip counted as a pass, the rows cannot fail, and this is red.
    {
      const plants = { governance: 'DECISIONS.md', state: 'state/seen-models.json', pending: 'reports/pending-publish.md', internal: 'reports/week-9.md' };
      const clean = exclusionRows(exRecorder(), plantTree(exBase), null);
      const cleanOk = clean.length === 4 && clean.every((r) => r.pass && !r.notApplicable);
      const results = Object.entries(plants).map(([key, file]) => {
        const rows = exclusionRows(exRecorder(), plantTree([...exBase, file]), null);
        const own = rows.find((r) => r.name === EX_ROWS[key]);
        const others = rows.filter((r) => r.name !== EX_ROWS[key]);
        const caught = !!own && !own.pass && !own.notApplicable && JSON.stringify(own.detail || {}).includes(file);
        const othersAssertPass = others.length === 3 && others.every((r) => r.pass && !r.notApplicable);
        return { key, file, caught, othersAssertPass, own: own ? rowSummary(own) : null };
      });
      gate.check('MUTATION: the four exclusion rows, read over published trees on disk without --scan-root, each go red on a plant from their own set and name it, and a clean tree asserts all four',
        cleanOk && results.every((r) => r.caught && r.othersAssertPass), { clean: clean.map(rowSummary), results });
    }

    // A-028-36, N-1: THE ROWS THIS RUN RECORDED ARE THOSE ROWS. Exactly one row of
    // each name in this gate's results, each the record exclusionRows made. A row
    // written at the call site beside the function, a skip or anything else, is a
    // second row of its name or a row the function did not make.
    {
      const named = gate.results.filter((r) => Object.values(EX_ROWS).includes(r.name));
      const perName = Object.values(EX_ROWS).map((n) => ({ name: n, count: named.filter((r) => r.name === n).length }));
      gate.check('the four exclusion rows this run recorded are the rows exclusionRows made, one of each, none written beside it',
        exRows.length === 4 && perName.every((p) => p.count === 1) && named.every((r) => exRows.includes(r)),
        { perName, fromFunction: named.map((r) => exRows.includes(r)) });
    }

    // MUTATION (A-028-36, N-2), THE WHOLE GOVERNANCE SET PLANTED. A published tree
    // carrying specs/, DECISIONS.md, CONSTITUTION.md, BACKLOG.md and a state/ file,
    // with its own one-commit history, read without the flag: all four rows assert,
    // and the governance and state/ rows go red naming their plants. CONTROL: the
    // same tree with the source history reported present is not applicable on all
    // four, so the history decides, not the set.
    {
      const gov = ['specs/000-planted/spec.md', 'DECISIONS.md', 'CONSTITUTION.md', 'BACKLOG.md'];
      const d = plantTree([...exBase, ...gov, 'state/seen-models.json']);
      const rows = exclusionRows(exRecorder(), d, null);
      const control = exclusionRows(exRecorder(), d, null, { history: () => ({ present: true, why: 'CONTROL: source history reported present' }) });
      const byName = (n) => rows.find((r) => r.name === n);
      const gRow = byName(EX_ROWS.governance), sRow = byName(EX_ROWS.state);
      const asserted = rows.length === 4 && rows.every((r) => !r.notApplicable);
      const govRed = !!gRow && !gRow.pass && gov.every((f) => JSON.stringify(gRow.detail || {}).includes(f));
      const stateRed = !!sRow && !sRow.pass && JSON.stringify(sRow.detail || {}).includes('state/seen-models.json');
      const controlNa = control.length === 4 && control.every((r) => r.notApplicable);
      gate.check('MUTATION: a published tree carrying the whole governance set and a state/ file, with no source history, asserts all four exclusion rows and goes red on its plants; CONTROL: the same tree with source history reported is not applicable',
        asserted && govRed && stateRed && controlNa,
        { asserted, govRed, stateRed, controlNa, rows: rows.map(rowSummary), control: control.map(rowSummary) });
    }
  } finally {
    for (const d of exPlanted) fs.rmSync(d, { recursive: true, force: true });
  }
}
...
gate.section('report-001 manifest');
{
  ...
  if (manifest && manifest.skills) {
    ...
    // A REPORT COMPARED WITH ONE OTHER REPORT (spec 047 R-8). Report 011 ran Report 009's
    // own suites, which are not the pins above, and its page says "the same suites" of
    // them. It is declared in suite_pins.compared_to, and the claim holds only while every
    // one of its receipts carries the suite_hash the other report's receipt for the same
    // skill carries, with at least one skill shared, so the check cannot pass on nothing.
    const comparedTo = (manifest.suite_pins && manifest.suite_pins.compared_to) || {};
    const hashesOf = (rep) => {
      const rdir = path.join(ROOT, 'receipts', rep);
      if (!fs.existsSync(rdir)) return null;
      return fs.readdirSync(rdir).filter((x) => x.endsWith('.json') && !x.includes('_index')).map((f) => {
        let j; try { j = JSON.parse(fs.readFileSync(path.join(rdir, f), 'utf8')); } catch (e) { return null; }
        return { f, slug: (j.skill && (j.skill.slug || j.skill.name)) || f.split('__')[0], h: j.suite && j.suite.suite_hash };
      }).filter(Boolean);
    };
    const comparedMismatch = (pairs) => {
      const bad = [];
      for (const [rep, other] of Object.entries(pairs)) {
        const mine = hashesOf(rep); const theirs = hashesOf(other);
        if (!mine || !theirs) { bad.push(`${rep} -> ${other}: a receipts directory is missing`); continue; }
        const want = new Map(theirs.map((x) => [x.slug, x.h]));
        const shared = mine.filter((x) => want.has(x.slug));
        if (!shared.length) bad.push(`${rep} -> ${other}: no skill shared, so nothing is compared`);
        for (const x of mine) if (!want.has(x.slug) || x.h !== want.get(x.slug)) bad.push(`${rep}/${x.f}: suite_hash ${String(x.h).slice(0, 10)} != ${other}'s ${String(want.get(x.slug)).slice(0, 10)}`);
      }
      return bad;
    };
    const cmpBad = comparedMismatch(comparedTo);
    gate.check('every report declared as comparing its suites with one other report carries that report\'s suite_hash for each skill, sharing at least one',
      cmpBad.length === 0, { compared_to: comparedTo, mismatch: cmpBad.slice(0, 8) });
    // The plant: the declaration pointed at a report whose suites differ reads red.
    gate.check('MUTATION: a compared_to entry pointing at a report with other suites is red',
      !('report-011' in comparedTo) || comparedMismatch({ 'report-011': 'report-001' }).length > 0);
    const undeclared = claimants.filter((r) => !comparable.includes(r) && !(r in comparedTo));
...
gate.section('report-001 receipts');
{
  ...
  if (fs.existsSync(costLog)) {
    ...
    // Either the build's own verification run, or a tree that says by its own
    // contents that it is a published one (spec 030 AC-5). Still non-vacuous in
    // both directions: on a dev tree `specs/` exists, so deleting the cost log
    // there still fails this.
    const rootIsPublishedTree = (!!SCAN_ROOT_ARG && path.resolve(ROOT) === SCAN_ROOT)
      || ROOT_IS_PUBLISHED_TREE;
...
gate.section('release trigger (mocked)');
{
  ...
  // THE TRIGGER REGISTERS NOTHING (spec 021, F-W1). This asserted the opposite:
  // that the new id was appended to the registry with auto_added. That behaviour
  // is the finding - the watcher wrote a TRACKED config/models.json in place, on
  // a timer, dirtying whichever worktree the unit fired in. The contract now is
  // that the registry is untouched and the discovery lands in an untracked queue
  // for a human to register deliberately, so this asserts both halves: what must
  // NOT happen, and what must.
  const regAfter = JSON.parse(fs.readFileSync(regPath, 'utf8'));
  gate.check('the trigger appended NOTHING to the registry: no auto_added row, and the new id is absent',
    !regAfter.models.some((m) => m.auto_added) && !regAfter.models.some((m) => m.id === 'claude-sonnet-6'),
    { autoAdded: regAfter.models.filter((m) => m.auto_added).map((m) => m.id) });
  const queueFile = path.join(stateDir, 'discovery-queue.jsonl');
  const queueRows = fs.existsSync(queueFile)
    ? fs.readFileSync(queueFile, 'utf8').split('\n').filter(Boolean).map((l) => JSON.parse(l))
    : [];
  const queued = queueRows.find((e) => e.model_id === 'claude-sonnet-6');
  gate.check('the discovery is queued instead, to an untracked state path, with a source and its prices marked unverified',
    !!queued && queued.price_verified === false && typeof queued.source === 'string' && /infer/i.test(queued.source)
    && queued.first_seen === '2026-07-29' && queued.date_is_first_seen === true,
    { queued: queued || null, rows: queueRows.length });
...
gate.section('github action');
{
  ...
  // REVERSED by spec 023 (audit A1), recorded in DECISIONS. This used to assert
  // `--max-usd "${{ inputs.max-usd }}"` reaches the CLI, which is the
  // interpolation the spec removes: an expression substituted into run: is
  // shell code. It now asserts the input is mapped into env: as INPUT_<NAME>
  // and that action/run.sh passes the quoted variable. The step bodies moved
  // to action/*.sh so the spec gate can execute them; the checks below read
  // those files for the strings that moved with them.
  const runSh = fs.existsSync(path.join(ROOT, 'action', 'run.sh')) ? fs.readFileSync(path.join(ROOT, 'action', 'run.sh'), 'utf8') : '';
  const enforceSh = fs.existsSync(path.join(ROOT, 'action', 'enforce.sh')) ? fs.readFileSync(path.join(ROOT, 'action', 'enforce.sh'), 'utf8') : '';
  const envName = (k) => `INPUT_${k.toUpperCase().replace(/-/g, '_')}`;
  const passes = (k) => new RegExp(`^\\s+${envName(k)}: \\$\\{\\{ inputs\\.${k} \\}\\}$`, 'm').test(a)
    && new RegExp(`--${k} "\\$${envName(k)}"`).test(runSh);
  ...
  gate.check('action runs bin/driftproof + badge (action/run.sh) + uploads the receipt artifact', /bin\/driftproof/.test(runSh) && /badge/.test(runSh) && /upload-artifact/.test(a));
  gate.check('action fails the job on REGRESSED (action/enforce.sh, guarded by fail-on-regression)', /fail-on-regression/.test(a) && /REGRESSED/.test(enforceSh) && /exit 1/.test(enforceSh));
  ...
  // REVERSED by spec 026 (AC-1, A-026-1): a stub run measures nothing, so the
  // self-test asserts NOT_MEASURED, UNVERIFIED and surface stub on it, and no
  // longer PASSED. Recorded in DECISIONS 2026-09-06 under the gate-drift rule.
  gate.check('selftest runs the bundled example skill and asserts NOT_MEASURED, UNVERIFIED and surface stub (a stub measures nothing)', /examples\/commit-message-conventions/.test(w) && /NOT_MEASURED/.test(w) && /UNVERIFIED/.test(w) && /surface[^\n]*stub|stub[^\n]*surface/.test(w) && !/test "\$VERDICT" = "PASSED"/.test(w));
  ...
  try {
    ...
    // REVERSED by spec 026 (AC-1): a stub run's receipt is UNVERIFIED, surface
    // stub, answered_by.kind stub, and verdicts NOT_MEASURED; the badge says so.
    gate.check('local stub run yields verdict NOT_MEASURED on the example skill (UNVERIFIED, surface stub, answered_by.kind stub)',
      v.verdict === 'NOT_MEASURED' && receipt.verification_level === 'UNVERIFIED' && receipt.run.surface === 'stub' && receipt.run.answered_by && receipt.run.answered_by.kind === 'stub', { v, level: receipt.verification_level, surface: receipt.run.surface });
    // REVERSED by spec 023 (audit A6): the writer no longer emits `verdict=PASSED`
    // as a bare name=value line; it emits GitHub's heredoc block with a random
    // delimiter, and this now asserts that block. Spec 026 moves the value to
    // NOT_MEASURED, which is what a stub run's verdict is.
    gate.check('badge --github-output emits verdict as a heredoc block (verdict<<ghadelim_<hex> / NOT_MEASURED / delimiter)',
      /^verdict<<(ghadelim_[0-9a-f]{32})\nNOT_MEASURED\n\1$/m.test(githubOutputLines(receipt)));
  } finally {
    fs.rmSync(outDir, { recursive: true, force: true });
  }
}

// ── 20a. Spec 023: the Action's inputs are data (BLOCKING) ────────────────────
// Offline mirror of specs/023-action-input-hardening (audit A1, A6, A9), here
// because specs/ is excluded from the published tree. The full gate, with the
// two mutations that execute the pre-fix bytes from git, is the spec's own.
gate.section('spec 023 action input hardening');
{
  const actionText = fs.readFileSync(path.join(ROOT, 'action.yml'), 'utf8');
  const wfText = fs.readFileSync(path.join(ROOT, '.github', 'workflows', 'action-selftest.yml'), 'utf8');
  const scripts = ['lib.sh', 'run.sh', 'enforce.sh'].map((f) => ({ f: `action/${f}`, text: fs.existsSync(path.join(ROOT, 'action', f)) ? fs.readFileSync(path.join(ROOT, 'action', f), 'utf8') : '' }));
  // every run: block, inline or block scalar, without a YAML parser
  const runBlocks023 = (yaml) => {
    const lines = yaml.split('\n'); const out = [];
    for (let i = 0; i < lines.length; i++) {
      const m = lines[i].match(/^(\s*)run:\s*(.*)$/); if (!m) continue;
      const indent = m[1].length; const rest = m[2].trim();
      if (/^[|>]-?$/.test(rest)) {
        const body = []; let j = i + 1;
        for (; j < lines.length; j++) { const l = lines[j]; if (l.trim() === '') { body.push(''); continue; } if (l.match(/^(\s*)/)[1].length <= indent) break; body.push(l); }
        out.push({ line: i + 1, text: body.join('\n') });
      } else out.push({ line: i + 1, text: rest });
    }
    return out;
  };
  // AC-1: no expression inside any script
  const exprHits = [];
  for (const [name, text] of [['action.yml', actionText], ['action-selftest.yml', wfText]]) {
    const blocks = runBlocks023(text);
    if (blocks.length === 0) exprHits.push(`${name}: no run: blocks`);
    for (const b of blocks) if (b.text.includes('${{')) exprHits.push(`${name}:${b.line}`);
  }
  gate.check('AC-1: no ${{ inside any run: block of action.yml or the selftest workflow', exprHits.length === 0, { exprHits });
  const inputs023 = ['skill-dir', 'models', 'max-usd', 'max-calls', 'fail-on-regression'];
  const envMissing = inputs023.filter((k) => !new RegExp(`^\\s+INPUT_${k.toUpperCase().replace(/-/g, '_')}: \\$\\{\\{ inputs\\.${k} \\}\\}$`, 'm').test(actionText));
  // DRIFTPROOF_RECEIPTS joins the pair at spec 030 AC-1: enforcement decides
  // over the receipt DIRECTORY, which reaches its shell the same way.
  const outMissing = [['DRIFTPROOF_VERDICT', 'verdict'], ['DRIFTPROOF_DELTA', 'delta'],
    ['DRIFTPROOF_RECEIPTS', 'receipts_dir']].filter(([k, v]) => !new RegExp(`^\\s+${k}: \\$\\{\\{ steps\\.run\\.outputs\\.${v} \\}\\}$`, 'm').test(actionText)).map(([k]) => k);
  // The expression body admits `_` (spec 030 adds receipts_dir, worst_state,
  // regressed_models, missing_models). Widening the CHARACTER CLASS does not
  // widen the rule: a line must still be an UPPER_CASE env key or the action's
  // own `value:` declaration, and neither of those is a shell.
  // Spec 023 v1.4, for spec 050 AC-5: the upload step's two `with:` lines name THIS
  // invocation's directory and artifact. A `with:` value is an input to the pinned
  // upload action, never text a shell parses, and both values are written by run.sh
  // from `mktemp -d`, not from any input. So exactly those two lines are admitted,
  // by their whole text; any other expression in a `with:` still escapes, which the
  // mutation below plants.
  const uploadWith023 = /^\s+(name: \$\{\{ steps\.run\.outputs\.artifact_name \}\}|path: \$\{\{ steps\.run\.outputs\.receipts_dir \}\})$/;
  const escapedIn = (text) => text.split('\n').filter((l) => /\$\{\{ (inputs|steps\.run\.outputs)\./.test(l) && !/^\s+[A-Z][A-Z0-9_]*: \$\{\{ [a-z0-9._-]+ \}\}$/.test(l) && !/^\s+value: \$\{\{ steps\.run\.outputs\.[a-z0-9_]+ \}\}$/.test(l) && !uploadWith023.test(l));
  const escaped = escapedIn(actionText);
  gate.check('AC-1: every input is an env: INPUT_<NAME> entry and all three step outputs are env: entries; no expression escapes env:/value:, but for the upload step\'s two per-invocation with: lines', envMissing.length === 0 && outMissing.length === 0 && escaped.length === 0, { envMissing, outMissing, escaped });
  {
    const planted = actionText.replace(/^(\s+)if-no-files-found: error$/m, '$1if-no-files-found: error\n$1retention-days: ${{ inputs.max-calls }}\n$1path: ${{ steps.run.outputs.verdict }}');
    const hits = escapedIn(planted);
    gate.check('AC-1 MUTATION: an input and a different step output planted in the upload step\'s with: both escape',
      planted !== actionText && hits.length === 2, { hits });
  }
  // every $INPUT_ / $DRIFTPROOF_ reference in the scripts sits inside double quotes
  const unquoted = [];
  for (const { f, text } of scripts) {
    if (!text) { unquoted.push(`${f}: missing`); continue; }
    text.split('\n').forEach((line, n) => {
      ...
      for (let i = 0; i < line.length; i++) {
        ...
        if (c === '$' && /^\$\{?(INPUT_|DRIFTPROOF_)/.test(line.slice(i)) && !dq) unquoted.push(`${f}:${n + 1}`);
      }
    });
  }
  gate.check('AC-1: action/*.sh reference INPUT_* and DRIFTPROOF_* only inside double quotes', unquoted.length === 0, { unquoted });
  // AC-2/AC-3 behaviour: a hostile models value is refused before any node starts, and nothing it carries runs
  {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'driftproof-023-'));
    try {
      const shim = path.join(tmp, 'shim'); fs.mkdirSync(shim);
      fs.writeFileSync(path.join(shim, 'node'), `#!/usr/bin/env bash\ntouch "${tmp}/node-started"\nexit 0\n`, { mode: 0o755 });
      const payload = `x"; touch ${tmp}/mark-semi; $(touch ${tmp}/mark-subst); \`touch ${tmp}/mark-tick\`; \ntouch ${tmp}/mark-nl; : "`;
      const r = spawnSync('bash', [path.join(ROOT, 'action', 'run.sh')], {
        cwd: tmp, encoding: 'utf8',
        env: { PATH: `${shim}:${process.env.PATH}`, HOME: process.env.HOME, GITHUB_ACTION_PATH: ROOT, RUNNER_TEMP: tmp, GITHUB_OUTPUT: path.join(tmp, 'out'),
          INPUT_SKILL_DIR: EXAMPLE, INPUT_MODELS: payload, INPUT_MAX_USD: '20', INPUT_MAX_CALLS: '2000', INPUT_FAIL_ON_REGRESSION: 'true' },
      });
      const left = fs.readdirSync(tmp).filter((f) => f.startsWith('mark-') || f === 'node-started');
      gate.check('AC-2/AC-3: a models value carrying " ; $() ` and a newline is refused with ::error before node, and no clause of it runs',
        r.status !== 0 && /^::error title=Driftproof::models:/m.test(r.stdout + r.stderr) && left.length === 0, { status: r.status, left, out: (r.stdout + r.stderr).slice(0, 300) });
    } finally { fs.rmSync(tmp, { recursive: true, force: true }); }
  }
  // AC-4: the writer refuses a line break and emits heredoc entries with distinct random delimiters
  {
    const rcp = synthReceipt([mkcase('c1', 'with_skill', [0.9, 0.85, 0.9, 0.88, 0.9], 0.7), mkcase('c1', 'baseline', [0.5, 0.55, 0.5, 0.52, 0.5], 0.7)]);
    const bad = JSON.parse(JSON.stringify(rcp)); bad.run.model_id = 'claude-haiku-4-5\nverdict=PASSED';
    let threw = false; try { githubOutputLines(bad); } catch (e) { threw = /line break/.test(e.message); }
    gate.check('AC-4: githubOutputLines throws on a model_id with a newline (nothing is written)', threw);
    const text = githubOutputLines(rcp);
    const blocks = [...text.matchAll(/^([a-z]+)<<(ghadelim_[0-9a-f]{32})\n([^\n]*)\n\2$/gm)];
    const keys = blocks.map((b) => b[1]); const delims = new Set(blocks.map((b) => b[2]));
    const plain = text.split('\n').filter((l) => /^[a-z]+=/.test(l));
    gate.check('AC-4: githubOutputLines emits exactly verdict/delta/message/color as heredoc blocks, four distinct random delimiters, no name=value line',
      JSON.stringify(keys) === JSON.stringify(['verdict', 'delta', 'message', 'color']) && delims.size === 4 && plain.length === 0, { keys, delims: [...delims], plain });
  }
  // AC-5: full-SHA pins with a version comment; npm ci with no fallback
  const uses = [];
  for (const [name, text] of [['action.yml', actionText], ['action-selftest.yml', wfText]]) {
    for (const [n, l] of text.split('\n').entries()) {
      const m = l.match(/^\s*-?\s*uses:\s*(.+)$/); if (!m) continue;
      const v = m[1].trim(); if (v === './' || v === "'./'") continue;
      uses.push({ where: `${name}:${n + 1}`, v, pinned: /^[\w.-]+\/[\w.-]+@[0-9a-f]{40}\s+#\s*v\d+\.\d+\.\d+$/.test(v) });
    }
  }
  gate.check('AC-5: every third-party uses: in action.yml and the workflow is owner/repo@<40-hex sha> # vX.Y.Z', uses.length === 3 && uses.every((u) => u.pinned), { uses });
  gate.check('AC-5: the install step is npm ci with no npm install fallback', /npm ci --omit=dev --no-audit --no-fund\s*$/m.test(actionText) && !/npm install/.test(actionText));
}

// ── 20a2. Spec 024: the public self-test proves refusal on the runner (BLOCKING) ──
// Offline mirror of specs/024-runner-hostile-input-proof, here because specs/
// is excluded from the published tree. The workflow that ships passes one
// hostile value through each input of the real action and fails unless every
// one is refused; this section reads that file and executes its assert block
// against this tree, once clean (green) and once with a planted marker (red).
gate.section('spec 024 runner hostile-input proof');
{
  const wfText = fs.readFileSync(path.join(ROOT, '.github', 'workflows', 'action-selftest.yml'), 'utf8');
  const lines = wfText.split('\n');
  // a block scalar under `<key>: |` at the first line matching `re`
  const block = (re) => {
    const i = lines.findIndex((l) => re.test(l)); if (i < 0) return null;
    const ind = lines[i].match(/^ */)[0].length; const body = []; let cind = null;
    for (let j = i + 1; j < lines.length; j++) {
      const l = lines[j]; if (l.trim() === '') { body.push(''); continue; }
      const li = l.match(/^ */)[0].length; if (li <= ind) break;
      if (cind === null) cind = li; body.push(l.slice(cind));
    }
    while (body.length && body[body.length - 1] === '') body.pop();
    return body.join('\n') + '\n';
  };
  const hostile = block(/^      HOSTILE: \|$/);
  const classes = hostile ? [/"/, /;/, /\$\(/, /`/, /\n[^\n]/].every((re) => re.test(hostile)) : false;
  const inputs024 = ['skill-dir', 'models', 'max-usd', 'max-calls', 'fail-on-regression'];
  const stepText = (id) => { const i = lines.findIndex((l) => l.trim() === `id: ${id}`); if (i < 0) return ''; let j = i; while (j > 0 && !/^      - /.test(lines[j])) j--; let k = i + 1; while (k < lines.length && !/^      - /.test(lines[k])) k++; return lines.slice(j, k).join('\n'); };
  const hostileOk = inputs024.filter((inp) => { const t = stepText(`hostile-${inp}`); return /uses: \.\//.test(t) && /continue-on-error: true/.test(t) && new RegExp(`^\\s+${inp}: \\$\\{\\{ env\\.HOSTILE \\}\\}$`, 'm').test(t); });
  const assertText = stepText('assert-refused');
  const outcomesMapped = inputs024.every((inp) => new RegExp(`^\\s+O_[A-Z_]+: \\$\\{\\{ steps\\.hostile-${inp}\\.outcome \\}\\}$`, 'm').test(assertText));
  gate.check('AC-1: HOSTILE declared once with " ; $( ` and a newline; five hostile-<input> steps through uses: ./ with continue-on-error; assert-refused maps the five outcomes through env:',
    classes && hostileOk.length === 5 && outcomesMapped, { classes, hostileOk, outcomesMapped });
  // AC-4 (spec 024 v1.1, approval F-1): action.yml uploads its receipt under ONE
  // fixed name and actions/upload-artifact v4 refuses a second upload of a name
  // on the run, so the workflow may run the action to success at most once:
  // exactly one `uses: ./` step without continue-on-error, and it is the benign
  // step. Mutation: a second such step spliced into the text moves the count.
  const actionText024 = fs.readFileSync(path.join(ROOT, 'action.yml'), 'utf8');
  const upIdx = actionText024.split('\n').findIndex((l) => /uses: actions\/upload-artifact@/.test(l));
  const upBlock = upIdx >= 0 ? actionText024.split('\n').slice(upIdx, upIdx + 8).join('\n') : '';
  // Spec 024 v1.3, for spec 050 AC-5: the name was ONE FIXED NAME, which is why a job
  // could run the action to success once. It is now the run step's artifact_name,
  // one per invocation, over that invocation's receipts_dir. The one-success shape of
  // the self-test below still holds, and is still asserted; the fixed name is what
  // no longer does.
  const perInvocation = /^\s+name: \$\{\{ steps\.run\.outputs\.artifact_name \}\}$/m.test(upBlock) && /^\s+path: \$\{\{ steps\.run\.outputs\.receipts_dir \}\}$/m.test(upBlock);
  const noOverwrite = !/^\s+overwrite:/m.test(upBlock);
  // step blocks: the text between one `      - ` and the next
  const successfulUses = (text) => text.split(/\n(?=      - )/).filter((b) => /^\s+uses: \.\/\s*$/m.test(b) && !/^\s+continue-on-error: true\s*$/m.test(b));
  const canSucceed = successfulUses(wfText);
  gate.check('AC-4: action.yml uploads each invocation\'s receipts_dir under its artifact_name, with no overwrite, and exactly one uses: ./ step of the self-test can run to success (the benign step)',
    perInvocation && noOverwrite && canSucceed.length === 1 && /id: driftproof/.test(canSucceed[0]), { perInvocation, noOverwrite, canSucceed: canSucceed.map((b) => (b.match(/id: (\S+)/) || [])[1]) });
  const spliced = wfText.replace(/(      - name: Run the Driftproof action)/, '      - name: Second successful run\n        id: second-run\n        uses: ./\n        with:\n          skill-dir: examples/commit-message-conventions\n\n$1');
  const canSucceedMut = successfulUses(spliced);
  gate.check('AC-4 MUTATION: a second uses: ./ step without continue-on-error spliced before the benign run takes the count to two',
    spliced !== wfText && canSucceedMut.length === 2, { count: canSucceedMut.length });
  // the assert block, executed against THIS tree the way the runner executes it
  const assertRun = (() => { const i = lines.findIndex((l) => l.trim() === 'id: assert-refused'); const sub = lines.slice(i); const r = sub.findIndex((l) => /^        run: \|$/.test(l)); if (r < 0) return null; const ind = 8; const body = []; let cind = null; for (let j = r + 1; j < sub.length; j++) { const l = sub[j]; if (l.trim() === '') { body.push(''); continue; } const li = l.match(/^ */)[0].length; if (li <= ind) break; if (cind === null) cind = li; body.push(l.slice(cind)); } return body.join('\n') + '\n'; })();
  const runAssert = (tmp, outcomes) => {
    const rt = path.join(tmp, 'rt'); fs.mkdirSync(rt, { recursive: true });
    const script = path.join(tmp, 'assert.sh'); fs.writeFileSync(script, assertRun);
    const r = spawnSync('bash', [script], { cwd: ROOT, encoding: 'utf8', env: { PATH: process.env.PATH, HOME: process.env.HOME, DRIFTPROOF_STUB: '1', GITHUB_WORKSPACE: ROOT, RUNNER_TEMP: rt, GITHUB_OUTPUT: path.join(tmp, 'out'), HOSTILE: hostile, ...outcomes } });
    return { status: r.status, out: (r.stdout || '') + (r.stderr || ''), rt };
  };
  const allFail = { O_SKILL_DIR: 'failure', O_MODELS: 'failure', O_MAX_USD: 'failure', O_MAX_CALLS: 'failure', O_FAIL_ON_REGRESSION: 'failure' };
  if (assertRun && hostile) {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'driftproof-024-'));
    try {
      const green = runAssert(tmp, allFail);
      gate.check('AC-3: the workflow\'s assert block, executed against this tree with five refusals, exits 0 (the shipped script refuses the payload in every input)',
        green.status === 0, { status: green.status, out: green.out.slice(-300) });
      const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), 'driftproof-024m-'));
      try {
        fs.mkdirSync(path.join(tmp2, 'rt'), { recursive: true });
        fs.writeFileSync(path.join(tmp2, 'rt', 'driftproof-injection-semi'), '');
        const red = runAssert(tmp2, allFail);
        gate.check('AC-3 MUTATION: with a marker planted and every outcome reading failure, the same block exits non-zero and its first refusal names the marker',
          red.status !== 0 && /^::error title=Driftproof selftest::injection marker present/m.test(red.out), { status: red.status, out: red.out.slice(0, 300) });
      } finally { fs.rmSync(tmp2, { recursive: true, force: true }); }
    } finally { fs.rmSync(tmp, { recursive: true, force: true }); }
  } else {
    gate.check('AC-3: the workflow\'s assert block, executed against this tree with five refusals, exits 0 (the shipped script refuses the payload in every input)', false, { assertRun: !!assertRun, hostile: !!hostile });
    gate.check('AC-3 MUTATION: with a marker planted and every outcome reading failure, the same block exits non-zero and its first refusal names the marker', false, {});
  }
}
...
gate.section('badge shields schema');
{
  ...
  // SPEC 035 R-3. A single-draw receipt whose arms do not separate cannot show an
  // across-draw spread, so below the floor it reads UNDERPOWERED, not NO_EFFECT; a
  // generation-sampled receipt whose spreads and draws resolve the floor still reads
  // NO_EFFECT. Both halves are asserted, so neither state can absorb the other.
  const sampledNoeff = JSON.parse(JSON.stringify(noeff));
  for (const c of sampledNoeff.results.cases) c.generation = { mean: c.mean, sd: 0.005, n_measured: 5, n_drawn: 5, draws: [] };
  const bs = badgeEndpoint(sampledNoeff);
  gate.check('badge below the effect floor on a single-draw receipt is UNDERPOWERED/blue (spec 035 R-3)', validShields(bn) && bn.color === 'blue' && verdictFromReceipt(noeff).verdict === 'UNDERPOWERED', bn);
  gate.check('badge below the effect floor on a sampled receipt that resolves the floor is NO_EFFECT/grey', validShields(bs) && bs.color === 'lightgrey' && verdictFromReceipt(sampledNoeff).verdict === 'NO_EFFECT', bs);
  ...
  gate.check('badge color is a known shields color', COLORS.has(bp.color) && COLORS.has(bn.color) && COLORS.has(br.color) && COLORS.has(bs.color));
...
gate.section('verdict logic');
{
  ...
  // SPEC 035: below the floor, a single-draw receipt is UNDERPOWERED and a sampled one
  // whose spreads and draws resolve the floor is NO_EFFECT.
  const sampled = (r, sd, n) => { for (const c of r.results.cases) c.generation = { mean: c.mean, sd, n_measured: n, n_drawn: n, draws: [] }; return r; };
  gate.check('|delta| < 0.05 on a single-draw receipt → UNDERPOWERED', verdictFromReceipt(mk(0.80, 0.78)).verdict === 'UNDERPOWERED');
  gate.check('|delta| < 0.05 on a sampled receipt that resolves the floor → NO_EFFECT', verdictFromReceipt(sampled(mk(0.80, 0.78), 0.005, 5)).verdict === 'NO_EFFECT');
...
{
  const prov = require('../lib/provider');
  // spec 022 AC-9 (approval F-1): NO -o. The final text is taken from the
  ...
  gate.check('codex template = read-only sandbox + --ephemeral + --skip-git-repo-check and NO -o / --output-last-message (spec 022 AC-9)',
    args[args.indexOf('-s') + 1] === 'read-only' && args.includes('--ephemeral') && args.includes('--skip-git-repo-check') && !args.includes('-o') && !args.includes('--output-last-message'));
  gate.check('codex args omit -m when no model is given', !prov.buildCodexArgs({}).includes('-m'));
  ...
  const finalArgs = prov.codexFinalArgs({ model: 'gpt-5.6-sol' });
...
gate.section('provider/surface receipts');
{
  ...
  try {
    ...
    // REVERSED by spec 026 (AC-1, F1): this run is a STUB run, and its receipt
    // now says so. The requested model's provider is still recorded (openai),
    // the surface is stub (nothing answered on the codex lane), answered_by.kind
    // is stub, and the codex preamble note is ABSENT: it describes a harness
    // that did not run. The live openai/cli surface's provider/surface pair is
    // asserted through lib/provider.js above (surfaceForModel), not through a
    // stub receipt that used to claim the surface name.
    gate.check('openai/cli STUB receipt: provider=openai (the requested model\'s), surface=stub, answered_by.kind=stub, UNVERIFIED', r.run.provider === 'openai' && r.run.surface === 'stub' && r.run.answered_by && r.run.answered_by.kind === 'stub' && r.verification_level === 'UNVERIFIED', { provider: r.run.provider, surface: r.run.surface, level: r.verification_level });
    gate.check('openai/cli STUB receipt carries NO surface_overhead_note (the codex preamble describes a harness that did not run)', r.run.surface_overhead_note === undefined);
...
{
  // SPEC 020 moved these to clean paths (/neutrality/, /methodology/) and left a
  // stub at each old address. The assertions follow the page rather than the
  // path: `pageAt` reads the clean path and falls back to the old one, so this
  // section keeps testing the document it has always tested, and would go red if
  // either the page or its stub went missing.
  const pageAt = (slug) => {
    for (const rel of [[slug, 'index.html'], [`${slug}.html`]]) {
      const f = path.join(ROOT, 'docs', ...rel);
      if (fs.existsSync(f)) return fs.readFileSync(f, 'utf8');
    }
    return '';
  };
  const nHtml = pageAt('neutrality');
  const mHtml = pageAt('methodology');
  const idx = fs.readFileSync(path.join(ROOT, 'docs', 'index.html'), 'utf8');
  const stubAt = (slug) => fs.existsSync(path.join(ROOT, 'docs', `${slug}.html`));
  gate.check('the neutrality page is present, uses the site stylesheet, and its old address still resolves',
    !!nHtml && /style\.css/.test(nHtml) && stubAt('neutrality'));
  ...
  gate.check('the methodology page is present, links neutrality, and its old address still resolves',
    !!mHtml && /href="\/neutrality\/"|neutrality\.html/.test(mHtml) && stubAt('methodology'));
  ...
  gate.check('index nav links Methodology + Neutrality',
    /href="\/methodology\/"|methodology\.html/.test(idx) && /href="\/neutrality\/"|neutrality\.html/.test(idx));
}
...
gate.section('interop import (honest epistemics)');
{
  ...
  // Spec 043 AC-5: this assertion pinned `run.judge.samples === 1`, the count the importer
  // invented (agentskills discussion #544). The source format defines no count, so the
  // corrected receipt carries none; the conjunct now asserts its absence.
  gate.check('provider inferred from imported model id (gpt-4o-mini → openai); delta from their with/without pass fractions; no invented judge-sample count',
    ase.run.provider === 'openai' && ase.comparison.delta === 0.75 && !('samples' in ase.run.judge));
...
gate.section('interop verdict exclusion (below TESTED is never verdicted)');
{
  ...
  gate.check('the revision headline classifies four ways',
    ...
    && revisionHeadline([{ verdict: require('../lib/stats').WITHIN_NOISE }]).startsWith('NO SEPARATION DETECTED'));
  ...
  gate.check('a cell with no separation detected gets NO amendment sentence (noise is not laundered into a finding)',
...
{
  // SPEC 020 moved these to clean paths and left a stub at each old address.
  // The assertions follow the DOCUMENT, not the path: read the clean path, fall
  // back to the old one, and fail loudly if neither is there.
  const docPage = (slug) => {
    for (const rel of [[slug, 'index.html'], [`${slug}.html`]]) {
      const f = path.join(SCAN_ROOT, 'docs', ...rel);
      if (fs.existsSync(f)) return fs.readFileSync(f, 'utf8');
    }
    throw new Error(`neither docs/${slug}/index.html nor docs/${slug}.html exists`);
  };
  const interopHtml = docPage('interop');
  const interopMd = fs.readFileSync(path.join(SCAN_ROOT, 'docs', 'interop.md'), 'utf8');
  const methodology = docPage('methodology');
  gate.check('the interop page ships in site chrome and explains DECLARED vs TESTED',
    /class="site"/.test(interopHtml) && /DECLARED/.test(interopHtml) && /TESTED/.test(interopHtml) && /open format/i.test(interopHtml));
  gate.check('methodology links the interop page', /href="\/interop\/"|interop\.html/.test(methodology));
  ...
  // SPEC 052 AC-4: the page's receipt version, its frozen range and its import lines, read against
  // the schema, the frozen files under spec/ and the CLI's own --from lists.
  {
    const { checkInteropPage } = require('./interop-page');
    const specDir = path.join(SCAN_ROOT, 'spec');
    const frozen = fs.readdirSync(specDir).map((f) => /^receipt\.v(\d+(?:\.\d+)+)\.schema\.json$/.exec(f)).filter(Boolean).map((m) => m[1]);
    const tools = [...require('../lib/importers').IMPORT_TOOLS, ...require('../lib/importers-anthropic').ANTHROPIC_TOOLS];
    const r = checkInteropPage({ html: interopHtml, current: JSON.parse(repoSchema).properties.schema_version.const, frozen, tools });
    gate.check('the interop page cites the current receipt version, the frozen range and an import line for every --from value', r.ok, r.ok ? null : { problems: r.problems });
  }
...
gate.section('v0.4 presentation rules (ratio floor, latency disclosure)');
{
  ...
  // DID clear the floor is not the no-separation cell: the Verdict basis on the same page
  ...
  gate.check('RATIO FLOOR RULE: overlapping bands render the no-separation cell',
...
gate.section('report-005 staging (value report, stub)');
{
  ...
  // disclosure promised every dollar re-derives from the frozen snapshot.
  {
    ...
    // renamed, a synthetic-object test would keep passing while every receipt broke.
    {
      ...
      // REVERSED by spec 026 (T14): the staged draft's receipts are UNVERIFIED
      // now (a stub run measures nothing), so a DECLARED receipt is no longer
      // the weaker one beside them. The "drags the level down" subject is a
      // PUBLISHED report-005 receipt, which is real and TESTED, read from the
      // archive; the staged sample still proves the field's name above.
      const publishedDir = path.join(ROOT, 'receipts', 'report-005');
      const publishedFile = fs.existsSync(publishedDir) ? fs.readdirSync(publishedDir).find((f) => f.endsWith('.json')) : null;
      const published = publishedFile ? JSON.parse(fs.readFileSync(path.join(publishedDir, publishedFile), 'utf8')) : null;
      gate.check('F5(b) tripwire: a weaker receipt drags the level down (a published TESTED receipt + a DECLARED one; the staged draft is UNVERIFIED and drags DECLARED down itself)',
        !!published && published.verification_level === 'TESTED'
        && weakestVerificationLevel([published, { verification_level: 'DECLARED' }]) === 'DECLARED'
        && weakestVerificationLevel([published, {}]) === 'UNVERIFIED'
        && weakestVerificationLevel([sample, { verification_level: 'DECLARED' }]) === sample.verification_level, { staged: sample.verification_level, published: published && published.verification_level });
    }
  ...
  // whose lift did not separate, so the ratio cell must render the no-separation string.
  const { NOISE_CELL } = require('../lib/value');
  gate.check('draft renders the no-separation cell where the lift did not clear the floor',
    html.includes(NOISE_CELL), { noiseCells: html.split(NOISE_CELL).length - 1 });
  gate.check('the stub exercises BOTH branches (some reportable, some not separated)',
...
// ── receipt integrity (spec 026) ─────────────────────────────────────────────
//
// Spec 026 AC-14 (F-R8). Runs HERE, not only in specs/026's gate, for the reason
// every spec section gives: specs/ is excluded from the published tree and no
// workflow runs a spec gate, so a rule that lived only beside its spec would
// never be executed by a build. In the published tree there is no specs/, so
// the rule reads 0 probes and says so rather than passing in silence.
gate.section('receipt integrity (spec 026)');
{
  const scope026 = require('./assertion-scope');
  // ONE predicate, in tests/assertion-scope.js (probeCopiesCheck), asserted
  // here and exercised by specs/026's mutation on a planted copy.
  const pc = scope026.probeCopiesCheck(ROOT);
  gate.check('no probe carries an undeclared copy of an exported product function (the probe-copies rule; count read)',
    pc.pass && pc.violations.length === 0, { probesRead: pc.probesRead, violations: pc.violations.slice(0, 10), excProblems: pc.excProblems.slice(0, 5) });
  // The count is in the detail, not the name: the name must be the same in
  // both trees for NFR-3's section comparison (it read 85 here and 0 there).
  gate.check('the probe-copies rule reports how many probe files it read (0 in the published tree, where specs/ is absent)',
    Number.isInteger(pc.probesRead), { probesRead: pc.probesRead, routed: pc.routed.length, declared: pc.declared.length });
  gate.check('every declared probe-copy exception names the product function it is independent of and an assertion in its own spec\'s gate that exists',
...
gate.section('publish unblock (generation_sampled, product paths, publish target)');
{
  ...
  // binds what is being merged now, not what was merged before it existed.
  {
    ...
    // SPEC 030 AC-5. An absence is POSITIVELY DETECTED and reported as not
    // applicable; anything else that stops the range being read is still a
    // failure. A `git clone --depth 1 --branch <tag>` carries no `main` ref at
    // all, so the range this rule is about DOES NOT EXIST in that checkout -
    // there is no branch being merged, and no commit for the rule to be wrong
    // about. Failing it there made a green gate unreachable on a clean clone of
    // the published tag however correct the tree was, which is what a
    // contributor following CONTRIBUTING.md actually runs.
    //
    // The assertion still REGISTERS EITHER WAY, which is the property approval
    // finding 2 was about: `notApplicable` adds a row and prints its cause. It
    // is never the answer to "I could not read it" - only to "the ref is not
    // here", tested on its own.
    const hasMain = (() => {
      try {
        execFileSync('git', ['rev-parse', '--verify', '--quiet', 'refs/heads/main'],
          { cwd: ROOT, stdio: ['ignore', 'ignore', 'ignore'] });
        return true;
      } catch { return false; }
    })();
    const NAME_D4 = 'no commit on this branch carries an approval record alongside anything else (DECISIONS #4)';
    if (!hasMain) {
      gate.notApplicable(NAME_D4,
        'this checkout carries no refs/heads/main (a shallow or tag-only clone), so the merge range this rule is about does not exist here');
    } else {
      ...
      gate.check(NAME_D4, mixed.length === 0, { base: base && base.slice(0, 7), mixed: mixed.slice(0, 3) });
    }
  }
...
gate.section('report 007 figures trace to receipts');
{
  ...
  // Read through whatever markup the figure is wrapped in. Spec 020 AC-9 makes
  // every homepage figure carry data-stat="<field>", so this one is now
  // <strong>sd <span data-stat="draw_spread_sd">0.355</span></strong>. A check
  // that keys on rendered HTML structure weakens the moment the copy moves,
  // which is the lesson this loop already recorded once in DECISIONS; the
  // figure, not the tags around it, is what this assertion is about.
  const homeSd = (homeHtml.match(/draw-to-draw spread reaches <strong>sd (?:<[^>]+>)*([0-9.]+)/) || [])[1];
...
// The page's own escaper, so a quoted differ line is looked for in the form the
// page actually renders it in rather than in the form the tool emitted.
const esc008 = (x) => String(x)
  .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

// ── Report 008 figures trace to receipts (PROMOTED to the published path) ────
//
// PROMOTED 2026-09-03. The page lived at `docs/reports/008-draft/`, which
// `build-public.sh` excluded from the published tree, so no site-wide assertion
// above saw it and these assertions were the only cover it had. It now lives at
// `docs/reports/008/` and the site-wide checks reach it like any other report;
// these stay because what they assert is specific to this report and is not
// implied by any of them.
//
// THE SENTENCE THIS COMMENT USED TO END ON WAS WRONG. It said the promote-rename
// "changes no bytes and so re-checks nothing". The promotion measured otherwise:
// `build-head-tags.js` keys on `^reports\/(\d+)\/index\.html$`, a pattern the
// draft path does not match, so losing the suffix changed the title, the
// canonical URL, the og:url and the JSON-LD type in one step. The draft was
// reviewed without the structured data it now ships. Promotion re-renders, and
// the pure-function check below is what holds the re-render to its receipts.
//
// The assertions below are what Report 008 specifically claims, over and above
// the pure-function check: that its verdict is a distinct statement from its
// headline, that every band it prints says what kind of band it is, that no
// dollar on it came from the run's pre-flight projection, and that its prose
// names Report 007 the way this project spells a report name.
gate.section('report 008 figures trace to receipts (published path)');
if (SCAN_ROOT_ARG) {
  // A PUBLISHED TREE, WHERE THE PROPERTY INVERTED AT PROMOTION. While Report 008
  // was a draft this asserted the page was ABSENT here, because EXCLUDE_RE kept
  // every `*-draft/` path out and asserting it rendered would have been
  // asserting the exclusion failed. Promotion moved it to `docs/reports/008/`,
  // which EXCLUDE_RE does not match, so the correct published-tree property is
  // now the opposite: the page ships, and the draft path it came from survives
  // nowhere. Both halves are stated, because "the draft is gone" and "the report
  // is here" fail differently and a half-done promotion satisfies exactly one.
  const draftGone8 = !fs.existsSync(path.join(ROOT, 'docs', 'reports', '008-draft'))
    && !fs.existsSync(path.join(SCAN_ROOT, 'docs', 'reports', '008-draft'));
  gate.check('report 008: the draft path it was promoted from exists in no tree',
    draftGone8, { root: ROOT === SCAN_ROOT ? 'same' : 'differs' });
  ...
  const page8 = prep8.buildPage(rows8, rec8);
  const onDisk8Path = path.join(ROOT, 'docs', prep8.PAGE_REL);
  const onDisk8 = fs.existsSync(onDisk8Path) ? fs.readFileSync(onDisk8Path, 'utf8') : null;

  // 1. PURE FUNCTION. The page on disk is the page its receipts and its run
  //    record produce. Same property Report 007 carries; a page that drifted
  //    from its evidence is a page carrying numbers nothing re-derives.
  gate.check('report 008: the page on disk is byte-identical to what its receipts render (a pure function of them)',
    onDisk8 != null && onDisk8 === page8,
    { present: onDisk8 != null, bytesOnDisk: onDisk8 ? onDisk8.length : 0, bytesRendered: page8.length });

  // 2. THE SCHEMA FIELDS THE PAGE RELIES ON ARE PRESENT IN THE RECEIPTS, on both
  //    sides of both pairs. The attribution claim is the whole report, and it
  //    rests on four fields being present AND equal across a pair, not merely on
  //    the page saying they are.
  {
    const missing8 = [];
    for (const r of rows8) {
      for (const [which, rcpt] of [['new', r.receipt], ['baseline', r.baselineReceipt]]) {
        const tag = `${r.slug}/${which}`;
        // The version the runner emitted when these receipts were made, a frozen
        // fact about the archive (spec 026 AC-15 holds it unmoved); it was compared
        // to the CURRENT version, which a published receipt does not follow.
        if (String(rcpt.schema_version) !== '0.5') missing8.push(`${tag}: schema_version`);
        if (rcpt.generation_sampled !== true) missing8.push(`${tag}: generation_sampled`);
        if (rcpt.verification_level !== 'TESTED') missing8.push(`${tag}: verification_level`);
        if (!rcpt.skill || !rcpt.skill.content_hash) missing8.push(`${tag}: skill.content_hash`);
        if (!rcpt.suite || !rcpt.suite.suite_hash) missing8.push(`${tag}: suite.suite_hash`);
        if (!rcpt.receipt_hash) missing8.push(`${tag}: receipt_hash`);
        if (rcpt.run.source) missing8.push(`${tag}: carries run.source (would be a DECLARED receipt)`);
        // The economics section re-prices from per-draw usage, so every measured
        // draw must carry the three fields it reads. A single draw missing
        // cached_tokens would make a fresh-input figure quietly wrong rather
        // than absent.
        for (const c of rcpt.results.cases) {
          for (const d of ((c.generation || {}).draws || [])) {
            if (d.status !== 'measured') continue;
            if (!d.usage || typeof d.usage.input_tokens !== 'number'
              || typeof d.usage.output_tokens !== 'number'
              || typeof d.usage.cached_tokens !== 'number') missing8.push(`${tag}/${c.id}: draw usage`);
          }
        }
      }
      if (r.receipt.skill.content_hash !== r.baselineReceipt.skill.content_hash) missing8.push(`${r.slug}: content_hash differs across the pair`);
      if (r.receipt.suite.suite_hash !== r.baselineReceipt.suite.suite_hash) missing8.push(`${r.slug}: suite_hash differs across the pair`);
    }
    gate.check('report 008: every schema field the page reads is present on all four receipts, and the pinned hashes are equal across each pair',
      missing8.length === 0, { missing: missing8.slice(0, 8) });
  }

  // 3. THE VERDICT IS A DISTINCT STATEMENT FROM THE HEADLINE. Both must be on
  //    the page, and the verdict must not be a copy of the differ's headline
  //    sentence: a report whose verdict restates its headline has not concluded
  //    anything, it has repeated the tool. Asserted structurally (the verdict
  //    lives in its own block, the headline in its own) and textually (neither
  //    contains the other).
  {
    const vm8 = page8.match(/<p class="verdict"><strong>([^<]+)<\/strong>/);
    const hm8 = page8.match(/<div class="headline">([\s\S]*?)<\/div>/);
    const verdict8 = vm8 ? vm8[1].trim() : null;
    const headline8 = hm8 ? hm8[1] : '';
    const norm = (s) => String(s).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
    const vN = norm(verdict8); const hN = norm(headline8);
    const differHeadlines = rows8.map((r) => norm(r.drift.verdictLine));
    gate.check('report 008: a Verdict line exists, is distinct from the headline block, and is not a restatement of the differ headline',
      !!verdict8 && vN.length > 0 && !hN.includes(vN) && !differHeadlines.some((d) => d.includes(vN) || vN.includes(d)),
      { verdict: verdict8, verdictInHeadline: hN.includes(vN) });

    // The headline must still carry the differ's own words, VERBATIM. The
    // distinctness above is satisfiable by dropping the differ line entirely,
    // which would be the wrong repair.
    const quoted8 = rows8.filter((r) => page8.includes(esc008(r.drift.verdictLine)));
    gate.check('report 008: the headline quotes each cell\'s differ verdict line verbatim',
      quoted8.length === rows8.length, { quoted: quoted8.length, of: rows8.length });
  }

  // 4. EVERY PER-CASE BAND ON THE PAGE CARRIES ITS SOURCE LABEL. A band without
  //    its source invites a reader to compare an across-draw spread with a
  //    judge-sample spread, which is spec 017 AC-7's whole subject. Counted, not
  //    spot-checked: every `mean ± sd` on the page is either immediately
  //    followed by a source span or is an aggregate the page states is not a
  //    per-case band.
  {
    const bandRe = /([0-9]\.[0-9]{3}) ± ([0-9]\.[0-9]{3})(\s*<span class="muted">\((generation|legacy)\)<\/span>)?/g;
    let m8; const unlabelled8 = []; let labelled8 = 0;
    while ((m8 = bandRe.exec(page8)) !== null) {
      if (m8[3]) { labelled8++; continue; }
      unlabelled8.push(m8[0].trim());
    }
    // AN UNLABELLED BAND IS ADMITTED ONLY WHERE THE PAGE HAS ALREADY SAID WHAT
    // KIND OF BAND IT IS, and the allowance is derived from those declarations
    // rather than written as a list of numbers. Two sources, both narrow:
    //
    //   · the cell AGGREGATES, which are the dispersion of the seven case means
    //     across the suite and not a band over draws at all. The page states
    //     that in words beside every one of them.
    //   · the bands inside the differ's own summary line, which this page quotes
    //     VERBATIM and which carries `band = suite dispersion` in the same
    //     sentence. Taken from the quoted string itself, so the allowance cannot
    //     outlive the quote: change what the page quotes and the set changes with
    //     it.
    //
    // Anything else unlabelled is a per-case band printed bare, which is what
    // this assertion exists to refuse.
    const aggAllowed8 = new Set();
    for (const r of rows8) {
      aggAllowed8.add(`${r.comparison.delta.toFixed(3)} ± ${r.comparison.delta_uncertainty.toFixed(3)}`);
      aggAllowed8.add(`${r.aggregates.with_skill.mean_score.toFixed(3)} ± ${r.aggregates.with_skill.stddev.toFixed(3)}`);
      aggAllowed8.add(`${r.aggregates.baseline.mean_score.toFixed(3)} ± ${r.aggregates.baseline.stddev.toFixed(3)}`);
      const quotedLine8 = r.drift.summaryLine;
      if (page8.includes(esc008(quotedLine8)) && /band = suite dispersion/.test(quotedLine8)) {
        for (const q of quotedLine8.matchAll(/[0-9]\.[0-9]{3} ± [0-9]\.[0-9]{3}/g)) aggAllowed8.add(q[0]);
      }
    }
    const stray8 = unlabelled8.filter((s) => ![...aggAllowed8].some((a) => s.startsWith(a)));
    gate.check('report 008: every per-case band on the page carries its source label, and the only unlabelled bands are the aggregates the page declares as such',
      labelled8 > 0 && stray8.length === 0,
      { labelled: labelled8, unlabelled: unlabelled8.length, stray: stray8.slice(0, 6) });

    // MUTATION. A label check that cannot fail is not a check.
    const planted8 = page8.replace(' <span class="muted">(generation)</span>', '');
    let pm8; let plantedStray8 = 0;
    const re2 = new RegExp(bandRe.source, 'g');
    while ((pm8 = re2.exec(planted8)) !== null) if (!pm8[3]) plantedStray8++;
    gate.check('report 008 MUTATION: stripping one source label takes the band-label check red',
      planted8 !== page8 && plantedStray8 > unlabelled8.length,
      { before: unlabelled8.length, after: plantedStray8 });
  }

  // 5. NO PROJECTION-SOURCED DOLLAR. The run log printed a pre-flight projection
  //    ($10.19), a budget ($11.00) and a hard stop ($13.75). None is a
  //    measurement, and a projection that reaches a report page is the exact
  //    defect the run record's own comment forbids. Asserted two ways: the
  //    literals are absent, and every dollar figure on the page is one this
  //    script can re-derive from the receipts.
  {
    const projected8 = ['10.19', '13.75'];
    const hits8 = projected8.filter((v) => new RegExp(`\\$${v.replace('.', '\\.')}`).test(page8));
    const dollars8 = [...page8.matchAll(/[-+]?\$([0-9]+\.[0-9]+)/g)].map((x) => x[1]);
    const derivable8 = new Set();
    for (const r of rows8) {
      for (const f of [r.fresh, r.priorFresh]) {
        for (const v of [f.freshIncremental, f.listedIncremental, f.balancedFreshIncremental,
          f.with_skill.freshCost, f.baseline.freshCost, f.with_skill.listedCost, f.baseline.listedCost]) {
          derivable8.add(Math.abs(v).toFixed(6));
          derivable8.add(Math.abs(v).toFixed(3));
        }
      }
    }
    derivable8.add('0.00');
    const undrivable8 = dollars8.filter((d) => !derivable8.has(d));
    gate.check('report 008: no projection-sourced dollar reaches the page, and every dollar on it re-derives from the receipts',
      hits8.length === 0 && undrivable8.length === 0,
      { projectionLiterals: hits8, dollarsOnPage: dollars8.length, notDerivable: undrivable8.slice(0, 6) });
  }

  // 6. HOUSE WORDING. This project spells a report name "Report 007", not
  //    "#007"; the hash form reads as an issue number and had leaked into prose
  //    before. Asserted over the page and over the two source files this report
  //    adds, prose only.
  {
    const files8 = [
      ['docs/' + prep8.PAGE_REL, page8],
      ['scripts/prepare-report-008.js', fs.readFileSync(path.join(ROOT, 'scripts', 'prepare-report-008.js'), 'utf8')],
      ['reports/report-008-run-record.json', fs.readFileSync(path.join(ROOT, 'reports', 'report-008-run-record.json'), 'utf8')],
    ];
    const hashForm8 = files8.filter(([, t]) => /#00[0-9]\b/.test(t)).map(([f]) => f);
    gate.check('report 008: new prose writes "Report 007", never the "#007" hash form',
      hashForm8.length === 0 && /Report 007/.test(page8), { offenders: hashForm8 });

    // NO EM DASH IN THE NEW FILES, with ONE declared exception: the differ's
    // verdict line is quoted verbatim and contains one, and quoting a tool's
    // output faithfully outranks a house punctuation rule. The exception is
    // narrow by construction, because the allowance is the differ's own string
    // rather than a character class, so an em dash anywhere else still takes
    // this red.
    const emOffenders8 = [];
    for (const [rel, text] of files8) {
      let stripped = text;
      for (const r of rows8) stripped = stripped.split(esc008(r.drift.verdictLine)).join('').split(r.drift.verdictLine).join('');
      if (/—|&mdash;/.test(stripped)) emOffenders8.push(rel);
    }
    gate.check('report 008: no em dash in the files this report adds, outside the differ verdict lines it quotes verbatim',
      emOffenders8.length === 0, { offenders: emOffenders8 });
  }

  // 7. THE RUN RECORD IS A RECORD, NOT A MEASUREMENT. The page states timing,
  //    the pre-launch assertion and the timeout policy from a committed data
  //    file; that file must carry no score, band, token count or dollar, or it
  //    becomes a second, unreceipted source of figures.
  {
    const recText8 = fs.readFileSync(path.join(ROOT, prep8.RUN_RECORD), 'utf8');
    const numericLeak8 = /"(mean|score|stddev|sd|delta|band|tokens|cost|usd)[a-z_]*"\s*:/i.test(recText8);
    const zeroLost8 = rows8.every((r) => r.sampling.unmeasured === 0);
    gate.check('report 008: the run record carries no measurement field, and the zero-timeout-loss claim is the receipts\' own count',
      !numericLeak8 && zeroLost8 && rec8.timeout_policy.timeout_ms === 300000 && rec8.timeout_policy.passed_as_flag === false,
      { numericLeak: numericLeak8, unmeasured: rows8.map((r) => r.sampling.unmeasured), timeoutMs: rec8.timeout_policy.timeout_ms });
  }

  // 8. THE PRE-LAUNCH ASSERTION THE PAGE REPORTS IS THE ONE THE RECEIPTS WITNESS.
  //    The run record transcribes hashes from a log the repository does not hold.
  //    They must equal the hashes in the committed receipts, or the page is
  //    reporting an assertion nothing can check.
  {
    const bad8 = [];
    for (const c of rec8.pre_launch_assertion.cells) {
      const r = rows8.find((x) => x.slug === c.slug);
      if (!r) { bad8.push(`${c.slug}: no such cell`); continue; }
      if (r.receipt.skill.content_hash !== c.skill_content_hash) bad8.push(`${c.slug}: content_hash`);
      if (r.receipt.suite.suite_hash !== c.suite_hash) bad8.push(`${c.slug}: suite_hash`);
      if (r.baselineReceipt.skill.content_hash !== c.skill_content_hash) bad8.push(`${c.slug}: baseline content_hash`);
      if (r.receipt.suite.case_count !== c.cases) bad8.push(`${c.slug}: case_count`);
    }
...
//
// PROMOTED 2026-09-19 the way Report 008 was, by moving the renderer's constants
// and re-rendering. While it was a draft its assertions lived in spec 034's gate
// (P-8: an unpublished draft does not move this gate's figure); these are the
// ones that must hold on the published tree. The page reads files from the two
// run bundles, which are ignored and absent from a clone, so the pure-function
// row is not applicable there rather than green: an absent input is not a pass.
gate.section('report 009 (published path)');
{
  const draftGone9 = !fs.existsSync(path.join(ROOT, 'docs', 'reports', '009-draft'))
    && !fs.existsSync(path.join(ROOT, 'receipts', 'report-009-draft'))
    && !fs.existsSync(path.join(SCAN_ROOT, 'docs', 'reports', '009-draft'));
  gate.check('report 009: the draft paths it was promoted from exist in no tree', draftGone9);
  if (SCAN_ROOT_ARG) {
    const ev9 = path.join(SCAN_ROOT, 'docs', 'reports', '009', 'evidence');
    gate.check('report 009: the promoted page and its evidence ship in the published tree',
    ...
    const bundles9 = fs.existsSync(path.join(ROOT, 'specs', '000-governance', 'external-runs', 'three-skill-comparison'))
      && fs.existsSync(path.join(ROOT, 'specs', '000-governance', 'external-runs', 'three-skill-full-plugin'));
    if (bundles9) {
      const page9 = prep9.buildPage(prep9.readRows());
      gate.check('report 009: the page on disk is byte-identical to what its files render (a pure function of them)',
        onDisk9 != null && onDisk9 === page9,
        { present: onDisk9 != null, bytesOnDisk: onDisk9 ? onDisk9.length : 0, bytesRendered: page9.length });
    } else {
      gate.notApplicable('report 009: the page on disk is byte-identical to what its files render (a pure function of them)',
        'the run bundles under specs/000-governance/external-runs/ are ignored and not in this tree');
    }
    // The card, the receipt pages and the badges read receipts/report-009/; the
    // page links its evidence copies. Two copies of one receipt are one receipt
    // only while they are the same bytes.
    const rdir9 = path.join(ROOT, 'receipts', 'report-009');
    const r9 = fs.existsSync(rdir9) ? fs.readdirSync(rdir9).sort() : [];
    const pairs9 = r9.map((f) => {
      const ev = path.join(ROOT, prep9.PUB, fs.readdirSync(path.join(ROOT, prep9.PUB)).find((e) => e.endsWith(`--receipts--${f}`)) || '');
      return { f, a: fs.readFileSync(path.join(rdir9, f)), b: fs.existsSync(ev) && fs.statSync(ev).isFile() ? fs.readFileSync(ev) : null };
    });
    const same9 = (ps) => ps.length === 6 && ps.every((x) => x.b !== null && Buffer.compare(x.a, x.b) === 0);
    gate.check('report 009: each receipt under receipts/report-009/ is byte-identical to its evidence copy beside the page, three receipts and three summaries',
      same9(pairs9), pairs9.map((x) => ({ f: x.f, same: x.b !== null && Buffer.compare(x.a, x.b) === 0 })));
    const flipped9 = pairs9.map((x, i) => (i === 0 ? { ...x, a: Buffer.concat([x.a, Buffer.from(' ')]) } : x));
    gate.check('report 009 MUTATION: one receipt copy a byte longer than its evidence twin takes the copy check red',
      pairs9.length === 6 && !same9(flipped9));
    const style9 = fs.readFileSync(path.join(ROOT, 'REPORT-STYLE.md'), 'utf8');
    const labels9 = [...style9.matchAll(/^\|[^|\n]+\|\s*`([^`]+ report)`\s*\|/gm)].map((m) => m[1]);
    const type9 = ((onDisk9 || '').match(/<p class="report-type">([^<]*)/) || [])[1] || '';
    gate.check('report 009: the page is not a draft (no noindex, no DRAFT) and its eyebrow type is a label REPORT-STYLE.md lists',
      onDisk9 != null && !/name="robots" content="noindex"/.test(onDisk9) && !/DRAFT/.test(onDisk9) && labels9.includes(type9),
      { type: type9, labels: labels9.length });
  }
}

gate.section('report 010 (published path)');
{
  // Report 010 (spec 041) takes no Driftproof measurement and has no receipts: its
  // figures are the upstream harness's output files, published beside the page and
  // each named, by its path in the raw set, in raw-SHA256SUMS, which is published
  // beside them. So what must keep holding in any tree that ships is that each
  // published copy is still the bytes that list vouches for, that the page still
  // renders from its files where the raw set is present, and that no receipt
  // directory appears for a report whose page says it has none.
  const dir10 = path.join(SCAN_ROOT, 'docs', 'reports', '010');
  const ev10 = path.join(dir10, 'evidence');
  const page10 = fs.existsSync(path.join(dir10, 'index.html')) ? fs.readFileSync(path.join(dir10, 'index.html'), 'utf8') : null;
  const style10 = fs.readFileSync(path.join(ROOT, 'REPORT-STYLE.md'), 'utf8');
  const labels10 = [...style10.matchAll(/^\|[^|\n]+\|\s*`([^`]+ report)`\s*\|/gm)].map((m) => m[1]);
  const type10 = ((page10 || '').match(/<p class="report-type">([^<]*)/) || [])[1] || '';
  gate.check('report 010: the page is published (no noindex, no DRAFT) and its eyebrow type is a label REPORT-STYLE.md lists',
    page10 != null && !/name="robots" content="noindex"/.test(page10) && !/DRAFT/.test(page10) && labels10.includes(type10),
    { type: type10 });
  gate.check('report 010: no receipt directory exists for a report that publishes none',
    !fs.existsSync(path.join(SCAN_ROOT, 'receipts', 'report-010')) && /no Driftproof receipts/.test(page10 || ''));
  const sums10 = fs.existsSync(path.join(ev10, 'raw-SHA256SUMS'))
    ? new Map(fs.readFileSync(path.join(ev10, 'raw-SHA256SUMS'), 'utf8').split('\n').filter(Boolean).map((l) => [l.slice(66), l.slice(0, 64)]))
    : new Map();
  // Spec 041 A-041-4: Report 010 Amendment 1's check record (issue 591 read again) is a third twin.
  // Spec 041 A-041-5: the third read (PR 598 and the reply on issue 591) is a fourth twin.
  const TWINS10 = ['raw-SHA256SUMS', 'github-check-20260921T172411Z.json', 'github-check-20260923T040657Z.json', 'github-check-20260923T045104Z.json'];
  const copies10 = fs.existsSync(ev10) ? fs.readdirSync(ev10).filter((f) => !TWINS10.includes(f)).sort() : [];
  const vouch10 = (name, bytes) => {
    const want = sums10.get(name.split('--').join('/'));
    return want != null && crypto.createHash('sha256').update(bytes).digest('hex') === want;
  };
  const unvouched10 = copies10.filter((f) => !vouch10(f, fs.readFileSync(path.join(ev10, f))));
  gate.check('report 010: every published evidence copy is the bytes raw-SHA256SUMS lists for its path in the raw set',
    sums10.size > 0 && copies10.length > 0 && unvouched10.length === 0, { copies: copies10.length, listed: sums10.size, unvouched: unvouched10.slice(0, 10) });
  gate.check('report 010 MUTATION: an evidence copy one byte longer is not vouched for',
  ...
  if (SCAN_ROOT_ARG) {
    ...
    // A clone of the published tree runs this gate with no --scan-root and carries no specs/
    // (build-public.sh excludes it), so there is no second copy to compare: not applicable,
    // with its cause, never a pass and never a failure (spec 030 AC-5 reads every N/A's cause).
    if (!fs.existsSync(spec10)) {
      gate.notApplicable('report 010: raw-SHA256SUMS and the GitHub check record beside the page are byte-identical to spec 041\'s evidence',
        'specs/ is not in this tree (a published tree excludes it), so spec 041\'s evidence twins are absent');
    } else {
    const twinsSame10 = TWINS10.every((f) => fs.existsSync(path.join(ev10, f)) && fs.existsSync(path.join(spec10, f))
      && Buffer.compare(fs.readFileSync(path.join(ev10, f)), fs.readFileSync(path.join(spec10, f))) === 0);
    gate.check('report 010: raw-SHA256SUMS and the GitHub check record beside the page are byte-identical to spec 041\'s evidence', twinsSame10);
    }
    const prep10 = require(path.join(ROOT, 'scripts', 'prepare-report-010.js'));
    const raw10 = fs.existsSync(path.join(ROOT, prep10.RAW, 'report-010-20260921')) && fs.existsSync(path.join(ROOT, prep10.RAW, 'addy-adr-confirmation'));
    if (raw10) {
      const rendered10 = prep10.buildPage(prep10.readAll());
      gate.check('report 010: the page on disk is byte-identical to what its files render (a pure function of them)',
        page10 != null && page10 === rendered10, { bytesOnDisk: page10 ? page10.length : 0, bytesRendered: rendered10.length });
    } else {
      gate.notApplicable('report 010: the page on disk is byte-identical to what its files render (a pure function of them)',
        'the raw set under specs/000-governance/external-runs/ is ignored and not in this tree');
    }
  }
}

// ── Report 011 (PROMOTED to the published path, spec 047) ────────────────────
//
// PROMOTED 2026-09-23 the way Report 009 was (spec 039), by moving the renderer's
// constants and re-rendering. While it was a draft its assertions lived in spec 042's
// gate (spec 034 P-8, as spec 042 carried it); these are the ones that must hold on the
// published tree. The page reads its evidence copies under docs/ and spec 042's committed
// run, and one file of Report 009's run bundle, which is ignored and absent from a clone,
// so the pure-function row is not applicable there rather than green.
gate.section('report 011 (published path)');
{
  const draftGone11 = !fs.existsSync(path.join(ROOT, 'docs', 'reports', '011-draft'))
    && !fs.existsSync(path.join(ROOT, 'receipts', 'report-011-draft'))
    && !fs.existsSync(path.join(SCAN_ROOT, 'docs', 'reports', '011-draft'));
  gate.check('report 011: the draft paths it was promoted from exist in no tree', draftGone11);
  if (SCAN_ROOT_ARG) {
    const ev11 = path.join(SCAN_ROOT, 'docs', 'reports', '011', 'evidence');
    gate.check('report 011: the promoted page and its evidence ship in the published tree',
    ...
    const bundle11 = fs.existsSync(path.join(ROOT, 'specs', '000-governance', 'external-runs', 'three-skill-comparison'));
    if (bundle11) {
      const page11 = prep11.buildPage(prep11.readAll());
      gate.check('report 011: the page on disk is byte-identical to what its files render (a pure function of them)',
        onDisk11 != null && onDisk11 === page11,
        { present: onDisk11 != null, bytesOnDisk: onDisk11 ? onDisk11.length : 0, bytesRendered: page11.length });
    } else {
      gate.notApplicable('report 011: the page on disk is byte-identical to what its files render (a pure function of them)',
        "Report 009's run bundle under specs/000-governance/external-runs/ is ignored and not in this tree");
    }
    // The card, the receipt pages and the badges read receipts/report-011/; the page
    // links its evidence copies, which carry the same names. Two copies of one receipt
    // are one receipt only while they are the same bytes.
    const rdir11 = path.join(ROOT, 'receipts', 'report-011');
    const r11 = fs.existsSync(rdir11) ? fs.readdirSync(rdir11).sort() : [];
    const pairs11 = r11.map((f) => {
      const ev = path.join(ROOT, prep11.PUB, f);
      return { f, a: fs.readFileSync(path.join(rdir11, f)), b: fs.existsSync(ev) && fs.statSync(ev).isFile() ? fs.readFileSync(ev) : null };
    });
    const same11 = (ps) => ps.length === 12 && ps.every((x) => x.b !== null && Buffer.compare(x.a, x.b) === 0);
    gate.check('report 011: each file under receipts/report-011/ is byte-identical to its evidence copy beside the page, six receipts and six summaries',
      same11(pairs11), pairs11.map((x) => ({ f: x.f, same: x.b !== null && Buffer.compare(x.a, x.b) === 0 })));
    const flipped11 = pairs11.map((x, i) => (i === 0 ? { ...x, a: Buffer.concat([x.a, Buffer.from(' ')]) } : x));
    gate.check('report 011 MUTATION: one receipt copy a byte longer than its evidence twin takes the copy check red',
      pairs11.length === 12 && !same11(flipped11));
    const style11 = fs.readFileSync(path.join(ROOT, 'REPORT-STYLE.md'), 'utf8');
    const labels11 = [...style11.matchAll(/^\|[^|\n]+\|\s*`([^`]+ report)`\s*\|/gm)].map((m) => m[1]);
    const type11 = ((onDisk11 || '').match(/<p class="report-type">([^<]*)/) || [])[1] || '';
    gate.check('report 011: the page is not a draft (no noindex, no DRAFT) and its eyebrow type is a label REPORT-STYLE.md lists',
      onDisk11 != null && !/name="robots" content="noindex"/.test(onDisk11) && !/DRAFT/.test(onDisk11) && labels11.includes(type11),
      { type: type11, labels: labels11.length });
  }
}
...
// SPEC 020 A2: the default card is content-addressed, docs/og.<contenthash>.png,
// and each report page carries its own. The old constant named one filename; the
// property that replaces it is asserted per page below.
const defaultCardOf = (root) => {
  const dir = path.join(root, 'docs');
  return (fs.existsSync(dir) ? fs.readdirSync(dir).find((n) => /^og\.[0-9a-f]{8}\.png$/.test(n)) : null) || null;
};
...
gate.section('the site is distributable');
{
  ...
  for (const f of pages) {
    ...
    // SPEC 020: a report page carries its OWN card, whose filename holds a hash
    // of its bytes so a redeployed card is a new URL. The property is no longer
    // "one URL for every page" but the stronger one that replaces it: the
    // og:image is an absolute URL on this origin, naming a PNG that EXISTS in
    // this tree, at exactly the card size, under the card limit. A wrong URL and
    // a missing file are both caught now; the equality check could not do that.
    const img = metaContent(h, 'og:image');
    if (img === null) { /* the missingOg pass above already records this */ }
    else if (!img.startsWith(`${ORIGIN_URL}/`)) wrongCard.push(`${r}: ${img} is not on the origin`);
    else {
      const cf = path.join(SCAN_ROOT, 'docs', img.slice(ORIGIN_URL.length + 1));
      const cb = fs.existsSync(cf) ? fs.readFileSync(cf) : null;
      const okPng = cb && cb.length > 33 && cb.toString('hex', 0, 8) === PNG_SIGNATURE_HEX && cb.toString('ascii', 12, 16) === 'IHDR';
      if (!okPng) wrongCard.push(`${r}: ${img} is not a PNG in this tree`);
      else if (cb.readUInt32BE(16) !== 1200 || cb.readUInt32BE(20) !== 630) wrongCard.push(`${r}: ${img} is ${cb.readUInt32BE(16)}x${cb.readUInt32BE(20)}`);
      else if (cb.length > 300 * 1024) wrongCard.push(`${r}: ${img} is ${Math.round(cb.length / 1024)} KB`);
    }
    ...
    // SPEC 020 A4: every page now carries a DERIVED house title, so og:title is
    // the page's own <title> on every page rather than the <h1> on a report. The
    // h1 special case existed because report <title>s were bare ("Driftproof
    // Report #001"); that is no longer true, and one uniform rule catches the
    // staleness the special case was written for without the exception.
    const srcEl = h.match(/<title>([\s\S]*?)<\/title>/);
    const wantTitle = srcEl ? plainText(srcEl[1]) : null;
    const gotTitle = metaContent(h, 'og:title');
    if (wantTitle === null) staleTitle.push(`${r}: no <title> to derive from`);
  ...
  idcheck('dist.og-meta', 'every og:image is an absolute URL on this origin naming a 1200x630 PNG present in this tree, under the card limit', wrongCard.length === 0, { wrongCard });
  ...
  const defaultCard = defaultCardOf(SCAN_ROOT);
  idcheck('dist.og-card', 'a content-addressed default card exists: docs/og.<contenthash>.png',
    defaultCard !== null, { found: defaultCard });
  const cardPath = path.join(SCAN_ROOT, 'docs', defaultCard || 'og.png');
  const cardBuf = fs.existsSync(cardPath) ? fs.readFileSync(cardPath) : Buffer.alloc(0);
  const isPng = cardBuf.length > 33 && cardBuf.toString('hex', 0, 8) === PNG_SIGNATURE_HEX && cardBuf.toString('ascii', 12, 16) === 'IHDR';
  const cardW = isPng ? cardBuf.readUInt32BE(16) : 0;
  const cardH = isPng ? cardBuf.readUInt32BE(20) : 0;
  const cardType = isPng ? cardBuf[25] : -1;
  idcheck('dist.og-card', 'the default card exists and is a PNG', isPng, { bytes: cardBuf.length });
  idcheck('dist.og-card', 'the default card is exactly 1200x630', cardW === 1200 && cardH === 630, { width: cardW, height: cardH });
  idcheck('dist.og-card', 'the default card is opaque — no alpha channel for a platform to composite',
    cardType === 0 || cardType === 2 || cardType === 3, { colorType: cardType });
  idcheck('dist.og-card', 'the default card is under the 300 KB card limit', cardBuf.length > 0 && cardBuf.length <= 300 * 1024, { kb: Math.round(cardBuf.length / 1024) });
  ...
  // SPEC 020: a REDIRECT STUB IS NOT A PAGE. Five documentation pages moved to
  // clean paths and left a meta-refresh stub at each old address so that already
  // indexed URLs, and the links inside published report bodies, keep resolving.
  // Listing a stub asks a crawler to index a document whose only content is
  // "this moved". The set equality below is therefore over NON-STUB pages, in
  // both directions still, plus one addition that keeps the exemption honest:
  // every stub's canonical target must itself be listed, so a stub can never be
  // the place a missing page hides.
  const isRedirectStub = (h) => /<meta\s+http-equiv="refresh"/i.test(h);
  const stubs = pages.filter((f) => isRedirectStub(fs.readFileSync(f, 'utf8')));
  const derivedUrls = pages.filter((f) => !stubs.includes(f)).map((f) => pageUrlOf(SCAN_ROOT, f));
  const orphanStubs = stubs.filter((f) => {
    const h = fs.readFileSync(f, 'utf8');
    const canon = (h.match(/<link\s+rel="canonical"[^>]*href="([^"]*)"/) || [])[1];
    return !canon || !listedUrls.includes(canon);
  }).map((f) => pageRelOf(SCAN_ROOT, f));
  ...
  idcheck('dist.sitemap', 'every redirect stub names a canonical target the sitemap does list',
    orphanStubs.length === 0, { orphanStubs });
...
// ── spec 022. eval-user isolation: every local-CLI spawn takes the hop ─────────
// These run HERE, not only in specs/022's gate: `specs/` is excluded from the
// published tree and the boundary ships in lib/provider.js. OFFLINE: the spawn
// is recorded through the child_process module object in a child node process;
// no sudo, no CLI and no provider is reached. The live layers (the real hop,
// the canary runs) live in specs/022-eval-user-isolation/gate.sh.
gate.section('eval-user isolation: the hop is the default (spec 022)');
{
  const prov = require('../lib/provider');
  const canary = 'ENVCANARY-' + crypto.randomBytes(6).toString('hex');
  // Records what complete() spawns, for one call, in a fresh process so the
  // module-object stub cannot leak into the rest of this gate.
  const recordSpawn = (model, trusted) => {
    const script = `
      ...
      cp.spawn = (file, args, options) => {
        ...
        c.stdin = { write() {}, on() {}, end() { setImmediate(() => { c.stdout.end(process.env.__CANNED); c.stderr.end(); setImmediate(() => c.emit('close', 0, null)); }); } };
        return c;
      };
      const prov = require(${JSON.stringify(path.join(ROOT, 'lib', 'provider.js'))});
      prov.complete({ system: '---\\nhostile: yes\\n---\\n\\'; rm -rf / #', prompt: 'x', model: ${JSON.stringify(model)}, timeoutMs: 5000, trusted: ${trusted ? 'true' : 'false'} })
        .then(() => process.stdout.write(JSON.stringify({ calls })))
        .catch((e) => process.stdout.write(JSON.stringify({ calls, error: String(e.message) })));
    `;
    const env = { ...process.env, DRIFTPROOF_CANARY: canary, CLAUDE_PROVIDER: 'cli', OPENAI_SURFACE: 'cli', __CANNED: JSON.stringify({ type: 'result', result: 'ok', usage: { input_tokens: 1, output_tokens: 1 } }) };
    delete env.DRIFTPROOF_STUB; delete env.DRIFTPROOF_EVAL_USER; delete env['OPENAI_API' + '_KEY'];
    return JSON.parse(execFileSync(process.execPath, ['-e', script], { env, encoding: 'utf8' }));
  };
  const iso = recordSpawn('haiku', false);
  const c0 = (iso.calls || [])[0] || { args: [], env: null };
  ...
  const head = ['-n', '-u', U, '/usr/bin/env', '-i', 'HOME=/home/' + U, 'PATH=/home/' + U + '/.local/bin:/usr/bin:/bin', 'bash', '-lc', prov.ISOLATED_WRAPPER, prov.HOP_LABEL, 'claude', '-p', '--output-format', 'json', '--model', 'claude-haiku-4-5-20251001', '--append-system-prompt'];
  ...
  gate.check('the sudo front process env is {} and the env -i segment names exactly HOME and PATH',
    JSON.stringify(c0.env) === '{}' && JSON.stringify(envSeg) === '["HOME","PATH"]', { env: c0.env, envSeg });
  gate.check('a canary planted in the parent env appears nowhere in the constructed spawn',
    !JSON.stringify(iso).includes(canary));
  const tru = recordSpawn('haiku', true);
  const t0 = (tru.calls || [])[0] || { args: [], env: {} };
  gate.check('MUTATION: trusted:true spawns bare claude with the parent canary inherited (the legacy path, argv-flag-gated)',
    !tru.error && t0.file === 'claude' && t0.env && t0.env.DRIFTPROOF_CANARY === canary, { error: tru.error, file: t0.file });
  gate.check('ISOLATED_ENV_ALLOWLIST is exactly [HOME, PATH] and frozen; isolatedEnv() constructs both from the user name',
    JSON.stringify(prov.ISOLATED_ENV_ALLOWLIST) === '["HOME","PATH"]' && Object.isFrozen(prov.ISOLATED_ENV_ALLOWLIST)
    && prov.isolatedEnv('u1').HOME === '/home/u1' && prov.isolatedEnv('u1').PATH === '/home/' + 'u1' + '/.local/bin:/usr/bin:/bin');
  const savedU = process.env.DRIFTPROOF_EVAL_USER;
  let refused = 0;
  for (const bad of ['x;id', '-root', 'a b', 'Upper', '../etc']) {
    process.env.DRIFTPROOF_EVAL_USER = bad;
    try { prov.buildSpawnPlan({ bin: 'claude', args: [] }); } catch (_e) { refused += 1; }
  }
  process.env.DRIFTPROOF_EVAL_USER = 'custom-eval';
  const custom = prov.buildSpawnPlan({ bin: 'claude', args: [] });
  if (savedU === undefined) delete process.env.DRIFTPROOF_EVAL_USER; else process.env.DRIFTPROOF_EVAL_USER = savedU;
  gate.check('DRIFTPROOF_EVAL_USER: five hostile names refused before any spawn; a plain name selects the user and its home',
    refused === 5 && custom.args[2] === 'custom-eval' && custom.args.includes('HOME=/home/custom-eval'), { refused, args: custom.args.slice(0, 7) });
  // AC-9 (approval F-1): the codex final text comes from the stream. A hostile
  // spawn stub, standing in for the eval user, plants a symlink to an
  // operator-owned secret at any -o path it is handed and inside any
  // driftproof-codex-* dir that appears under the system tmp during the call,
  // then answers with a canned JSONL stream. The returned text must be the
  // stream's, the argv must carry no -o, and no read-back location may exist.
  const rbDir = fs.mkdtempSync(path.join(os.tmpdir(), 'driftproof-readback-'));
  const rbSecret = 'OPERATOR-SECRET-' + crypto.randomBytes(6).toString('hex');
  const rbSecretPath = path.join(rbDir, 'operator-secret.txt');
  fs.writeFileSync(rbSecretPath, rbSecret + '\n', { mode: 0o600 });
  const rbScript = `
    const fs = require('fs'); const os = require('os'); const path = require('path');
    const cp = require('child_process');
    const { EventEmitter } = require('events'); const { PassThrough } = require('stream');
    const secret = process.env.__RB_SECRET_PATH;
    const known = new Set(fs.readdirSync(os.tmpdir()).filter((n) => n.startsWith('driftproof-codex-')));
    const planted = []; const calls = [];
    const plant = (t) => { try { fs.rmSync(t, { force: true }); } catch (_e) {} fs.symlinkSync(secret, t); planted.push(t); };
    cp.spawn = (file, args, options) => {
      calls.push({ file, args });
      const i = args.indexOf('-o'); if (i >= 0 && args[i + 1]) plant(args[i + 1]);
      for (const n of fs.readdirSync(os.tmpdir())) { if (!n.startsWith('driftproof-codex-') || known.has(n)) continue; known.add(n); plant(path.join(os.tmpdir(), n, 'last-message.txt')); }
      const c = new EventEmitter();
      c.stdout = new PassThrough(); c.stderr = new PassThrough(); c.kill = () => true;
      c.stdin = { write() {}, on() {}, end() { setImmediate(() => { c.stdout.end(process.env.__CANNED); c.stderr.end(); setImmediate(() => c.emit('close', 0, null)); }); } };
      return c;
    ...
    const prov = require(${JSON.stringify(path.join(ROOT, 'lib', 'provider.js'))});
    prov.complete({ system: 'hostile', prompt: 'x', model: 'gpt-5.6-sol', timeoutMs: 5000 })
      .then((r) => process.stdout.write(JSON.stringify({ text: r.text, calls, planted })))
      .catch((e) => process.stdout.write(JSON.stringify({ calls, planted, error: String(e.message) })));
  `;
  const rbCanned = JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'stream answer' } }) + '\n'
    + JSON.stringify({ type: 'turn.completed', usage: { input_tokens: 1, cached_input_tokens: 0, output_tokens: 1 } }) + '\n';
  const rbEnv = { ...process.env, OPENAI_SURFACE: 'cli', CLAUDE_PROVIDER: 'cli', __RB_SECRET_PATH: rbSecretPath, __CANNED: rbCanned };
  delete rbEnv.DRIFTPROOF_STUB; delete rbEnv.DRIFTPROOF_EVAL_USER; delete rbEnv['OPENAI_API' + '_KEY'];
  let rb;
  try { rb = JSON.parse(execFileSync(process.execPath, ['-e', rbScript], { env: rbEnv, encoding: 'utf8' })); }
  finally { fs.rmSync(rbDir, { recursive: true, force: true }); }
  const rbArgv = (rb.calls || []).flatMap((c) => c.args);
  gate.check('codex lane: the argv carries no -o and the parent hands the child no read-back location (spec 022 AC-9, approval F-1)',
    !rb.error && rb.calls.length === 1 && !rbArgv.includes('-o') && !rbArgv.includes('--output-last-message') && rb.planted.length === 0, { error: rb.error, planted: rb.planted });
  gate.check('codex lane: the generation is the stream agent_message; a symlink planted at the read-back path to an operator secret never reaches it',
    rb.text === 'stream answer' && !JSON.stringify(rb).includes(rbSecret), { text: rb.text });
  const help = execFileSync(process.execPath, [path.join(ROOT, 'bin', 'driftproof')], { encoding: 'utf8' });
  gate.check('bin/driftproof usage documents --trusted-skill as self-authored-skills-only and the isolation default',
    /--trusted-skill/.test(help) && /FOR SKILLS YOU AUTHORED/.test(help) && /ISOLATION \(default\)/.test(help) && /DRIFTPROOF_EVAL_USER/.test(help));
}

// ── the instrument/documentation coupling (spec 025 A-025-14) ───────────────
//
// RE-HOMED FROM A SPEC GATE. Spec 025 AC-22 says of itself that it is "evergreen
// by design": it exists to fire the NEXT time the band computation, the differ,
// `spec/RECEIPT.md`, the receipt schema or the effect floor moves without the
// page that explains it moving too. CONSTITUTION.md, at the rule added in
// 1a335b0, says such a property belongs in the gate that runs on every branch,
// and this is the first application of that rule. The predicate is not restated
// here: it is `tests/instrument-coupling.js`, which spec 025's own gate also asks.
//
// IN FORCE FROM a recorded commit rather than from the beginning of history.
// The rule is per-commit and commits are immutable, so a violation inside a
// range anchored at a fixed base can never be answered and the check would be
// red forever after its first real hit. Anchored here, it measures new commits,
// fires on them, and returns to green once one is answered. The anchor was first the
// merge that carried spec 026 — the merge whose own commits AC-22 caught, and
// which is why this check moved. Commits at or before it are out of scope and
// are recorded in DECISIONS instead.
//
// MOVED PAST c5c45bb, A RECORDED VIOLATION (DECISIONS, 2026-09-14). c5c45bb, spec
// 031's defect 3 fix, changed lib/diff.js with no documentation source. The rule
// is per commit, so the documentation commit that followed it, ed530d3, answers
// what the page says and does not clear c5c45bb. The anchor is now c5c45bb
// itself: every commit after it is measured, and c5c45bb is recorded in
// DECISIONS as a violation of this rule, not exempted from it.
//
// A TREE WITHOUT THE ANCHOR measures nothing: the published tree is a fresh
// git-init with one commit, so the anchor is absent there and the real-commit
// half is vacuous. That is said out loud in the printed line and in the detail
// rather than left to be inferred, and the two planted halves below — which need
// no history — run identically in both trees, so the predicate is exercised
// wherever this gate runs.
gate.section('instrument/documentation coupling (spec 025 A-025-14, in force from the 026 merge, anchor moved past c5c45bb)');
{
  const { docsLag } = require('./instrument-coupling');
  const IN_FORCE_FROM = 'c5c45bbd7459b54f10ec92b969b436a83c2e547e';
  const g = (...a) => execFileSync('git', a, { cwd: ROOT, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
  let anchorPresent = false;
  try { g('cat-file', '-e', `${IN_FORCE_FROM}^{commit}`); anchorPresent = true; } catch (_e) { anchorPresent = false; }
  let commits = [];
  if (anchorPresent) {
    const shas = g('rev-list', `${IN_FORCE_FROM}..HEAD`).trim().split('\n').filter(Boolean);
    commits = shas.map((sha) => {
      const files = g('show', '--name-only', '--pretty=format:', sha).split('\n').map((x) => x.trim()).filter(Boolean);
      const diffs = {};
      if (files.includes('config.js')) diffs['config.js'] = g('show', '--pretty=format:', sha, '--', 'config.js');
      return { sha, files, diffs };
    });
  }
  const offenders = docsLag(commits);
  // eslint-disable-next-line no-console
  console.log(`  [coupling] in force from ${IN_FORCE_FROM.slice(0, 7)}: ${anchorPresent ? `${commits.length} commit(s) measured` : 'anchor absent in this repository, 0 commits measured'}`);
  gate.check('no commit since the coupling came into force moves an instrument file without a documentation source',
    offenders.length === 0,
    { in_force_from: IN_FORCE_FROM, anchor_present: anchorPresent, measured: commits.length, offenders });
  // A guard nobody has seen fire is a guard nobody should believe. Both halves
  // are planted commit lists, so they prove the predicate on every tree, and the
  // control is what stops the mutation passing for the wrong reason.
  gate.check('MUTATION: a commit that moves an instrument file and no documentation source is named',
    docsLag([{ sha: 'a'.repeat(40), files: ['lib/diff.js'] }]).length === 1);
  gate.check('CONTROL: the same commit, also moving the methodology page, is not named',
    docsLag([{ sha: 'a'.repeat(40), files: ['lib/diff.js', 'docs/methodology/index.html'] }]).length === 0);
}

// ── the Claude Code plugin self-test (spec 028 AC-13) ────────────────────────
//
// Evergreen, so it lives here and not only in the spec gate: the spec gate
// measures a closed range and stops when the loop does, while the properties
// below must hold on every branch for as long as the plugin ships. It is the
// bounded subset — the door refuses before it spawns, the shipped contract is
// still the Action's, and a rendered badge says what it did not check.
//
// It SPENDS NOTHING: no network, no provider, no API key, no `claude` binary.
// Every invocation goes through spec 028's fake npx to this checkout's own
// bin/driftproof, and its one run is under DRIFTPROOF_STUB=1.
// The steps of .github/workflows/plugin-selftest.yml as they stood at bc78063, verbatim,
// before A-028-34: MUTATION 2 below plants them. A fixture, because a published tree
// carries no history to read them from.
const OLD_PLUGIN_SELFTEST_STEPS = "    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version-file: .nvmrc\n      - name: install (the runner's own deps; the plugin has none)\n        run: npm ci --omit=dev\n      - name: the plugin self-test\n        run: node specs/028-claude-code-plugin/probes/selftest.mjs\n      - name: the manifests validate as manifests\n        run: |\n          node -e \"JSON.parse(require('fs').readFileSync('.claude-plugin/marketplace.json','utf8'))\"\n          node -e \"JSON.parse(require('fs').readFileSync('plugin/driftproof/.claude-plugin/plugin.json','utf8'))\"\n";
gate.section('claude code plugin self-test (028)');
{
  const PLUGIN = path.join(ROOT, 'plugin', 'driftproof');
  const selftest = path.join(ROOT, 'specs', '028-claude-code-plugin', 'probes', 'selftest.mjs');
  const specsPresent = fs.existsSync(path.join(ROOT, 'specs'));

  // ── what holds in BOTH trees ───────────────────────────────────────────────
  // The plugin ships to the published tree; the spec probes that drive it do
  // not, because build-public.sh excludes specs/. So the shape checks are here,
  // reading only files that exist in both, and the probe-driven half is gated on
  // the probes being present — with the ABSENCE itself asserted rather than
  // skipped, so a deleted self-test cannot look like a published tree.
  ...
  gate.check('the plugin manifest parses, names driftproof, and pins a semver runner',
  ...
  //
  // Every rule that names a `bash_name` is the Action's and is held against it.
  // A rule that names none is the DOOR'S OWN — spec 028 A-028-13 added a
  // shell-metacharacter rule for skill-dir and receipt, which action/lib.sh does
  // not carry and will not, because widening the Action would change its
  // accepted inputs. Held instead between the shipped contract and spec 028's
  // fixture, in both directions, by that spec's NFR-4.
  //
  // The skip is bounded so it cannot empty this check: the count of rules held
  // against action/lib.sh is compared to the RE_* it declares for the four
  // inputs. If a rule stopped declaring a bash_name this goes red rather than
  // quietly comparing nothing. (Before this, the loop read `undefined` as a
  // regex name and reported "undefined is not assigned in action/lib.sh" —
  // which is a true sentence about a rule that was never the Action's.)
  const contractDrift = [];
  try {
    const lib = fs.readFileSync(path.join(ROOT, 'action', 'lib.sh'), 'utf8');
    const shipped = JSON.parse(fs.readFileSync(path.join(PLUGIN, 'input-contract.json'), 'utf8'));
    let heldAgainstAction = 0;
    for (const [name, rule] of Object.entries(shipped.rules || {})) {
      if (!rule.bash_name) continue;
      heldAgainstAction += 1;
      const m = new RegExp(`^${rule.bash_name}='(.*)'$`, 'm').exec(lib);
      if (!m) contractDrift.push(`${rule.bash_name} is not assigned in action/lib.sh`);
      else if (m[1] !== rule.re) contractDrift.push(`${name} differs from action/lib.sh`);
    }
    const govern = [...lib.matchAll(/^(RE_[A-Z_]+)=/gm)].map((m) => m[1]).filter((n) => !['RE_VERDICT', 'RE_DELTA'].includes(n));
    if (heldAgainstAction !== govern.length) contractDrift.push(`${heldAgainstAction} rule(s) held against action/lib.sh, which declares ${govern.length} governing the inputs (${govern.join(', ')})`);
  } catch (e) { contractDrift.push(String(e.message)); }
  gate.check("the plugin's shipped input contract is byte-identical to action/lib.sh",
    contractDrift.length === 0, { drift: contractDrift });

  // ── the probe-driven half ──────────────────────────────────────────────────
  if (fs.existsSync(selftest)) {
    const r = spawnSync('node', [selftest], { cwd: ROOT, encoding: 'utf8', timeout: 10 * 60 * 1000 });
    gate.check('the plugin self-test passes', r.status === 0, {
      status: r.status,
      tail: String(`${r.stdout || ''}${r.stderr || ''}`).trim().split('\n').slice(-6).join('\n'),
    });
  } else if (specsPresent) {
    // A self-test deleted from a tree that carries specs/ fails here instead of
    // quietly not running (absence-vs-unreadable). Its ABSENCE IS NO LONGER A PASS
    // anywhere (spec 028 A-028-34): until 2026-09-13 this branch recorded "the
    // plugin self-test is absent only because this tree carries no specs/" as a
    // pass in the published tree, and it read PASS over a build whose shipped
    // workflow ran exactly that missing file. What a published tree is held to
    // now is the assertion below: every file its workflows run is in it.
    gate.check('the plugin self-test is present in a tree that carries specs/', false, { specs_present: specsPresent, expected: selftest });
  }

  // ── what a shipped workflow runs is in the tree that ships it (A-028-34) ────
  //
  // Over SCAN_ROOT, which is the built output under --scan-root and this tree
  // otherwise, read from disk and never from EXCLUDE_RE. Any problem fails:
  // a missing subject, no workflow read, or workflows read with no subject found.
  const wfScope = require('./assertion-scope');
  const wfReal = wfScope.workflowRunSubjects(SCAN_ROOT);
  gate.check("every file a shipped workflow's steps run is in this tree, and at least one was found",
    wfReal.problems.length === 0,
    { problems: wfReal.problems, workflows: wfReal.workflows, subjects: wfReal.subjects.length });

  // MUTATION 1: the tree's own workflow, plus one planted step naming a path this
  // tree does not carry. It must be named, or the check is not reading the steps.
  {
    const planted = 'tools/a-path-no-published-tree-carries.mjs';
    const wfDir = path.join(SCAN_ROOT, '.github', 'workflows');
    const target = wfReal.workflows.find((w) => /plugin/i.test(w)) || wfReal.workflows[0];
    const r = target ? wfScope.workflowRunSubjects(SCAN_ROOT, {
      readFile: (f) => {
        const t = fs.readFileSync(f, 'utf8');
        return path.join(SCAN_ROOT, target) === f ? `${t.replace(/\n*$/, '\n')}      - name: planted\n        run: node ${planted}\n` : t;
      },
    }) : { problems: [] };
    gate.check('MUTATION: a workflow step planted to run a path this tree lacks is named by the check',
      !!target && r.problems.some((x) => x.includes(planted)) && fs.existsSync(wfDir),
      { target, problems: r.problems });
  }

  // MUTATION 2: the step as it stood at bc78063, against a planted published tree:
  // the files the current workflow runs, and no specs/. It must name the missing
  // self-test. A CONTROL with today's step on the same tree must stay clean, or
  // the arm would pass on a tree that fails everything.
  {
    const tree = new Set(['tests/gate.js', '.claude-plugin/marketplace.json', 'plugin/driftproof/.claude-plugin/plugin.json', 'package.json']);
    const head = 'name: plugin self-test\non:\n  push:\njobs:\n  selftest:\n    runs-on: ubuntu-latest\n';
    const atBc78063 = head + OLD_PLUGIN_SELFTEST_STEPS;
    const today = atBc78063.replace('run: node specs/028-claude-code-plugin/probes/selftest.mjs', 'run: node tests/gate.js --scan-root .');
    const scan = (text) => wfScope.workflowRunSubjects('/planted', {
      readDir: () => ['plugin-selftest.yml'], readFile: () => text, exists: (f) => tree.has(path.relative('/planted', f)),
    });
    const red = scan(atBc78063);
    const control = scan(today);
    gate.check('MUTATION: the workflow as it stood at bc78063 is red against a published tree with no specs/, and today\'s step on the same tree is clean',
      red.problems.some((x) => x.includes('specs/028-claude-code-plugin/probes/selftest.mjs')) && control.problems.length === 0 && control.subjects.length > 0,
      { red: red.problems, control: control.problems });
  }

  // MUTATION 3: a tree with no workflow at all is a problem, not a pass.
  {
...
// ── case identity and receipt selection (spec 050) ───────────────────────────
//
// Two false-pass paths an outside correctness audit found in 0.11.2, held here
// because both are properties of the tree as it stands, not of one loop's range.
// (1) Two cases with one normalized id were both measured, and the verdict kept
// whichever row came last, so a measured regression could read PASSED. (2) Every
// Action invocation wrote to one fixed directory and the decision took the first
// matching receipt, so a second step could be decided by the first step's skill.
// The spec gate runs the audit's two reproductions end to end; these are the
// in-process properties, each beside the planted input that must trip it.
gate.section('case identity and receipt selection (spec 050)');
{
  const { duplicateCaseRows } = require('../lib/receipt');
  const { receiptVerdict } = require('../lib/verdict');
  const { decisionState, decideSet } = require('../lib/decision');
  const suiteErr = (cases) => { try { normalizeCases({ cases }); return null; } catch (e) { return e; } };
  const c = (id, prompt) => ({ ...(id ? { id } : {}), prompt, rubric: 'r' });
  const dupErr = suiteErr([c('same', 'a'), c('same', 'b')]);
  const implicitErr = suiteErr([c('case-2', 'a'), c(null, 'b')]);
  gate.check('the loader refuses two cases with one normalized id, an explicit id against an assigned case-<n> included, and accepts distinct ids',
    !!(dupErr && dupErr.code === 'DUPLICATE_CASE_ID' && implicitErr && implicitErr.code === 'DUPLICATE_CASE_ID' && suiteErr([c('same', 'a'), c('different', 'b')]) === null),
    { duplicate: dupErr && dupErr.message, implicit: implicitErr && implicitErr.message });

  const rows = [mkcase('same', 'with_skill', [0, 0, 0], 0.7), mkcase('same', 'baseline', [1, 1, 1], 0.7)];
  const unique = synthReceipt([...rows, mkcase('different', 'with_skill', [0.6, 0.6, 0.6], 0.7), mkcase('different', 'baseline', [0.4, 0.4, 0.4], 0.7)]);
  const dup = synthReceipt([...rows, mkcase('same', 'with_skill', [0.6, 0.6, 0.6], 0.7), mkcase('same', 'baseline', [0.4, 0.4, 0.4], 0.7)]);
  const vd = validateReceipt(dup);
  gate.check('a receipt with two rows for one (id, mode) fails validation, naming the row; the same receipt with distinct ids validates',
    vd.valid === false && vd.errors.some((e) => e.instancePath === '/results/cases/2' && /duplicate case id and mode/.test(e.message)) && validateReceipt(unique).valid === true,
    { errors: vd.errors.slice(0, 3), unique_valid: validateReceipt(unique).valid });
  let thrown = null; try { receiptVerdict(dup); } catch (e) { thrown = e; }
  let uniqueThrew = null; try { receiptVerdict(unique); } catch (e) { uniqueThrew = e.message; }
  gate.check('receiptVerdict refuses an ambiguous receipt with AMBIGUOUS_RECEIPT and decisionState decides it refused; the distinct-id receipt is verdicted',
    !!(thrown && thrown.code === 'AMBIGUOUS_RECEIPT') && decisionState(dup) === 'refused' && uniqueThrew === null && decisionState(unique) !== 'refused' && duplicateCaseRows(unique).length === 0,
    { thrown: thrown && thrown.message, state: decisionState(dup), unique_threw: uniqueThrew });

  const setDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gate-050-set-'));
  try {
    const sealed = (skill, delta) => { const r = synthReceipt([...rows]); r.skill.name = skill; r.comparison.delta = delta; return r; };
    fs.writeFileSync(path.join(setDir, 'aaa-claude-haiku-4-5-2026-07-27.json'), JSON.stringify(sealed('aaa', 0.8)));
    const one = decideSet(setDir, 'claude-haiku-4-5');
    fs.writeFileSync(path.join(setDir, 'zzz-claude-haiku-4-5-2026-07-27.json'), JSON.stringify(sealed('zzz', -0.8)));
    const two = decideSet(setDir, 'claude-haiku-4-5');
    gate.check('decideSet refuses two receipts for one requested model, naming both and taking no delta; one receipt is decided as before',
      one.rows[0].state !== 'refused' && two.rows[0].state === 'refused' && two.rows[0].delta === null && two.fails === true
        && /aaa-claude-haiku-4-5/.test(two.rows[0].reason) && /zzz-claude-haiku-4-5/.test(two.rows[0].reason) && two.ambiguous.length === 1 && two.absent.length === 0,
      { one: one.rows[0].state, two: two.rows[0] });
  } finally { fs.rmSync(setDir, { recursive: true, force: true }); }

  // The Action's run step, twice in one RUNNER_TEMP, as two steps of one job run it:
  // on the stub surface, so no model is called.
  const rt = fs.mkdtempSync(path.join(os.tmpdir(), 'gate-050-rt-'));
  try {
    const outs = [];
    for (const n of [1, 2]) {
      const gho = path.join(rt, `gho-${n}`); fs.writeFileSync(gho, '');
      const r = spawnSync('bash', [path.join(ROOT, 'action', 'run.sh')], {
        cwd: ROOT, encoding: 'utf8', timeout: 120000,
        env: { PATH: process.env.PATH, HOME: rt, TMPDIR: os.tmpdir(), DRIFTPROOF_STUB: '1', GITHUB_ACTION_PATH: ROOT, RUNNER_TEMP: rt, GITHUB_OUTPUT: gho,
          INPUT_SKILL_DIR: 'examples/commit-message-conventions', INPUT_MODELS: 'claude-haiku-4-5', INPUT_MAX_USD: '3.53', INPUT_MAX_CALLS: '1200', INPUT_FAIL_ON_REGRESSION: 'true' },
      });
      const text = fs.readFileSync(gho, 'utf8');
      const field = (k) => ((new RegExp(`^${k}<<(\\S+)\\n([^\\n]*)\\n\\1$`, 'm').exec(text) || [])[2] || null);
      outs.push({ exit: r.status, dir: field('receipts_dir'), name: field('artifact_name'), stderr: (r.stderr || '').slice(-200) });
    }
    const [a, b] = outs;
    gate.check('two Action run steps sharing RUNNER_TEMP write to two directories under it, each named by its own artifact_name',
      a.exit === 0 && b.exit === 0 && !!a.dir && !!b.dir && a.dir !== b.dir && path.dirname(a.dir) === rt && path.dirname(b.dir) === rt
        && a.name === path.basename(a.dir) && b.name === path.basename(b.dir),
      { outs: outs.map((o) => ({ ...o, dir: o.dir && path.basename(o.dir) })) });
...
fs.writeFileSync(path.join(__dirname, 'gate-results.json'),
  JSON.stringify({
    ...summary,
    executed_assertion_ids: [...new Set(EXECUTED_ASSERTION_IDS)],
    // The classification of every file the confidentiality scan walked: counts
    // by class, the full list of files that failed to be read for any reason
    // (expected empty), and one entry per binary saying what its format-specific
    // check returned. Emitted by the RUN rather than maintained beside it, so an
    // assertion about the classification reads what happened (spec 027 AC-14).
