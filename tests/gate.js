...
gate.section('github action');
{
  ...
  try {
    ...
    // delimiter, and this now asserts that block.
    gate.check('badge --github-output emits verdict as a heredoc block (verdict<<ghadelim_<hex> / PASSED / delimiter)',
  ...
  //    the page saying they are.
  {
    ...
    for (const r of rows8) {
      for (const [which, rcpt] of [['new', r.receipt], ['baseline', r.baselineReceipt]]) {
        ...
        if (String(rcpt.schema_version) !== String(RECEIPT_SCHEMA_VERSION)) missing8.push(`${tag}: schema_version`);
...
gate.section('the site is distributable');
{
  ...
  for (const f of pages) {
    ...
    if (img === null) { /* the missingOg pass above already records this */ }
      ...
      const okPng = cb && cb.length > 33 && cb.toString('hex', 0, 8) === '89504e470d0a1a0a' && cb.toString('ascii', 12, 16) === 'IHDR';
