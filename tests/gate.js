...
gate.section('spec 023 action input hardening');
{
  ...
  const outMissing = [['DRIFTPROOF_VERDICT', 'verdict'], ['DRIFTPROOF_DELTA', 'delta']].filter(([k, v]) => !new RegExp(`^\\s+${k}: \\$\\{\\{ steps\\.run\\.outputs\\.${v} \\}\\}$`, 'm').test(actionText)).map(([k]) => k);
  const escaped = actionText.split('\n').filter((l) => /\$\{\{ (inputs|steps\.run\.outputs)\./.test(l) && !/^\s+[A-Z][A-Z0-9_]*: \$\{\{ [a-z.-]+ \}\}$/.test(l) && !/^\s+value: \$\{\{ steps\.run\.outputs\.(verdict|delta) \}\}$/.test(l));
  gate.check('AC-1: every input is an env: INPUT_<NAME> entry and both step outputs are env: entries; no expression escapes env:/value:', envMissing.length === 0 && outMissing.length === 0 && escaped.length === 0, { envMissing, outMissing, escaped });
