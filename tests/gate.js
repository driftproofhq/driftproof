...
gate.section('publish-tree draft guard');
{
  ...
  const EX_ROWS = {
    ...
    governance: 'published tree excludes BACKLOG.md, CONSTITUTION.md, DECISIONS.md and specs/',
  ...
  function exclusionRows(gate, root, scanRootArg, modeOpts = {}) {
    ...
    // enumeration should stop being an enumeration. Publishing a curated public constitution stays open as a
...
gate.section('spec 023 action input hardening');
{
  ...
  const escaped = actionText.split('\n').filter((l) => /\$\{\{ (inputs|steps\.run\.outputs)\./.test(l) && !/^\s+[A-Z][A-Z0-9_]*: \$\{\{ [a-z0-9._-]+ \}\}$/.test(l) && !/^\s+value: \$\{\{ steps\.run\.outputs\.[a-z0-9_]+ \}\}$/.test(l));
  gate.check('AC-1: every input is an env: INPUT_<NAME> entry and all three step outputs are env: entries; no expression escapes env:/value:', envMissing.length === 0 && outMissing.length === 0 && escaped.length === 0, { envMissing, outMissing, escaped });
...
gate.section('spec 024 runner hostile-input proof');
{
  ...
  const fixedName = /^\s+name: [A-Za-z0-9._-]+$/m.test(upBlock) && !/\$\{\{/.test((upBlock.match(/^\s+name: .*$/m) || [''])[0]);
  ...
  gate.check('AC-4: action.yml uploads under one fixed artifact name with no overwrite, and exactly one uses: ./ step of the self-test can run to success (the benign step)',
    fixedName && noOverwrite && canSucceed.length === 1 && /id: driftproof/.test(canSucceed[0]), { fixedName, noOverwrite, canSucceed: canSucceed.map((b) => (b.match(/id: (\S+)/) || [])[1]) });
