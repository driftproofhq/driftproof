...
gate.section('schema v0.4 + round-trip');
{
  ...
  gate.check('sampled receipt validates against schema v0.4', v.valid && v.version === '0.4', v.errors);
  gate.check('receipt stamps schema_version 0.4 + run.provider + run.registry + run.transcripts',
    r.schema_version === '0.4' && !!r.run.provider && !!r.run.registry && !!r.run.transcripts, { sv: r.schema_version, prov: r.run.provider, reg: r.run.registry, tr: r.run.transcripts });
...
gate.section('publish-tree draft guard');
{
  ...
  if (process.env.DRIFTPROOF_SCAN_ROOT) {
    ...
    //
    // This is NOT a blanket relaxation. A draft PAGE, or any other draft-shaped
    // path, still fails here: only receipt JSON may be tracked in a -draft
    // directory, so a draft report can never become tracked by accident and the
    // publish-side check above is unchanged.
    const allowed = /^receipts\/report-[0-9]+-draft\/[^/]+\.json$/;
    const bad = draftHits.filter((f) => !allowed.test(f));
    gate.check('only receipt JSON may be tracked under a *-draft/ path on the dev tree', bad.length === 0, { bad: bad.slice(0, 20) });
  }
  ...
  if (manifest && manifest.skills) {
    ...
    for (const rep of comparable) {
      ...
      for (const f of fs.readdirSync(rdir).filter((x) => x.endsWith('.json') && !x.includes('_index'))) {
        ...
        const slug = (j.skill && j.skill.slug) || f.split('__')[0];
...
gate.section('provider/surface receipts');
{
  ...
  try {
    ...
    const r = JSON.parse(fs.readFileSync(path.join(outDir, f), 'utf8'));
    const v = validateReceipt(r);
    gate.check('openai/cli stub receipt validates v0.4', v.valid && v.version === '0.4', v.errors);
...
gate.section('timeout tolerance / failed_timeout');
{
  ...
  gate.check('incomplete receipt (failed_timeout case) validates v0.4 + self-verifies', validateReceipt(inc).valid && verifyReceiptHash(inc));
...
gate.section('interop import (honest epistemics)');
{
  ...
  gate.check('agent-skills-eval import → schema-valid v0.4 + self-hash verifies', vAse.valid && vAse.version === '0.4' && verifyReceiptHash(ase), vAse.errors);
...
gate.section('report-005 staging (value report, stub)');
{
  ...
  gate.check('report-005 projection is under the $40 report guard', out.overGuard === false && out.projection > 0, { usd: out.projection });
  ...
  for (const f of recFiles) {
    ...
    const v = validateReceipt(r);
    if (!(v.valid && v.version === '0.4' && verifyReceiptHash(r))) allV4 = false;
  ...
  gate.check('every report-005 receipt validates v0.4 + self-verifies', allV4);
