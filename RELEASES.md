...
To follow, after the publish.

### Known open

- **Spec 028's NFR-7 reads red until this version is on npm.** It compares `lib/` against the
  tarball the plugin pin names, and `lib/hygiene.js` is ahead of 0.11.0 (A-028-43). It clears at
  the publish, and is re-run after it, never before.
- **The re-freezes the bump forces** (BACKLOG.md § 2 step 4) follow the sibling sweep at the
  release commit.
