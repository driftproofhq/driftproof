...
const REPORT_VERSION = 'v1.2';
const REPORT_VERSION_DATE = '2026-07-30';

// THE BAND-WORDING AMENDMENT (spec 031 A-031-20, fix loop B). A published record
// is amended with a versioned entry and never silently edited: the entry quotes
// the wording it corrects, and the page keeps that wording where it was published.
const WORDING_VERSION_NOTE_MD = `> **v1.3** (2026-09-14): the entry below corrects how this report's band wording is read.`;
const WORDING_VERSION_NOTE_HTML = `<p class="version-note"><strong>v1.3</strong> (2026-09-14): the entry below corrects how this report&rsquo;s band wording is read.</p>`;
const WORDING_AMENDMENT_MD = `**v1.3** · 2026-09-14. This entry corrects how the report's band wording is read. It follows the entries recorded before it, no earlier text in the report is changed, and no figure, verdict token, table value or receipt reference changes.

- **The headline.** This report says *"9 of 10 skills moved beyond noise on this model pair"*, glossed *(per-case bands do not overlap)*. Each of those nine skills has at least one case whose two bands do not overlap and whose mean moved by at least the 0.05 effect floor: a separation detected under the rule, which is not proof that the skill's behaviour moved. The gloss leaves out the floor, and no skill's aggregate with_skill band separated. As amended: **9 of 10 skills showed a separation detected under the rule** on this model pair.
- **How to read this report.** A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. The four rows marked *within noise (below floor)* are of that kind. Wherever the report reads a result that was not separated as settled, as *within noise*, *never as drift*, *nothing moved*, *no drift on this case*, *hair-trigger*, a regression that *did not reproduce*, or a cause confirmed because a later run was not separated, as the entry of 2026-07-30 does of the imported criterion and the entry of 2026-07-28 of the artifact, read it as no separation detected at the sample size used, which is not evidence of equivalence, not evidence that nothing changed, and not proof of the cause. Wherever it reads a separation as settled, as *a real regression and a real improvement*, *real regressions surfaced*, *reconfirmed as a genuine new-model failure*, a driver that *held*, or a regression claimed because two bands are *fully separated*, read it as a separation detected under the rule, which is not proof that the model's behaviour moved; the Methodology's condition leaves out the floor, which every verdict here applies. Labels, tallies and headings are read the same way and are left as published.
...
const WORDING_AMENDMENT_HTML = `<div class="amendment" id="amendment-wording">
    <p><strong>v1.3</strong> &middot; 2026-09-14. This entry corrects how the report&rsquo;s band wording is read. It follows the entries recorded before it, no earlier text in the report is changed, and no figure, verdict token, table value or receipt reference changes.</p>
    <ul>
      <li><strong>The headline.</strong> This page says <em>&ldquo;9 of 10 skills moved beyond noise on this model pair.&rdquo;</em> Each of those nine skills has at least one case whose two bands do not overlap and whose mean moved by at least the 0.05 effect floor: a separation detected under the rule, which is not proof that the skill&rsquo;s behaviour moved. No skill&rsquo;s aggregate with_skill band separated. As amended:</li>
    </ul>
    <p class="amended-headline"><strong>9 of 10 skills showed a separation detected under the rule</strong> on this model pair.</p>
    <ul>
      <li><strong>How to read this page.</strong> A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. The four rows marked <em>within noise (below floor)</em> are of that kind, and what the headline rule calls bands that <em>separate by less than the floor</em> are bands that do not overlap. Wherever the page reads a result that was not separated as settled, as <em>within noise</em>, <em>never as drift</em>, <em>no drift on this case</em>, <em>hair-trigger</em>, a regression that <em>did not reproduce</em>, or a cause confirmed because a later run was not separated, as the entry of 2026-07-30 does of the imported criterion and the entry of 2026-07-28 of the artifact, read it as no separation detected at the sample size used, which is not evidence of equivalence, not evidence that nothing changed, and not proof of the cause. Wherever it reads a separation as settled, as <em>a real regression and a real improvement</em>, <em>real regressions surfaced</em>, <em>reconfirmed as a genuine new-model failure</em>, or a driver that <em>held</em>, read it as a separation detected under the rule, which is not proof that the model&rsquo;s behaviour moved. The Methodology&rsquo;s <em>no meaningful change</em> is a hypothetical that explains the floor, and the footer&rsquo;s <em>Verdicts age because the substrate moves</em> names a cause that no separation here proves. Labels, tallies and headings, the summary card&rsquo;s <em>What moved</em> among them, are read the same way and are left as published.</li>
      <li><strong>The bands.</strong> Each band on this page is a mean plus or minus one sample standard deviation, of two kinds: a case&rsquo;s band, over its five judge samples, and a skill&rsquo;s with_skill and baseline bands, over its seven per-case means, which is suite dispersion. Each band is a descriptive spread, not a confidence interval, with no coverage probability.</li>
      <li><strong>Where the amended headline appears.</strong> The site&rsquo;s report index and feed quote the amended headline; this page&rsquo;s summary card, description and social card image keep the headline as published. Filed under the wording rules of the repository's spec 031, amendment A-031-20.</li>
    </ul>
  </div>`;
...
const AMENDMENTS = [
  {
    version: 'v1.2', date: '2026-07-30',
    summary: 'In-text grounding policy adopted and applied across all 10 suites. Headline: v1.1 read 3 regressed / 3 improved / 3 mixed / 1 within noise; v1.2 reads 2 regressed / 3 improved / 4 mixed / 1 within noise. This supersedes v1.1.',
    points: [
      '<strong>Policy adopted (codified, applied unilaterally).</strong> <em>Every gradable rubric criterion must trace to text present in the skill\'s SKILL.md at the pinned SHA. Criteria that import domain knowledge the skill does not state are not gradable, regardless of how standard that knowledge is. Claims may summarize; rubrics may not extrapolate.</em> The grading standard does not depend on the audited party\'s tolerance. Full sweep of all 10 suites (70 cases): <code>reports/rubric-sweep.md</code>.',
      '<strong>Criteria amended (2 of 10 suites; the other 8 were grounded and unchanged).</strong> <code>git-workflow-and-versioning/semver-clean-bump</code> required "the highest-precedence bump governs when changes are combined / reset PATCH to 0" — a rule the SKILL.md never states (it defines only MAJOR/MINOR/PATCH). That criterion was removed; the case now grades only the stated MAJOR/MINOR/PATCH definitions. <code>commit-work</code> (all 7 cases) required a <em>specific</em> Conventional Commits type token (e.g. <code>chore(deps)</code> vs <code>feat</code>) and, in one case, a subject &lt;=72-chars / no-trailing-period / imperative-mood rule — none of which its SKILL.md states (it names Conventional Commits and shows the <code>type(scope): summary</code> shape but does not enumerate the type vocabulary). Type grading is now shape-only; the length/period/mood rule was removed. The logical-split, patch-staging, and secret/debug checks the SKILL.md <em>does</em> state remain fully graded.',
      '<strong>Re-run.</strong> Only the two amended suites were re-run (both models, n=5, claude-cli, transcripts retained); the amendment changes only those two <code>suite_hash</code>es. The <code>grounding</code> reference now recorded on every case is excluded from <code>suite_hash</code>, so annotating the other eight suites did not disturb their receipts.',
      '<strong>Verdict changes (stated plainly, not softened).</strong> The amended case <code>git-workflow-and-versioning/semver-clean-bump</code> flips <strong>regression → within noise</strong> (v1.1 Δ−0.054 on separated bands; v1.2 Δ−0.004). The v1.1 "regression" was driven by the imported precedence/reset criterion; grading only the MAJOR/MINOR/PATCH definitions the SKILL.md actually states, the new model shows no drift on this case. But re-running a suite re-rolls all seven of its cases (one receipt covers the whole suite), and the fresh n=5 measurement surfaced, on two <em>unamended</em>, well-grounded cases, a real regression and a real improvement: <code>commit-message-conventional-type</code> now regresses (Δ−0.256 — the new model wrote <code>feat:</code> instead of <code>fix:</code> for a bug fix; all five judge samples scored 0.60 with that same reason, transcript-verified) and <code>semver-hidden-breaking-change</code> now improves (Δ+0.062). Net, the skill moves <strong>REGRESSED(1) → MIXED</strong>. <code>commit-work</code> is unchanged at <strong>IMPROVED(1)</strong>: its driver (<code>two-sentence-describability-test</code>, Δ+0.168) held, and its amended cases (specific type token → shape, and the removed ≤72-char/no-period/imperative-mood rule) remain within noise. The other eight skills were not re-run and keep their v1.1 labels.',
    ],
  },
  {
...
function renderMarkdown(d) {
  ...
  L.push('');
  L.push(`> **${REPORT_VERSION}** (${REPORT_VERSION_DATE}) — supersedes v1.1. An in-text grounding policy was codified and applied across all 10 suites; imported rubric criteria in \`git-workflow-and-versioning\` and \`commit-work\` were amended and those two suites re-run. See [Amendments](#amendments).`);
  L.push('');
  ...
  // Appended after the entries recorded before it, so the section only grows. The
  // version note goes with it: above the report's opening it would be body text.
  L.push(WORDING_VERSION_NOTE_MD);
  L.push('');
  L.push(WORDING_AMENDMENT_MD);
  L.push('');
  return L.join('\n') + '\n';
}
...
<main class="report">
  ...
  <p class="version-note" style="margin:0 0 1rem;padding:.5rem .75rem;border-left:3px solid #62C7EF;background:rgba(98,199,239,.08);font-size:.95rem"><strong>${esc(REPORT_VERSION)}</strong> (${esc(REPORT_VERSION_DATE)}) — this edition supersedes v1.1. An in-text grounding policy was codified and applied across all 10 suites; imported rubric criteria in <code>git-workflow-and-versioning</code> and <code>commit-work</code> were amended and those two suites re-run. See <a href="#amendments">Amendments</a>.</p>
...
${sections}
  ...
  <ul class="method">
    ...
    <li><strong>Grounding policy (codified).</strong> <em>Every gradable rubric criterion must trace to text present in the skill's <code>SKILL.md</code> at the pinned SHA. Criteria that import domain knowledge the skill does not state are not gradable, regardless of how standard that knowledge is. Claims may summarize; rubrics may not extrapolate.</em> Each case records a <code>grounding</code> reference; the gate requires it. v1.2 applied this policy across all 10 suites (<a href="${REPO_BLOB}/reports/rubric-sweep.md">sweep</a>) — see <a href="#amendments">Amendments</a>.</li>
...
${a.points.map((p) => `      <li>${p}</li>`).join('\n')}
  ...
  ${WORDING_VERSION_NOTE_HTML}
  ${WORDING_AMENDMENT_HTML}

  <p class="foot">Receipts validate against the Driftproof receipt spec (v0.2; the two suites re-run in v1.2 are v0.3, a superset the loader also accepts). Every verdict above is a function of the two <code>with_skill</code> bands recorded in the receipts. Verdicts age because the substrate moves — that is the phenomenon this report measures.</p>
</main>
...
// Applies the band-wording amendment to the two published files in place. A
// rebuild is not used: the published HTML page also carries the site chrome and
// head block the later build steps add, so main() would not reproduce it.
// Idempotent: a file that already carries the entry is left as it is.
//
// THE ENTRY IS APPENDED, after the entries recorded before it (fix loop C). Placed
// at the top of the section it put new bytes ahead of the recorded ones, which is
// what spec 020 AC-21 reads as a recorded amendment rewritten in place. The v1.3
// version note sits directly above the entry, inside the section: above the
// report's opening it was body text, and moved the recorded body digest.
function applyWordingAmendment(root = ROOT) {
  const md = path.join(root, 'reports', 'report-001.md');
  const html = path.join(root, 'docs', 'reports', '001', 'index.html');
  let m = fs.readFileSync(md, 'utf8');
  if (!m.includes(WORDING_AMENDMENT_MD)) {
    if (m.indexOf('## Amendments\n\n') < 0) throw new Error('reports/report-001.md: no Amendments heading');
    m = `${m}${WORDING_VERSION_NOTE_MD}\n\n${WORDING_AMENDMENT_MD}\n\n`;
    fs.writeFileSync(md, m);
  }
  let h = fs.readFileSync(html, 'utf8');
  if (!h.includes(WORDING_AMENDMENT_HTML)) {
    const foot = '\n\n  <p class="foot">';
    const end = h.indexOf(foot, h.indexOf('<h2 id="amendments">Amendments</h2>\n'));
    if (end < 0 || h.split(foot).length !== 2) throw new Error('docs/reports/001/index.html: no single end to the Amendments entries');
    h = h.slice(0, end) + `\n  ${WORDING_VERSION_NOTE_HTML}\n  ${WORDING_AMENDMENT_HTML}` + h.slice(end);
    fs.writeFileSync(html, h);
  }
}

// GUARDED. This file used to call main() at import, so `require()`-ing it - which
// a checker, a probe or an editor's language server does routinely - REWROTE
// docs/reports/001/index.html. Measured on 2026-09-01 during spec 020: the
// import replaced the published page with a 46-line regeneration, destroying 303
// lines including the report's own v1.2 amendment block. CONSTITUTION invariant 4
// says a published report is amended with a versioned record and never silently
// edited; an import is the most silent edit there is.
//
// Every other generator in scripts/ already sits behind this guard.
module.exports = { applyWordingAmendment };
