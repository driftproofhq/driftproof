...
function plan() {
  const rows = CELLS.map((c) => {
    ...
    const calls = n * (2 + 2 * SAMPLES);
    const formula = estimateRunCostUSD({
      caseCount: n, samples: SAMPLES, models: [c.model], judgeModel: JUDGE_MODEL,
    }).totalUSD;
...
function printPlan({ rows, totals }, { dryRun }) {
  ...
  for (const r of rows) {
    ...
    console.log(`        cases ${r.cases} × (2 + 2×${SAMPLES}) = ${r.calls} calls   guard $${r.formulaUSD.toFixed(4)}   measured basis $${(r.measuredUSD || 0).toFixed(3)}`);
