...
# ── the publish target may not contain a nested repository (spec 015 AC-9) ──
#
# Backlog F-014-E. `~/driftproof-public/driftproof-v0.6.0/` was an untracked
# shallow clone of the published tag, sitting inside the publish target with its
# own `.git`, most likely a hand verification of a release. `rm -rf "$PUB"` below
# wipes it, so the sanctioned path could not have shipped it — and that is
# precisely why nothing noticed for a week.
#
# WHAT WAS ACTUALLY MISSING IS THE ASSERTION. Nothing checked the state of the
# target BEFORE a build. The hygiene scans read the tree this script builds, not
# what was lying in the target beforehand, so a nested repository plus a
# duplicate tree — the exact shape `git add -A` would stage if the destructive
# step were ever skipped or reordered — was invisible to every scan there is.
# `P-1`'s family: a thing in the publish target that everything is looking past.
#
# IT REFUSES RATHER THAN CLEANING UP. Deleting a repository somebody put there by
# hand is not this script's call, and a target holding one is a target whose state
# nobody understands. Saying so and stopping is the honest move; the operator
# removes it, or explains it, and runs again.
#
# PLACED HERE, with the other target rules and far above `rm -rf "$PUB"`, so the
# refusal precedes the destruction — which is the whole content of the finding.
# `-mindepth 2` is what excludes the target's OWN `.git` at depth 1 without
# naming it, so the rule reads "no repository inside this one" rather than "no
# repository except the one I expect".
if [ -d "$PUB" ]; then
  NESTED_GIT="$(find "$PUB" -mindepth 2 -name .git -print 2>/dev/null | head -5 || true)"
  if [ -n "$NESTED_GIT" ]; then
    echo "build-public.sh: REFUSING — the publish target contains a nested git repository." >&2
    echo >&2
    echo "  target: $PUB" >&2
    echo "  found:" >&2
    # Globbing off across the only split in this block. A path containing `*`
    # must not have the refusal enumerate the working directory instead of the
    # offender — the same defect specs/006's marker parse fixed the same way.
    set -f
    printf '    %s\n' $NESTED_GIT >&2
    set +f
    echo >&2
    echo "  This script deletes and rebuilds its target. A repository inside it is" >&2
    echo "  something this script did not create and cannot account for, and a nested" >&2
    echo "  repository beside a duplicate tree is the shape a stray 'git add -A' would" >&2
    echo "  stage into a tree about to become public. Nothing here removes it for you:" >&2
    echo "  a target whose contents nobody can explain is not a target to build into." >&2
    echo >&2
    echo "  Remove it, or move it outside the target, and run again." >&2
    exit 9
  fi
fi
# ── end AC-9 guard ──
...
EXCLUDE_RE='^reports/(week-[0-9][^/]*|phase-[0-9][^/]*)\.md$|^tests/gate-results\.json$|(^|/)[^/]*-draft/|^reports/pending-publish\.md$|^reports/interop-outreach\.md$|^(BACKLOG|CLAUDE|CONSTITUTION|DECISIONS)\.md$|^specs/|^state/'
...
# not itself a file the target has to account for, and read back after the
# commit to prove the tree carries every one of them (spec 030 AC-5).
PUB_FILELIST="$(mktemp "${TMPDIR:-/tmp}/driftproof-publish-manifest.XXXXXX")"
trap 'rm -f "$PUB_FILELIST"' EXIT

# Copy exactly the git-tracked product files, minus the exclude list.
cd "$SRC"
git ls-files | grep -vE "$EXCLUDE_RE" > "$PUB_FILELIST"
while read -r f; do
...
# receipts/commit-message-conventions-…-2026-07-27.json, which the source
# repository carries force-added and `scripts/site-data.mjs` reads
# unconditionally at module load. The published tree therefore shipped a reader
# with no artifact, and `npm run gate` on a fresh clone of v0.9.0 died with an
# unhandled ENOENT after 559 checks - no summary, no verdict, nothing a
# contributor could act on.
#
# The defect is GENERAL, not one filename: every force-added tracked file is
# silently dropped, and the build reports success. `receipts/report-00N/*.json`
# survived only because `/receipts/*.json` matches direct children.
#
# So: add everything, then FORCE-ADD exactly the list that was copied. The
# publish tree now commits what the build put in it, by construction, and the
...
# the manifest against what was actually committed, and refuses rather than
# publishing a subset.
MISSING="$(comm -23 <(sort -u "$PUB_FILELIST") <(git ls-files | sort -u))"
if [ -n "$MISSING" ]; then
  echo >&2
  echo "  REFUSED: the published tree does not carry every file this build copied." >&2
  echo "  These were copied into the target and are not in the commit:" >&2
  printf '%s\n' "$MISSING" | sed 's/^/    /' >&2
  echo >&2
  echo "  A file is usually missing here because the .gitignore this build copied" >&2
  echo "  matches it and nothing force-added it. The publish tree must commit what" >&2
  echo "  the build put in it; a reader shipped without its artifact is how a fresh" >&2
  echo "  clone's gate dies with ENOENT instead of reporting a verdict." >&2
  exit 10
fi
