...
async function prepareReport005(opts = {}) {
  ...
  const projection = estimateRunCostUSD({ caseCount: nCasesTotal, samples: 5, models, judgeModel: REPORT_005_JUDGE_MODEL });
  const overGuard = projection.totalUSD > maxUsd;
  const projectedCalls = projectCalls(nCasesTotal, 5) * models.length;
...
async function main() {
  ...
  if (!renderOnly && (execute || smokeOnly)) {
    ...
    const proj = estimateRunCostUSD({ caseCount: 70, samples: 5, models, judgeModel: REPORT_005_JUDGE_MODEL });
    console.log(`\nPROJECTED full-run cost: ~$${proj.totalUSD.toFixed(2)} (guard $${maxUsd})  [${proj.perModel.map((p) => `${p.model} ~$${p.usd.toFixed(2)}`).join(', ')}]`);
    console.log(`PROJECTED live calls: ~${projectCalls(70, 5) * models.length} for the full run (${models.length} substrates × 10 suites × 70 cases × (2 gens + 2×5 judge)) + ${models.length} for the light smoke.`);
