...
const { promptTemplateHash } = require('../lib/judge');
...
// where the lift cleared the effect floor (else lib/value.js NOISE_CELL). Those
...
const { buildReceipt, validateReceipt, verifyReceiptHash , caseFailed } = require('../lib/receipt');
...
// did not separate (the ratio cell must read NOISE_CELL), and a spread
...
function synthSamples(idx, ci, mIdx, withSkill) {
  ...
  return spread(0.83, false); // ceremonial: baseline already ~0.82, so no separation
}
...
function stubReceipt(skill, idx, model, mIdx, nowIso) {
  ...
  return buildReceipt({
    ...
    run: {
      ...
      provider: providerForModel(model), surface: 'stub', // the RUN's surface: nothing answered (spec 026)
      // no surface_overhead_note on a stub run: the codex preamble describes a harness that did not run
      runner_version: RUNNER_VERSION, date_utc: nowIso, registry: registryStatus(model), transcripts: 'hashes-only',
      judge: { samples: 5, temperature: null, sampling: 'surface-controlled', surface: 'stub', model_id: REPORT_005_JUDGE_MODEL, prompt_template_hash: promptTemplateHash() },
      pricing_snapshot: pricingSnapshot,
      // v0.6 (spec 026): a staged draft's rows are synthesised; the receipt says
      // so. Nothing answered (kind stub, surface stub), and the draft is
      // UNVERIFIED: it stages the page, it does not measure the skill.
      answered_by: { kind: 'stub', attested: false, reported_model: null, reported_models: null, isolation: 'none' },
    },
    cases, economics, verificationLevel: 'UNVERIFIED',
  });
}
...
function skillDirection(receipt) {
  ...
  for (const id of Object.keys(by)) {
    ...
    if (bandVerdict(b.mean, b.stddev, w.mean, w.stddev) === WITHIN_NOISE || Math.abs(dmean) < EFFECT_FLOOR) continue;
...
function draftHtml({ rows, models, surfaces, projection, stub, tally, notMeasured, runRecord, nowIso, pricingFrozenAt, wallClock, money, costFacts, skillCheck, maxUsd, judgeFacts, priceSpread, projectionAtRun, projectionUSD, receiptsLabel, published }) {
  const cell = (c) => {
    ...
    // read the no-separation mark beside their own named drivers (QA V-4).
...
async function prepareReport005(opts = {}) {
  ...
  for (let i = 0; i < skills.length; i++) {
    ...
    for (let mIdx = 0; mIdx < models.length; mIdx++) {
      ...
      byModel[model] = {
        ...
        // renderer, so a lift that did not separate can never become a number here.
  ...
  const summary = `Report #${REPORT_NUMBER} ${stub ? 'STUB DRAFT staged' : 'DRAFT ready'}: VALUE report over ${models.length} substrates (${models.join(', ')})`
    ...
    + ` ${tally.within_noise} with no separation detected.`
...
const WORDING_ENTRY_005 = `    <p><strong>v1.3 &middot; 2026-09-14</strong>. <strong>This entry corrects how the page&rsquo;s band wording is read; it follows the entries recorded before it, no earlier text is changed, and no figure, verdict token, table value or receipt reference changes.</strong> The lift column marks cells <em>(within noise)</em>, the cost-per-benefit cells and the method notes read <em>n/a (within noise)</em>, and the entry of 2026-09-01 says a comparison <em>&ldquo;rendered WITHIN NOISE, which says the two measurements are the same as far as the instrument can tell&rdquo;</em>. The ten cells marked within noise have no case separated under the rule at the sample size used: no separation detected, which is not evidence of equivalence and not evidence that nothing changed. The comparison that entry quotes, Report 005 against Report 007 on <code>writing-plans</code>, had none of its seven with_skill cases separated, so it did not show the two measurements are the same: it detected no separation between them. The lift column&rsquo;s mark and the cost-per-benefit label are left as published on this page; for the cost-per-benefit cell the renderer now writes <code>n/a (no separation detected)</code>, and it still writes the lift column&rsquo;s <em>(within noise)</em> mark as published. <strong>How to read this page.</strong> A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. No cell&rsquo;s aggregate lift on this page is separated: in every cell the with_skill and baseline aggregate bands overlap. A cell&rsquo;s separations are its cases&rsquo;. Wherever the page reads a result with no separation detected as settled, as <em>within noise</em>, <em>dividing noise by a real cost</em>, or a figure with <em>nothing under it</em>, read it as no separation detected at the sample size used, which is not evidence that nothing is there; of the sixteen cells whose aggregate lift is below the floor, six have a case separated and read <em>n/a (driver-only)</em>. Sampling noise that <em>accounts for the observations</em>, and a draw that is <em>the likelier fluke</em>, are causes offered for baseline cases that Reports 006 and 007 found separated, from draws to which no separation rule was applied: an observation, not proof of a cause. Wherever it reads a separation in a cell&rsquo;s cases as a settled property of the skill, as a lift <em>called real</em>, <em>the effect is a clean grid step</em>, or <em>The verdict stands</em>, read it as a separation detected under the rule in one or more cases, which is not proof that the skill moved the score. Wherever it reads a saving as a settled improvement, as <em>having improved quality while reducing cost</em> or <em>the benefit is free</em>, read it as a floor-clearing aggregate lift that is not separated under the rule, in a cell with a separation detected in one or more cases, which is not proof that the skill raised quality or cut cost. The no-skill arm that <em>did not reproduce</em>, in the entry of 2026-09-01, reads a baseline control that found baseline cases whose bands did not overlap, which is not proof that a baseline changed. The page&rsquo;s <em>a skill that measurably hurt</em> names a rendering for a floor-clearing negative aggregate lift, which no cell on this page has. The baselines the entry of 2026-08-29 calls <em>now shown unstable</em> rest on the spread of Report 006&rsquo;s draws, to which no separation rule was applied. Labels, tallies and reason strings are read the same way and are left as published. <strong>The bands.</strong> The band printed beside each cell, under the header <em>lift (band)</em> and in the legend&rsquo;s <em>Accuracy lift (band)</em>, is not a lift band: it is the cell&rsquo;s with_skill mean plus or minus the sample standard deviation of its seven per-case means, which is suite dispersion, and the legend&rsquo;s <em>mean ± stddev over 5 judge samples</em> and the lede&rsquo;s <em>the accuracy lift with its bands</em> read as that. The per-case bands the Verdict basis and the low-resolution note describe are a case&rsquo;s mean plus or minus the sample standard deviation of its five judge samples. The lift band the entry of 2026-09-01 names, <code>delta_uncertainty</code>, combines the two arms&rsquo; suite dispersions in quadrature; that entry&rsquo;s <em>its band is the spread of the judge re-scoring that single response</em> describes neither the band printed beside a cell nor the lift band, and its <em>none of the three separates from its own band</em> compares a lift with its lift band, which is not the separation rule. The <em>different error bars</em> of the axes are different descriptive spreads. Each band is a descriptive spread, not a confidence interval, with no coverage probability. Filed under the wording rules of the repository's spec 031, amendment A-031-20.</p>`;
// Its own card, after the card recorded before it: the section only grows.
const WORDING_AMENDMENT_005 = `<div class="card">\n${WORDING_ENTRY_005}\n  </div>\n  `;
// Applies it to docs/reports/005/index.html in place, at the end of its Amendments
// section, ahead of the heading that follows it. Idempotent.
function applyWordingAmendment005(root = ROOT) {
  const file = path.join(root, 'docs', 'reports', '005', 'index.html');
  const html = fs.readFileSync(file, 'utf8');
  if (html.includes(WORDING_AMENDMENT_005)) return false;
  const heading = '  <h2>Amendments</h2>\n';
  if (html.split(heading).length !== 2) throw new Error('docs/reports/005/index.html: the Amendments heading is not unique');
  const next = html.indexOf('<h2>', html.indexOf(heading) + heading.length);
  if (next < 0) throw new Error('docs/reports/005/index.html: no heading after the Amendments section');
  fs.writeFileSync(file, html.slice(0, next) + WORDING_AMENDMENT_005 + html.slice(next));
  return true;
}
