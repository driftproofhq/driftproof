...
function heroReceipt(V) {
  ...
  const src = fs.readFileSync(path.join(DOCS, V('example_receipt_plot')), 'utf8').trim();
  const plot = src.replace('class="bandplot', 'class="receipt-plot bandplot');
  return chrome.receiptCard({
    variant: 'hero',
    heading: 'A receipt, in full',
    rows: [
      { key: 'Verdict', rowClass: 'receipt-row-verdict', stamp: STAMP[V('example_receipt_verdict')] || 'refused', value: esc(V('example_receipt_verdict')) },
      ...
      { key: 'Range', value: mono(`${V('example_receipt_baseline')} without, ${V('example_receipt_with_skill')} with`) },
      { plot },
...
<img class="card-plot bandplot" src="/${esc(r.plot.value)}" alt="Band plot for the first cell of Report ${esc(r.number.value)}." width="260" height="72">
</article>`;
}
...
const TERMS = [
  ...
  ['band', 'The range a score carries instead of a single number, because a scorer asked twice does not answer identically twice. A result is a band, not a point, and the width of that band is what decides whether a difference is real. A band is the across-draw spread of n generations scored by the fixed judge.', CORE, 'with vs without, sampled, banded'],
...
function bandsSection() {
  ...
  <figure class="panel plot-figure is-prose">
    <img class="bandplot" src="/${esc(plot)}" alt="Two horizontal ranges from a published cell, one without the skill and one with it, drawn apart with the effect floor marked." width="520" height="160">
    <figcaption>
...
function floorFigure(html) {
  ...
  return html.replace(re, (_m, li, p) => `<figure class="panel plot-figure is-prose" id="effect-floor-figure">
    <img class="bandplot" src="/${esc(plot)}" alt="Two horizontal ranges from a published cell that overlap, with the effect floor marked as a dashed line." width="520" height="160">
    <figcaption>
    <ul class="method">
      ${li}
    </ul>
