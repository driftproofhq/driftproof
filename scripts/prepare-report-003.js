...
// per-skill verdict is regressed, improved, mixed or none separated under the exact
...
// (regressed, improved, mixed, none separated) over the 10 suites. Baseline ~0.40.
...
function stubReceipt(skill, idx, model, isNew, nowIso) {
  ...
  return buildReceipt({
    ...
    run: {
      ...
      judge: { samples: 5, temperature: null, sampling: 'surface-controlled', surface: 'stub', model_id: REPORT_003_JUDGE_MODEL, prompt_template_hash: promptTemplateHash() },
    ...
    cases, verificationLevel: 'UNVERIFIED',
  });
}
...
async function prepareReport003(opts = {}) {
  ...
  const projection = estimateRunCostUSD({ draws: SAMPLING.max, caseCount: nCasesTotal, samples: 5, models: [pair.new, pair.old], judgeModel: REPORT_003_JUDGE_MODEL });
  const overGuard = projection.totalUSD > maxUsd;
  const projectedCalls = projectCalls(nCasesTotal, 5, SAMPLING.max) * 2; // both versions
...
async function main() {
  ...
  if (execute || smokeOnly) {
    ...
    // Projection FIRST.
    const proj = estimateRunCostUSD({ draws: SAMPLING.max, caseCount: 70, samples: 5, models: [REPORT_003_NEW_MODEL, REPORT_003_OLD_MODEL], judgeModel: REPORT_003_JUDGE_MODEL });
    console.log(`\nPROJECTED full-run cost: ~$${proj.totalUSD.toFixed(2)} (guard $${maxUsd})  [${proj.perModel.map((p) => `${p.model} ~$${p.usd.toFixed(2)}`).join(', ')}]`);
    console.log(`PROJECTED live calls: ~${projectCalls(70, 5, SAMPLING.max) * 2} for the full run (2 versions × 10 suites × 70 cases × (${SAMPLING.max} draws × (2 gens + 2×5 judge))) + ~12 for the smoke.`);
...
const WORDING_AMENDMENT_003 = `  <h2>Amendments</h2>
  <div class="card">
    <p><strong>v1.1 &middot; 2026-09-14</strong>. <strong>This entry corrects how the page&rsquo;s band wording is read; no earlier text is changed, and no figure, verdict token, table value or receipt reference changes.</strong> The headline says <em>&ldquo;2 regressed · 4 improved · 0 mixed · 4 within noise&rdquo;</em>, the table labels four skills <em>WITHIN NOISE</em>, and the introduction says <em>&ldquo;Bands that overlap, or separate by less than the floor, are within noise&rdquo;</em> and <em>&ldquo;never drift&rdquo;</em>. None of those four skills had a with_skill case separated under the rule between the two models: no separation detected at the sample size used, which is not evidence of equivalence, not evidence that nothing changed, and not a finding of no drift. The labels are the verdict values this report recorded, and are left as published. As amended:</p>
    <p class="amended-headline"><strong>2 regressed · 4 improved · 0 mixed · 4 with no separation detected</strong> over 10 skills.</p>
    <p><strong>How to read this page.</strong> A case is separated under the rule when its two bands do not overlap and its mean moved by at least the 0.05 effect floor; bands that do not overlap by a smaller move are not separated. What the introduction calls bands that <em>separate by less than the floor</em> are bands that do not overlap. Wherever the page reads a result that was not separated as settled, as <em>within noise</em> or <em>never drift</em>, read it as no separation detected at the sample size used. Wherever it reads a separation as settled, as the low-resolution note&rsquo;s <em>The verdict stands</em>, read it as a separation detected under the rule, which is not proof that the model&rsquo;s behaviour moved; that note&rsquo;s <em>confidence</em> is not a property of any band on this page. Labels and headings are read the same way and are left as published.</p>
    <p><strong>The bands.</strong> Each band on this page is a mean plus or minus one sample standard deviation, of two kinds: a case&rsquo;s band, over its five judge samples, and a skill&rsquo;s <code>with_skill</code> band, over its seven per-case means, which is suite dispersion. The column note&rsquo;s <em>band (mean ± stddev over 5 judge samples)</em>, said of the skill bands, reads as suite dispersion. Each band is a descriptive spread, not a confidence interval, with no coverage probability. The site&rsquo;s report index and feed quote the amended headline; this page&rsquo;s summary card, description and social card image keep the headline as published. Filed under the wording rules of the repository's spec 031, amendment A-031-20.</p>
  </div>

`;
// Applies it to docs/reports/003/index.html in place, before its Receipts section. Idempotent.
function applyWordingAmendment003(root = ROOT) {
  const file = path.join(root, 'docs', 'reports', '003', 'index.html');
  const html = fs.readFileSync(file, 'utf8');
  if (html.includes(WORDING_AMENDMENT_003)) return false;
  const anchor = '  <h2>Receipts</h2>';
  if (html.split(anchor).length !== 2) throw new Error('docs/reports/003/index.html: the insertion point is not unique');
  fs.writeFileSync(file, html.replace(anchor, `${WORDING_AMENDMENT_003}${anchor}`));
  return true;
}

module.exports = { applyWordingAmendment003, prepareReport003, driftVerdict, draftHtml, disclosedSurface, smokeTest, REPORT_NUMBER };
