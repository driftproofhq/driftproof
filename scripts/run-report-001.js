...
async function main() {
  ...
  for (const s of skills) {
    ...
    projTotal += estimateRunCostUSD({ caseCount: nC, samples, models: [newModel, oldModel], judgeModel }).totalUSD;
  }
  ...
  for (const s of skills) {
    ...
    for (const [role, modelId] of [['new', newModel], ['old', oldModel]]) {
      ...
      const { receipt, calls, transcripts } = await runSkillOnModel({
        skill, model: modelId,
        opts: { samples, judgeModel, concurrency, maxCases, maxCalls: 500, timeoutMs, keepTranscripts, budget },
      });
      totalCalls += calls;
