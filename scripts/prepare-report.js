...
function stubReceipt(skill, idx, model, isNew, nowIso) {
  ...
  return buildReceipt({
    skill: { name: skill.name || skill.slug, version: '0.0.0', contentHash: h(`${skill.slug}|content`) },
    suite: { format: 'agentskills.io/evals', suiteHash: h(`${skill.slug}|suite`), caseCount: caseList.length },
    run: {
      model_id: model, model_release_date: releaseDateFor(model), surface: 'claude-cli',
...
async function prepareReport(opts = {}) {
  ...
  const proj = estimateRunCostUSD({ caseCount: nCasesTotal, samples: 5, models: [pair.new, pair.old], judgeModel: 'claude-haiku-4-5' });
  let trimmed = false;
