...
function synthSamples(slug, idx, isNew, withSkill) {
  ...
  if (isNew && withSkill && idx % 3 === 1) center = base + 0.02; // no separation
...
function stubReceipt(skill, idx, model, isNew, nowIso) {
  ...
  return buildReceipt({
    ...
    run: {
      ...
      judge: { samples: 5, temperature: null, sampling: 'surface-controlled', surface: 'stub', model_id: 'claude-haiku-4-5', prompt_template_hash: promptTemplateHash() },
