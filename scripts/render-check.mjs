...
//
// IT ALSO WRITES SCREENSHOTS, one per CAPTURED page per viewport, into
...
//
// MEASURED AND CAPTURED ARE NOT THE SAME SET (spec 025 A-025-18, 2026-09-16).
// Every page in the sitemap is opened at both viewports on both passes, and that
// has not changed. What changed is the sitemap: spec 036's A-036-1 listed one
// page per archived receipt, and there are 120 of them, so `sitemapPaths` went
// from 21 entries to 141. A screenshot of each at each viewport is 282 files and
// about 36MB of PNG re-encoded into git history on every --final run, and 240 of
// those files are the same generator's same template with different values in
// it. So the capture clause narrows and the measurement clause does not:
// `captureSet` keeps every page that is not a receipt page, plus ONE receipt
// page for each distinct state the archive actually renders.
//
...
// ── the capture set (A-025-18) ──────────────────────────────────────────────
// A receipt page, by its published path. The generator writes exactly this shape
// and `build-public.sh` publishes it; nothing else under docs/ matches.
export const RECEIPT_PATH = /^\/r\/[0-9a-f]{64}\/$/;
//
// THE STATE IS READ OFF THE PAGE, not off lib/verdict.js. The generator writes
// the state into the verdict block's own class - `state-not-measured` - and the
// stylesheet paints from it, so this reads the thing that shipped rather than
// re-deriving what should have shipped. A capture set computed from the verdict
// rule would share code with the rule, and an archive whose pages all carried
// the wrong state would still produce a "complete" capture set.
//
// A page that is a receipt page and whose state cannot be read is CAPTURED, not
// dropped: an unreadable page is the one a reader most needs a picture of, and
// silently omitting it is `absence-vs-unreadable`.
export function captureSet(root = ROOT) {
  const docs = path.join(root, 'docs');
  const seen = new Set();
  const out = [];
  for (const p of sitemapPaths(root)) {
    if (!RECEIPT_PATH.test(p)) { out.push(p); continue; }
    let html = null;
    try { html = fs.readFileSync(path.join(docs, p.replace(/^\//, ''), 'index.html'), 'utf8'); } catch { html = null; }
    const m = html && html.match(/class="receipt-verdict\s+state-([a-z-]+)"/);
...
export async function run({ out, screens, limit, passes, docs, pages, viewports, capture } = {}) {
  ...
  // Which of the opened pages get a PNG. `capture` names it outright (the gate's
  // stability re-run passes the same set it captured the first time); otherwise
  // it is derived from the tree being served. A one-page mutation run serves a
  // scratch copy of docs/ and writes no screenshots at all, and `captured` is
  // then irrelevant because `screens` is unset.
  const captured = new Set(capture && capture.length ? capture : (screens ? captureSet(path.join(served, '..')) : []));
    ...
    for (const [name, viewport] of vps) {
      for (const motionPref of motionPasses) {
        ...
        for (const p of paths) {
          ...
          if (m.playground) {
            // A-025-17: the default toggle draws the dispersion band, whose mean
            // marker only that mode paints; the "point" toggle draws the
            // zero-width band as a dot with an outline, a rendering path the
            // default view reaches only when the linked receipt's own cases
            // happen to carry no spread (spec 026 A-026-13 gave it real per-case
            // samples, so it no longer does). Read pointStroke from the point
            // toggle separately, after the default-view paint is already
            // captured, so this does not cost the mean-marker check its subject.
            const clicked = await page.click('.playground [data-def="point"]').then(() => true).catch(() => false);
            if (clicked) m.playground.pointStroke = await page.evaluate(() => [...document.querySelectorAll('.playground .pg-plot svg .point')].map((el) => getComputedStyle(el).stroke));
          }
          record.pages.push({ page: p, viewport: name, motion_pref: motionPref, requests, ...m });
          if (screens && motionPref === SCREENSHOT_PASS && captured.has(p)) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (arg('--screens')) console.log(`captured ${captureSet().length} of ${sitemapPaths().length} pages at each viewport`);
}
