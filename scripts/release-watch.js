...
// Fetch the served model ids. Returns an array of id strings.
async function fetchModelIds({ modelsFile }) {
  if (modelsFile) {
    const raw = readJson(modelsFile, null);
    if (!raw) throw new Error(`could not read models file ${modelsFile}`);
...
async function runWatch(opts = {}) {
  ...
  // A new id = served but not yet seen and a real Anthropic claude-* id.
  const newIds = endpointIds.filter((id) => !seen.has(id) && /^claude-/.test(id));

  const result = { now, endpointCount: endpointIds.length, newModels: newIds, triggered: [], skipped: [], dryRun: !!opts.dryRun };
  ...
  if (!opts.dryRun) {
    ...
    writeJson(seenPath, { seen: Array.from(new Set([...seen, ...endpointIds.filter((id) => seen.has(id))])).sort(), updated: now });
...
module.exports = { runWatch, fetchModelIds };
