...
// first-seen date) into an UNTRACKED queue and kick scripts/prepare-report.js to prepare a
...
// The discovery queue: one JSON object per line, append-only, under state/ which
// .gitignore excludes. Idempotent on model_id, so a daily retry does not stack
// duplicate entries. Returns true if a line was appended.
function queueDiscovery(queuePath, entry) {
  let existing = '';
  try { existing = fs.readFileSync(queuePath, 'utf8'); } catch (_e) { /* first entry */ }
  for (const line of existing.split('\n')) {
    if (!line.trim()) continue;
    try { if (JSON.parse(line).model_id === entry.model_id) return false; } catch (_e) { /* keep going */ }
  }
...
async function runWatch(opts = {}) {
  ...
  // A FIXED filename, never one built from a discovered id. The ids come from an
  // upstream models list, and a path assembled from one would let an upstream
  // value choose where this process writes (NFR-2).
  const queuePath = opts.queuePath ? path.resolve(opts.queuePath) : path.join(stateDir, 'discovery-queue.jsonl');
  ...
  // provider inferred from the id (lib/models.inferredPrice / familyOf).
  ...
  for (const id of newIds) {
    ...
    // QUEUE IT. DO NOT REGISTER IT (F-W1, spec 021).
    //
    // This used to call models.addDiscoveredModel, which appends a row to the
    // TRACKED config/models.json of whatever worktree the timer fires in. It
    // dirtied a gate run, blocked the 019b rebase, and fired again unattended at
    // 00:15:55 UTC on 2026-09-03. A process on a timer must never mutate a
    // gate-covered file: the next person to run `git status` cannot tell an
    // unattended write from their own work.
    //
    // Registration stays a HUMAN-APPROVED SPEC COMMIT binding prices to a dated
    // docs snapshot, the way spec 019b did it. What the watcher may do is leave a
    // note in an untracked queue saying what it saw.
    //
    // The prices in that note are a GUESS and say so. The watcher reads no price
    // source at all; lib/models.inferredPrice derives them from the id, and for
    // any Anthropic family that is not haiku or sonnet it returns the opus rate.
    // That is F-W2, recorded rather than trusted.
    const inferred = models.inferredPrice(id);
    const queued = queueDiscovery(queuePath, {
      model_id: id,
      first_seen: day,
      date_is_first_seen: true,
      first_seen_at: now,
      provider: inferred.provider,
      family: inferred.family,
      tier: inferred.tier,
      input_price: inferred.price.input,
      output_price: inferred.price.output,
      price_verified: false,
      source: 'inferred:lib/models.js inferredPrice() family-to-tier table; no price source is read',
      discovered_from: id.startsWith('gpt') || inferred.provider === 'openai' ? openaiSource : anthropic.source,
      status: 'awaiting-registration',
    });
    log(`queued ${id} for registration review (first-seen ${day}, prices UNVERIFIED ${inferred.price.input}/${inferred.price.output} inferred)${queued ? '' : ' — already queued'}`, { now });
...
async function main() {
  ...
  const res = await runWatch({
    ...
    // --queue exists for the gate's mutation, which points it at a TRACKED file
    // and requires the tree to go dirty. A containment check that cannot be made
    // to fail is not a check.
    queuePath: f['queue'] || undefined,
