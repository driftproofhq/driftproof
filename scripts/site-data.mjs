...
const ENT = { amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", nbsp: ' ', middot: '·', hellip: '…', mdash: '—', ndash: '–', times: '×', plusmn: '±', lsquo: '‘', rsquo: '’', ldquo: '“', rdquo: '”' };
...
// THE CARD'S DESCRIPTION HAS A FLOOR AS WELL AS A CAP. Spec 020 AC-14 requires
// every page's description to reach 50 characters, and Report 008's headline
// sentence was thirty-four characters long as published. One sentence
// is the rule and a second is added only where the first does not reach the
// floor, which is the same shape `pageDescription` in build-head-tags.js already
// used on a page's paragraphs.
const DESC_MIN = 50;
function cardDescription(bigHtml) {
  const parts = sentencesOf(textOf(bigHtml));
  let out = parts.length ? parts[0] : '';
  for (let i = 1; i < parts.length && out.length < DESC_MIN; i++) out = `${out} ${parts[i]}`;
  return out.trim();
}
...
// AN AMENDED HEADLINE AND VERDICT BEAT THE PUBLISHED ONES (spec 031 A-031-20). A
// report whose wording was corrected carries, inside its amendment entry, the
// headline and verdict as amended: <p class="amended-headline"> and
// <p class="amended-verdict"><strong>. Where they exist the site quotes them, so a
// card, a feed entry or a page description does not repeat wording the report has
// itself corrected. They are still substrings of the page they cite, and the
// original sentences stay on the page above the amendment.
// THE LATEST, read the way spec 020's gate reads it (its amendment 51): the last
// <p class="amended-headline"> in the report's Amendments section, found by
// splitReportBody, because an entry is appended. The first on the page was a second
// definition that agreed with the gate only while a record carried one.
const amendedHeadline = (html) => {
  const { amendments } = require(path.join(ROOT, 'scripts', 'site-chrome.js')).splitReportBody(String(html));
  const all = [...amendments.matchAll(/<p class="amended-headline">([\s\S]*?)<\/p>/g)];
  return all.length ? all[all.length - 1][1] : null;
};
function verdictLine(bigHtml, pageHtml = '', { amended: readAmended = true } = {}) {
  const amended = readAmended ? /<p class="amended-verdict">\s*<strong>([\s\S]*?)<\/strong>/.exec(String(pageHtml)) : null;
  if (amended) {
    const stated = textOf(amended[1]).replace(/[.\s]+$/, '').trim();
...
// the page beside it. Re-cut at the 0.11.0 pre-build (DECISIONS 2026-09-16, spec
// 035 F-3): the 2026-07-27 receipt predates answered_by and badges not measured, so
// the example is a fresh receipt that says what answered it. Re-run again on the
// released 0.11.0 so the example's runner_version is the release's own.
const EXAMPLE_REL = 'receipts/commit-message-conventions-claude-haiku-4-5-20251001-2026-09-17.json';
...
function buildStats() {
  ...
  const rule = textOf(fs.readFileSync(meth, 'utf8'))
    .split(/(?<=[.])\s+/).find((s) => /bands touch or overlap/i.test(s));
  ...
  const latestBig = amendedHeadline(fs.readFileSync(latestPage, 'utf8')) || (fs.readFileSync(latestPage, 'utf8')
  ...
  return {
    ...
    // A latest report with no receipts (spec 041) takes the date its own row reads
    // off the page, so the homepage card and this field agree.
    latest_report_date: dates.length ? F(dates[dates.length - 1].d, dates[dates.length - 1].f) : reportRow(latest).date,
    // Spec 047: the latest card's title tail, which the homepage wraps in data-stat so a
    // numeral in it (Report 011's model name) names this field (spec 020 AC-9).
    latest_report_what_moved: reportRow(latest).what_moved,
    ...
    // THE WHOLE HASH (spec 025 V8, amendment A-025-6). It printed the first
    // sixteen characters, which is a prefix of the thing the site's first
    // invariant is about: a receipt hash a reader cannot check against the file
    // is a decoration. Sixty-four characters wrap freely on the one row that is
    // allowed to, and the copy allowlist records the change.
    example_receipt_hash: F(EXAMPLE.receipt_hash, EXAMPLE_REL),
    // THREE SIZES, THREE PATHS (spec 025 AC-6). The hero card, the playground's
    // static fallback and an index thumbnail are the same cell drawn at three
    // sizes, and each names the one it uses rather than scaling a fourth.
    example_receipt_plot: F(`plots/${EXAMPLE_KEY}.hero.svg`, 'docs/plots'),
    example_receipt_plot_play: F(`plots/${EXAMPLE_KEY}.play.svg`, 'docs/plots'),
    example_receipt_plot_card: F(`plots/${EXAMPLE_KEY}.card.svg`, 'docs/plots'),
...
export function reportRow(n, opts = {}) {
...

  {
    ...
    const bigHtml = amendedHeadline(html) || (html.match(/<div class="headline">[\s\S]*?<p class="big"[^>]*>([\s\S]*?)<\/p>/) || [])[1] || '';
    ...
    // A REPORT WITH NO DRIFTPROOF RECEIPTS (spec 041, Report 010). Its figures are
    // another harness's output files, so there is no receipt to read a model or a
    // date from. The page marks the one <data> element that carries each, with
    // data-row="model" or data-row="date", and the field cites that element's first
    // source file, which is where the value was read. A page with receipts is
    // unaffected: this runs only when receipts/report-<n>/ holds none.
    if (!rfiles.length) {
      for (const m of html.matchAll(/<data data-row="(model|date)" value="([^"]*)" data-src="([^" ]+)[^"]*"/g)) {
        const value = m[2].replace(/&amp;/g, '&');
        if (m[1] === 'model' && !models.some((x) => x.value === value)) models.push(F(value, m[3]));
        if (m[1] === 'date' && value.slice(0, 10) > date.d) date = { d: value.slice(0, 10), f: m[3] };
      }
    }
    ...
    // A PAGE THAT NAMES ITS RECEIPTS WITHOUT LINKING THEM (spec 039). Report 009
    // lists its evidence copies as paths, not links, so `linked` is empty and the
    // card said "no receipts linked" over a report that publishes three. Then the
    // card takes the directory its models and date are already read from, and the
    // count is that directory's receipts, which is the destination it links.
    const receiptPaths = linked.length
      ? linked.map((p) => F(p, pageRel))
      : rfiles.map((f) => rel(f)).sort().map((p) => F(p, `receipts/report-${n}`));
    ...
    return {
      ...
      // THE CARD'S DESCRIPTION, CAPPED WHERE THE CARD IS READ (spec 025 AC-15,
      // closing the [025] carry from approval-20260902T104317Z F-020-B). It was
      // capped at 200 by one function and the meta description at 160 by
      // another, so a report page shipped two descriptions of two lengths cut in
      // two places, and Report 005's card read "18 with an". One source, one cap,
      // one ellipsis: the page's own headline sentence, and the same value fills
      // the meta description and og:description.
      og_description: F(prefix(cardDescription(bigRaw), 160), pageRel),
      ...
      // No receipts, no band plot: the field is absent rather than naming a plot
      // file that does not exist (spec 041).
      ...(rfiles.length ? { plot: F(`plots/${keyOf(rfiles[0], ROOT)}.card.svg`, 'docs/plots') } : {}),
      essay_links: [{ title: F(essayTitle, essay), href: F('/writing/three-releases/', 'docs/writing') }],
    };
  }
}

// THE RECORD'S OWN FIELDS STAY AS PUBLISHED (spec 031 A-031-24, F-1 of its
// approval on b5e99ed). A report page is a dated record, and its summary card and
// description are part of it: an amended headline or verdict is added in the
// amendment entry and may not rewrite them (CONSTITUTION invariant 4). For a page
// that carries an amended headline or verdict this returns headline_counts,
// og_description and verdict_line derived the way reportRow derived them before any
// amendment was read: from the headline block, and the verdict without
// <p class="amended-verdict">. The page's TL;DR card (site-chrome.js), head tags
// (build-head-tags.js) and social card (build-site-pages.js) render these; the row,
// and so the index, the homepage, the feed and llms.txt, keeps the amended values. It is computed from
// the page when a page is rendered and is never written to docs/data/, a published
// surface that carries no band-lexicon wording. null for a page with no amendment.
export function recordedFields(html, pageRel) {
  html = String(html);
  if (amendedHeadline(html) === null && !/<p class="amended-verdict">/.test(html)) return null;
  const publishedBigHtml = (html.match(/<div class="headline">[\s\S]*?<p class="big"[^>]*>([\s\S]*?)<\/p>/) || [])[1] || '';
  const publishedRaw = textOf(publishedBigHtml);
  const publishedVerdict = verdictLine(publishedBigHtml, html, { amended: false });
  return {
    headline_counts: F(prefix(firstSentence(publishedRaw), 200), pageRel),
    og_description: F(prefix(cardDescription(publishedRaw), 160), pageRel),
    ...(publishedVerdict ? { verdict_line: F(publishedVerdict, pageRel) } : {}),
  };
}
