...
function stubReceipt(skill, idx, model, isFrontier, nowIso) {
  ...
  return buildReceipt({
    ...
    run: {
      ...
      judge: { samples: 5, temperature: null, sampling: 'surface-controlled', surface: 'stub', model_id: REPORT_004_JUDGE_MODEL, prompt_template_hash: promptTemplateHash() },
...
function tierDirection(receipt) {
  ...
  for (const id of Object.keys(by)) {
    ...
    // Overlap test via the shared geometry: WITHIN_NOISE is the value for bands that touch or overlap.
...
async function prepareReport004(opts = {}) {
  ...
  const projection = estimateRunCostUSD({ draws: SAMPLING.max, caseCount: nCasesTotal, samples: 5, models: [models.base, models.frontier], judgeModel: REPORT_004_JUDGE_MODEL });
  const overGuard = projection.totalUSD > maxUsd;
  const projectedCalls = projectCalls(nCasesTotal, 5, SAMPLING.max) * 2;
...
async function main() {
  ...
  if (execute || smokeOnly) {
    ...
    // Projection FIRST.
    const proj = estimateRunCostUSD({ draws: SAMPLING.max, caseCount: 70, samples: 5, models: [REPORT_004_BASE_MODEL, REPORT_004_FRONTIER_MODEL], judgeModel: REPORT_004_JUDGE_MODEL });
    console.log(`\nPROJECTED full-run cost: ~$${proj.totalUSD.toFixed(2)} (guard $${maxUsd})  [${proj.perModel.map((p) => `${p.model} ~$${p.usd.toFixed(2)}`).join(', ')}]`);
    console.log(`PROJECTED live calls: ~${projectCalls(70, 5, SAMPLING.max) * 2} for the full run (2 tiers × 10 suites × 70 cases × (${SAMPLING.max} draws × (2 gens + 2×5 judge))) + 2 for the light smoke.`);
