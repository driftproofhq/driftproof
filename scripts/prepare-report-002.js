...
function stubReceipt(skill, idx, substrate, model, nowIso) {
  ...
  return buildReceipt({
    ...
    run: {
      ...
      judge: { ...judgeSettingsFor(), surface: 'stub', model_id: REPORT_002_JUDGE_MODEL, prompt_template_hash: promptTemplateHash() },
...
// least EFFECT_FLOOR. Band-overlap alone, or a sub-floor move, is no separation
// detected, never a directional claim. The per-substrate direction is the tally of those
...
async function prepareReport002(opts = {}) {
  ...
  const projection = estimateRunCostUSD({ draws: SAMPLING.max, caseCount: nCasesTotal, samples: 5, models: [models.claude, models.gpt], judgeModel: REPORT_002_JUDGE_MODEL });
